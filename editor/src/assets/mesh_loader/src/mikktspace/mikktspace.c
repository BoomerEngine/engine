...
static void BuildNeighborsFast(STriInfo pTriInfos[], SEdge * pEdges, const int piTriListIn[], const int iNrTrianglesIn)
{
	...
	for (f=0; f<iNrTrianglesIn; f++)
		for (i=0; i<3; i++)
		{
			...
			pEdges[f*3+i].indices.i0 = i0 < i1 ? i0 : i1;			// put minimum index in i0
			pEdges[f*3+i].indices.i1 = !(i0 < i1) ? i0 : i1;		// put maximum index in i1
			pEdges[f*3+i].indices.f = f;							// record face number
		}
	...
	{
		if (pEdges[iCurStartIndex].indices.i0 != pEdges[i].indices.i0)
		{
	...
	for (i=1; i<iEntries; i++)
	{
		if (pEdges[iCurStartIndex].indices.i0 != pEdges[i].indices.i0 || pEdges[iCurStartIndex].indices.i1 != pEdges[i].indices.i1)
		{
			const int iL = iCurStartIndex;
			const int iR = i-1;
			//const int iElems = i-iL;
	...
	{
		const int i0=pEdges[i].indices.i0;
		const int i1=pEdges[i].indices.i1;
		const int f = pEdges[i].indices.f;
		tbool bUnassigned_A;
		...
		if (bUnassigned_A)
		{
			...
			int j=i+1, t;
			tbool bNotFound = TTRUE;
			while (j<iEntries && i0==pEdges[j].indices.i0 && i1==pEdges[j].indices.i1 && bNotFound)
			{
				tbool bUnassigned_B;
				int i0_B, i1_B;
				t = pEdges[j].indices.f;
				// flip i0_B and i1_B
				GetEdge(&i1_B, &i0_B, &edgenum_B, &piTriListIn[t*3], pEdges[j].indices.i0, pEdges[j].indices.i1);	// resolve index ordering and edge_num
