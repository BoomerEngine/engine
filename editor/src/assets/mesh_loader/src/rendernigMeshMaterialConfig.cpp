...
#include "build.h"
#include "rendernigMeshMaterialConfig.h"
...
namespace rendering
{
    ...
    RTTI_BEGIN_TYPE_CLASS(MeshMaterialConfig);
        RTTI_PROPERTY(m_materials);
    RTTI_END_TYPE();

    MeshMaterialConfig::MeshMaterialConfig()
    {}

    MeshMaterialBindingSingleMaterialData* MeshMaterialConfig::getMaterial(base::StringID name)
    {
        if (!name)
            return nullptr;
    ...
    const MeshMaterialBindingSingleMaterialData* MeshMaterialConfig::findMaterial(base::StringID name) const
    {
        if (!name)
            return nullptr;
    ...
    bool MeshMaterialConfig::captureMaterial(base::StringID name, const rendering::MaterialInstance& material, const rendering::MaterialInstance* baseMaterial)
    {
    ...
    bool MeshMaterialConfig::applyMaterial(base::StringID name, rendering::MaterialInstance& material) const
    {
