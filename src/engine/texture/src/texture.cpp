...
TRuntimeObjectPtr<Texture> Texture::CreateFromBlob(const TextureDataPtr& blob, const StringBuf& debugLabel)
{
...
TRuntimeObjectPtr<Texture> Texture::CreateFromData(const ImageCreationInfo& setup, const SourceData* sourceData, const StringBuf& debugLabel)
{
    auto ret = CreateRuntimePtr<Texture>();
...
TRuntimeObjectPtr<Texture> Texture::CreateFromUserData(IDevice* drv, TextureType type, const ImagePtr* images, uint8_t numMips, uint16_t numSlices)
{
    ...
    for (uint32_t slice=0; slice<numSlices; ++slice)
    {
        for (uint32_t mip=0; mip<numMips; ++mip, ++imageIndex)
        {
            ...
            sourceMip.data = Buffer::Create(POOL_TEMP, sourceMip.size, 1, convertedImage->pixelBuffer());
...
void Texture::describe(IFormatStream& f) const
{
    f.appendf("{}x{}x{}, {}, {}",
...
bool Texture::recreateDriverRelatedData(IDevice* driver)
{
    ...
    else if (sourceData)
    {
        ...
        for (auto& blobMip : sourceData->mips())
        {
            ...
            {
                // decompress
                auto compressedData  = OffsetPtr(persistentData, blobMip.dataOffset);
            ...
            {
                // use the direct data
                sourceMip.data = (OffsetPtr(persistentData, blobMip.dataOffset), blobMip.dataSize);
                sourceMip.offset = 0;
...
Sampler Texture::SelectSamplerForTextureGroup(IDevice* drv, StringID textureGroupName, bool hasMipmaps)
{
