...
extern ENGINE_MESH_API uint32_t GetMeshStreamStride(MeshStreamType type);

/// create empty stream buffer for given vertex stream
extern ENGINE_MESH_API Buffer CreateMeshStreamBuffer(MeshStreamType type, uint32_t vertexCount);

///----

// data stream in a mesh
struct ENGINE_MESH_API MeshRawChunkData
{
...
struct ENGINE_MESH_API MeshRawChunk
{
...
public:
    ...
    Array<MeshRawChunkData> streams; // stream of per-vertex data

    Box bounds; // position bounds in model space
};
