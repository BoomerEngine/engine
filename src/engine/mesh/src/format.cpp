...
class MeshVertexFormatRegistry : public ISingleton
{
...
public:
    ...
    bool findFormat(StringView name, MeshVertexFormat& outFormat)
    {
        return false;
    }
    ...
    void addFormat(const char* name, const std::initializer_list<MiniStreamInfo>& streams)
    {
        ...
        for (const auto& info : streams)
        {
            ...
            newStream.dataOffset = Align<uint32_t>(format.stride, 4);
...
private:
    Array<MeshVertexStreamInfo> m_streams;
    Array<MeshVertexFormatInfo> m_formats;
...
bool GetVertexFormatByName(StringView name, MeshVertexFormat& outFormat)
{
...
template<>
struct PackElement<Vector2, Half2>
{
    INLINE static void Pack(const MeshVertexQuantizationHelper& quantization, const Vector2& src, Half2& dest)
    {
        dest.x = Float16Helper::Compress(src.x);
        dest.y = Float16Helper::Compress(src.y);
    }
};
    

template<>
struct PackElement<Vector3, Ubyte4Unorm>
{
    INLINE static void Pack(const MeshVertexQuantizationHelper& quantization, const Vector3& src, Ubyte4Unorm& dest)
    {
        dest.x = FloatTo255(src.x);
        dest.y = FloatTo255(src.y);
        dest.z = FloatTo255(src.z);
        dest.w = 255;
    }
};
...
struct PackElement<NormalVector, Ubyte4Unorm>
{
    INLINE static void Pack(const MeshVertexQuantizationHelper& quantization, const NormalVector& src, Ubyte4Unorm& dest)
    {
        ...
        dest.x = FloatTo255(0.5f + 0.5f * src.x);
        dest.y = FloatTo255(0.5f + 0.5f * src.y);
        dest.z = FloatTo255(0.5f + 0.5f * src.z);
        dest.w = 255;
    }
};
...
template<>
struct PackElement<Vector4, Half4>
{
    INLINE static void Pack(const MeshVertexQuantizationHelper& quantization, const Vector4& src, Half4& dest)
    {
        dest.x = Float16Helper::Compress(src.x);
        dest.y = Float16Helper::Compress(src.y);
        dest.z = Float16Helper::Compress(src.z);
        dest.w = Float16Helper::Compress(src.w);
    }
};

template<>
struct PackElement<Vector4, Ubyte4Unorm>
{
    INLINE static void Pack(const MeshVertexQuantizationHelper& quantization, const Vector4& src, Ubyte4Unorm& dest)
    {
        dest.x = FloatTo255(src.x);
        dest.y = FloatTo255(src.y);
        dest.z = FloatTo255(src.z);
        dest.w = FloatTo255(src.w);
    }
};
...
template<>
struct PackElement<Color, Ubyte4Unorm>
{
    INLINE static void Pack(const MeshVertexQuantizationHelper& quantization, const Color& src, Ubyte4Unorm& dest)
    {
        dest.x = src.r;
        dest.y = src.g;
        dest.z = src.b;
...
template<>
struct PackElement<Vector3, QuantizedPosition_11_11_10>
{
    INLINE static void Pack(const MeshVertexQuantizationHelper& quantization, const Vector3& src, QuantizedPosition_11_11_10& dest)
    {
...
template<>
struct PackElement<Vector3, QuantizedPosition_22_22_20>
{
    INLINE static void Pack(const MeshVertexQuantizationHelper& quantization, const Vector3& src, QuantizedPosition_22_22_20& dest)
    {
...
static void PackDataT(const MeshVertexQuantizationHelper& quantization, const void* srcData, uint32_t srcStride, void* destData, uint32_t destStride, ImageFormat destFormat, uint32_t count)
{
    switch (destFormat)
    {
        ...
        case ImageFormat::RG32F:
            PackData<ST, Vector2>(quantization, srcData, srcStride, destData, destStride, count);
            break;

        case ImageFormat::RGB32F:
            PackData<ST, Vector3>(quantization, srcData, srcStride, destData, destStride, count);
            break;
        ...
        case ImageFormat::RGBA32F:
            PackData<ST, Vector4>(quantization, srcData, srcStride, destData, destStride, count);
            break;
...
{        
    ...
    case ImageFormat::RGB32F:
        FillStreamDataT<Vector3>(destData, destStride, src, count);
        break;

    case ImageFormat::RGBA32F:
        FillStreamDataT<Vector4>(destData, destStride, src, count);
        break;
...
void PackStreamData(const MeshVertexQuantizationHelper& quantization, const void* srcData, uint32_t srcStride, MeshStreamType type, void* destData, uint32_t destStride, ImageFormat destFormat, uint32_t count)
{
    ...
    {
    case MeshStreamType::Position_3F:
        PackDataT<Vector3>(quantization, srcData, srcStride, destData, destStride, destFormat, count);
        break;
    ...
    case MeshStreamType::TexCoord2_2F:
    case MeshStreamType::TexCoord3_2F:
        PackDataT<Vector2>(quantization, srcData, srcStride, destData, destStride, destFormat, count);
        break;
    ...
    case MeshStreamType::Color2_4U8:
    case MeshStreamType::Color3_4U8:
        PackDataT<Color>(quantization, srcData, srcStride, destData, destStride, destFormat, count);
        break;
    ...
    case MeshStreamType::TreeFrondData_4F:
    case MeshStreamType::TreeWindBranchData_4F:
        PackDataT<Vector4>(quantization, srcData, srcStride, destData, destStride, destFormat, count);
        break;

    case MeshStreamType::TreeBranchData_7F:
        //PackDataT<Vector4>(srcData, srcStride, destData, destStride, destFormat, count);
        break;
...
MeshVertexQuantizationHelper::MeshVertexQuantizationHelper(const Box& absoluteBounds)
...
uint32_t MeshVertexQuantizationHelper::QuantizePosition_11_11_10(const Vector3& pos) const
{
...
uint64_t MeshVertexQuantizationHelper::QuantizePosition_22_22_20(const Vector3& pos) const
{
    auto safeX = std::clamp<float>(pos.x, m_absoluteBounds.min.x, m_absoluteBounds.max.x);
    auto safeY = std::clamp<float>(pos.y, m_absoluteBounds.min.y, m_absoluteBounds.max.y);
    auto safeZ = std::clamp<float>(pos.z, m_absoluteBounds.min.z, m_absoluteBounds.max.z);
...
void MeshVertexQuantizationHelper::QuantizePositions_11_11_10(void* outData, uint32_t outDataStride, const void* inData, uint32_t inputDataStride, uint32_t count) const
{
    ...
    for (uint32_t i = 0; i < count; ++i)
    {
        *(uint32_t*)writePtr = QuantizePosition_11_11_10(*(const Vector3*)readPtr);
...
void PackVertexData(const MeshVertexQuantizationHelper& quantization, const SourceMeshStream* srcStreams, uint32_t srcStreamCount, void* destData, MeshVertexFormat destFormat, uint32_t count)
{
    ...
    for (uint32_t i = 0; i < formatInfo.numStreams; ++i)
    {
        ...
        auto* destPtr = OffsetPtr(destData, destStreamInfo.dataOffset);
...
Buffer UncompressVertexBuffer(const void* compressedVertexData, uint32_t compressedDataSize, MeshVertexFormat format, uint32_t count)
{
...
Buffer UncompressIndexBuffer(const void* compressedIndexData, uint32_t compressedDataSize, uint32_t count)
{
    Buffer ret;
