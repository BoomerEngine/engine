...
public:
    ...
    virtual CodeChunk compile(MaterialStageCompiler& compiler, StringID outputName) const override
    {
        ...
        {
            n = compiler.evalInput(this, "Normal"_id, Vector3(0, 0, 1)).conform(3);
        }
        else
        {
        ...
        {
            a = compiler.evalInput(this, "X"_id, Vector3(0, 0, 0)).conform(3);
        }
        else
        {
...
RTTI_BEGIN_TYPE_CLASS(MaterialGraphBlock_VectorReflect);
    RTTI_METADATA(graph::BlockInfoMetadata).title("reflect(x,n)").group("Vector").name("Reflect");
