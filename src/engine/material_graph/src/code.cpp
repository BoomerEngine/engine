...
CodeChunk::CodeChunk(const Vector2& val, CodeChunkType type_ /*= CodeChunkType::Numerical2*/)
    : m_type(type_)
    , m_constant(true)
    , m_text(TempString("vec2({}, {})", val.x, val.y))
{
}

CodeChunk::CodeChunk(const Vector3& val, CodeChunkType type_ /*= CodeChunkType::Numerical3*/)
    : m_type(type_)
    , m_constant(true)
    , m_text(TempString("vec3({}, {}, {})", val.x, val.y, val.z))
{
}

CodeChunk::CodeChunk(const Vector4& val, CodeChunkType type_ /*= CodeChunkType::Numerical4*/)
    : m_type(type_)
    , m_constant(true)
    , m_text(TempString("vec4({}, {}, {}, {})", val.x, val.y, val.z, val.w))
{
}

CodeChunk::CodeChunk(CodeChunkType type_, StringView text_, bool isConstant /*= false*/)
    : m_type(type_)
    , m_constant(isConstant)
    , m_text(text_)
{}

void CodeChunk::print(IFormatStream& f) const
{
    f << m_text;
}
...
CodeChunk CodeChunk::swizzle(StringView mask) const
{
...
CodeChunk CodeChunk::swizzle(uint8_t inCount, uint8_t outCount, StringView mask) const
{
    DEBUG_CHECK_EX(inCount <= components(), "Invalid swizzle for current code chunk type");
    return CodeChunk(TypeForComponentCount(outCount), TempString("({}).{}", *this, mask), m_constant);
}
...
CodeChunk CodeChunk::binaryOp(const CodeChunk& other, StringView op) const
{
    const auto outType = resolveBinaryOpType(other);
    return CodeChunk(outType, TempString("({} {} {})", *this, op, other), m_constant && other.m_constant);
}

CodeChunk CodeChunk::binaryOpF(StringView op, float value) const
{
    const auto thisComps = components();
    DEBUG_CHECK_EX(0 != thisComps, "Non numerical type cannot participate in binary operation");

    return CodeChunk(m_type, TempString("({} {} {})", *this, op, value), m_constant);
}

CodeChunk CodeChunk::binaryOpF(float value, StringView op) const
{
    const auto thisComps = components();
    DEBUG_CHECK_EX(0 != thisComps, "Non numerical type cannot participate in binary operation");

    return CodeChunk(m_type, TempString("({} {} {})", value, op, *this), m_constant);
}

CodeChunk CodeChunk::unaryOp(StringView op) const
{
    const auto thisComps = components();
    DEBUG_CHECK_EX(0 != thisComps, "Non numerical type cannot participate in unary operation");

    return CodeChunk(m_type, TempString("({}{})", op, *this), m_constant);
}
...
CodeChunk CodeChunk::saturate() const
{
    ...
    return CodeChunk(m_type, TempString("saturate({})", *this), m_constant);
}
...
CodeChunk CodeChunk::abs() const
{
    ...
    return CodeChunk(m_type, TempString("abs({})", *this), m_constant);
}
...
CodeChunk CodeChunk::length() const
{
    ...
    return CodeChunk(CodeChunkType::Numerical1, TempString("length({})", *this), m_constant);
}
...
CodeChunk CodeChunk::normalize() const
{
    ...
    return CodeChunk(m_type, TempString("normalize({})", *this), m_constant);
}
...
CodeChunk CodeChunk::sqrt() const
{
    ...
    return CodeChunk(m_type, TempString("sqrt({})", *this), m_constant);
}
...
CodeChunk CodeChunk::log() const
{
    ...
    return CodeChunk(m_type, TempString("log({})", *this), m_constant);
}
...
CodeChunk CodeChunk::log2() const
{
    ...
    return CodeChunk(m_type, TempString("log2({})", *this), m_constant);
}
...
CodeChunk CodeChunk::exp() const
{
    ...
    return CodeChunk(m_type, TempString("exp({})", *this), m_constant);
}
...
CodeChunk CodeChunk::exp2() const
{
    ...
    return CodeChunk(m_type, TempString("exp2({})", *this), m_constant);
}
...
CodeChunk CodeChunk::pow(const CodeChunk& other) const
{
    ...
    return CodeChunk(m_type, TempString("pow({}, {})", *this, other), m_constant);
}
...
CodeChunk CodeChunk::floor() const
{
    ...
    return CodeChunk(m_type, TempString("floor({})", *this), m_constant);
}
...
CodeChunk CodeChunk::round() const
{
    ...
    return CodeChunk(m_type, TempString("round({})", *this), m_constant);
}
...
CodeChunk CodeChunk::ceil() const
{
    ...
    return CodeChunk(m_type, TempString("ceil({})", *this), m_constant);
}
...
CodeChunk CodeChunk::frac() const
{
    ...
    return CodeChunk(m_type, TempString("frac({})", *this), m_constant);
}
...
CodeChunk CodeChunk::sign() const
{
    ...
    return CodeChunk(m_type, TempString("sign({})", *this), m_constant);
}
...
CodeChunk CodeChunk::ddx() const
{
    ...
    return CodeChunk(m_type, TempString("ddx({})", *this), m_constant);
}
...
CodeChunk CodeChunk::ddy() const
{
    ...
    return CodeChunk(m_type, TempString("ddy({})", *this), m_constant);
}
...
{
    return CodeChunk(m_type, TempString("{}[{}]", *this, index), m_constant);
}
...
namespace CodeChunkOp
{
    CodeChunk All(const CodeChunk& a)
    {
        ...
        return CodeChunk(CodeChunkType::Numerical1, TempString("all({})", a), a.constant());
    }

    CodeChunk Any(const CodeChunk& a)
    {
        DEBUG_CHECK_EX(a.components(), "Expected operand with components");
        return CodeChunk(CodeChunkType::Numerical1, TempString("any({})", a), a.constant());
    }

    CodeChunk Min(const CodeChunk& a, const CodeChunk& b)
    {
        const auto outType = a.resolveBinaryOpType(b);
        return CodeChunk(outType, TempString("min({}, {})", a, b), a.constant() && b.constant());
    }

    CodeChunk Max(const CodeChunk& a, const CodeChunk& b)
    {
        const auto outType = a.resolveBinaryOpType(b);
        return CodeChunk(outType, TempString("max({}, {})", a, b), a.constant() && b.constant());
    }
    ...
    CodeChunk Dot2(const CodeChunk& a, const CodeChunk& b)
    {
        ...
        return CodeChunk(CodeChunkType::Numerical1, TempString("dot({}.xy, {}.xy)", a, b), a.constant() && b.constant());
    }
    ...
    CodeChunk Dot3(const CodeChunk& a, const CodeChunk& b)
    {
        ...
        return CodeChunk(CodeChunkType::Numerical1, TempString("dot({}.xyz, {}.xyz)", a, b), a.constant() && b.constant());
    }
    ...
    CodeChunk Dot4(const CodeChunk& a, const CodeChunk& b)
    {
        ...
        return CodeChunk(CodeChunkType::Numerical1, TempString("dot({}.xyzw, {}.xyzw)", a, b), a.constant() && b.constant());
    }
    ...
    CodeChunk Clamp(const CodeChunk& x, const CodeChunk& a, const CodeChunk& b)
    {
        ...
        return CodeChunk(x.type(), TempString("clamp({}, {}, {})", x, a, b), x.constant() && a.constant() && b.constant());
    }
    ...
    CodeChunk Cross(const CodeChunk& a, const CodeChunk& b)
    {
        DEBUG_CHECK_EX(3 == a.components(), "Expected operand with 3 components");
        DEBUG_CHECK_EX(3 == b.components(), "Expected operand with 3 components");
        return CodeChunk(CodeChunkType::Numerical3, TempString("cross({}.xyz, {}.xyz)", a, b), a.constant() && b.constant());
    }
    ...
    CodeChunk Lerp(const CodeChunk& x, const CodeChunk& y, const CodeChunk& s)
    {
        ...
        return CodeChunk(x.type(), TempString("lerp({}, {}, {})", x, y, s), x.constant() && y.constant() && s.constant());
    }
    ...
    CodeChunk Sign(const CodeChunk& a)
    {
        ...
        DEBUG_CHECK_EX(comp >= 1 && comp <= 4, "Expected operand with numerical type");
        return CodeChunk(a.type(), TempString("sign({})", a), a.constant());
    }
    ...
    CodeChunk Step(const CodeChunk& a)
    {
        auto comp = a.components();
        DEBUG_CHECK_EX(comp >= 1 && comp <= 4, "Expected operand with numerical type");
        return CodeChunk(a.type(), TempString("step({})", a), a.constant());
    }
    ...
    CodeChunk SmoothStep(const CodeChunk& min, const CodeChunk& max, const CodeChunk& s)
    {
        ...
        return CodeChunk(s.type(), TempString("smoothstep({}, {}, {})", min, max, s), min.constant() && max.constant() && s.constant());
    }
    ...
    CodeChunk Mad(const CodeChunk& a, const CodeChunk& b, const CodeChunk& c)
    {
        ...
        return CodeChunk(a.type(), TempString("mad({}, {}, {})", a, b, c), a.constant() && b.constant() && c.constant());
    }
    ...
    CodeChunk Sin(const CodeChunk& a)
    {
        auto comp = a.components();
        DEBUG_CHECK_EX(comp >= 1 && comp <= 4, "Expected operand with numerical type");
        return CodeChunk(a.type(), TempString("sin({})", a), a.constant());
    }
    ...
    CodeChunk Cos(const CodeChunk& a)
    {
        auto comp = a.components();
        DEBUG_CHECK_EX(comp >= 1 && comp <= 4, "Expected operand with numerical type");
        return CodeChunk(a.type(), TempString("cos({})", a), a.constant());
    }
    ...
    CodeChunk Tan(const CodeChunk& a)
    {
        auto comp = a.components();
        DEBUG_CHECK_EX(comp >= 1 && comp <= 4, "Expected operand with numerical type");
        return CodeChunk(a.type(), TempString("tan({})", a), a.constant());
    }
    ...
    CodeChunk ArcSin(const CodeChunk& a)
    {
        auto comp = a.components();
        DEBUG_CHECK_EX(comp >= 1 && comp <= 4, "Expected operand with numerical type");
        return CodeChunk(a.type(), TempString("asin({})", a), a.constant());
    }
    ...
    CodeChunk ArcCos(const CodeChunk& a)
    {
        auto comp = a.components();
        DEBUG_CHECK_EX(comp >= 1 && comp <= 4, "Expected operand with numerical type");
        return CodeChunk(a.type(), TempString("acos({})", a), a.constant());
    }
    ...
    CodeChunk ArcTan(const CodeChunk& a)
    {
        auto comp = a.components();
        DEBUG_CHECK_EX(comp >= 1 && comp <= 4, "Expected operand with numerical type");
        return CodeChunk(a.type(), TempString("atan({})", a), a.constant());
    }
    ...
    CodeChunk ArcTan2(const CodeChunk& a, const CodeChunk& b)
    {
        ...
        DEBUG_CHECK_EX(comp >= 1 && comp <= 4, "Expected operand with numerical type");
        DEBUG_CHECK_EX(comp == b.components(), "Expected operand with the same number of components");
        return CodeChunk(a.type(), TempString("atan2({}, {})", a, b), a.constant() && b.constant());
    }
    ...
    CodeChunk Pow(const CodeChunk& a, const CodeChunk& b)
    {
        ...
        DEBUG_CHECK_EX(comp >= 1 && comp <= 4, "Expected operand with numerical type");
        DEBUG_CHECK_EX(comp == b.components(), "Expected operand with the same number of components");
        return CodeChunk(a.type(), TempString("pow({}, {})", a, b), a.constant() && b.constant());
    }
    ...
    CodeChunk Floor(const CodeChunk& a)
    {
        auto comp = a.components();
        DEBUG_CHECK_EX(comp >= 1 && comp <= 4, "Expected operand with numerical type");
        return CodeChunk(a.type(), TempString("floor({})", a), a.constant());
    }
    ...
    CodeChunk Ceil(const CodeChunk& a)
    {
        auto comp = a.components();
        DEBUG_CHECK_EX(comp >= 1 && comp <= 4, "Expected operand with numerical type");
        return CodeChunk(a.type(), TempString("ceil({})", a), a.constant());
    }
    ...
    CodeChunk Fract(const CodeChunk& a)
    {
        auto comp = a.components();
        DEBUG_CHECK_EX(comp >= 1 && comp <= 4, "Expected operand with numerical type");
        return CodeChunk(a.type(), TempString("frac({})", a), a.constant());
    }
    ...
    CodeChunk Trunc(const CodeChunk& a)
    {
        auto comp = a.components();
        DEBUG_CHECK_EX(comp >= 1 && comp <= 4, "Expected operand with numerical type");
        return CodeChunk(a.type(), TempString("trunc({})", a), a.constant());
    }
    ...
    CodeChunk Round(const CodeChunk& a)
    {
        auto comp = a.components();
        DEBUG_CHECK_EX(comp >= 1 && comp <= 4, "Expected operand with numerical type");
        return CodeChunk(a.type(), TempString("round({})", a), a.constant());
    }
    ...
    CodeChunk Normalize(const CodeChunk& a)
    {
        ...
        return CodeChunk(a.type(), TempString("normalize({})", a), a.constant());
    }

    CodeChunk Float2(const CodeChunk& a)
    {
        DEBUG_CHECK_EX(a.components() == 2, "Expected operand with 2 components");
        return CodeChunk(CodeChunkType::Numerical2, TempString("vec2({})", a), a.constant());
    }
    ...
    CodeChunk Float2(const CodeChunk& a, const CodeChunk& b)
    {
        ...
        return CodeChunk(CodeChunkType::Numerical2, TempString("vec2({}, {})", a, b), a.constant() && b.constant());
    }

    CodeChunk Float3(const CodeChunk& a)
    {
        DEBUG_CHECK_EX(a.components() == 3, "Expected operand with 3 components");
        return CodeChunk(CodeChunkType::Numerical3, TempString("vec3({})", a), a.constant());
    }
    ...
    CodeChunk Float3(const CodeChunk& a, const CodeChunk& b)
    {
        ...
        return CodeChunk(CodeChunkType::Numerical3, TempString("vec3({}, {})", a, b), a.constant() && b.constant());
    }
    ...
    CodeChunk Float3(const CodeChunk& a, const CodeChunk& b, const CodeChunk& c)
    {
        ...
        return CodeChunk(CodeChunkType::Numerical3, TempString("vec3({}, {}, {})", a, b, c), a.constant() && b.constant() && c.constant());
    }

    CodeChunk Float4(const CodeChunk& a)
    {
        DEBUG_CHECK_EX(a.components() == 4, "Expected operand with 3 components");
        return CodeChunk(CodeChunkType::Numerical4, TempString("vec4({})", a), a.constant());
    }
    ...
    CodeChunk Float4(const CodeChunk& a, const CodeChunk& b)
    {
        ...
        return CodeChunk(CodeChunkType::Numerical3, TempString("vec4({}, {})", a, b), a.constant() && b.constant());
    }
    ...
    CodeChunk Float4(const CodeChunk& a, const CodeChunk& b, const CodeChunk& c)
    {
        ...
        return CodeChunk(CodeChunkType::Numerical3, TempString("vec4({}, {}, {})", a, b, c), a.constant() && b.constant() && c.constant());
    }
    ...
    CodeChunk Float4(const CodeChunk& a, const CodeChunk& b, const CodeChunk& c, const CodeChunk& d)
    {
        ...
        return CodeChunk(CodeChunkType::Numerical3, TempString("vec4({}, {}, {}, {})", a, b, c, d), a.constant() && b.constant() && c.constant() && d.constant());
    }
...
MaterialStageCompiler::MaterialStageCompiler(const MaterialDataLayout* dataLayout, gpu::ShaderStage stage, StringView materialPath, const MaterialCompilationSetup& context)
    : m_stage(stage)
...
const MaterialDataLayoutEntry* MaterialStageCompiler::findParamEntry(StringID entryName) const
{
...
/*static CodeChunkType VarTypeToCodeChunkType(StringView type)
{
...
bool MaterialStageCompiler::resolveOutputBlock(const MaterialGraphBlock* sourceBlock, StringID socketName, const MaterialGraphBlock*& outOutputBlock, StringID& outOutputName, StringID& outOutputSwizzle) const
{
    if (sourceBlock)
    {
        if (auto inputSocket = sourceBlock->findSocket(socketName))
        {
            ...
            if (connections.size() == 1)
            {
                ...
                {
                    outOutputBlock = rtti_cast<MaterialGraphBlock>(outputSocket->block());
                    if (outOutputBlock)
                    {
                        outOutputSwizzle = outputSocket->info().m_swizzle;
                        outOutputName = outOutputSwizzle ? StringID() : outputSocket->name();
                        return true;
                    }
                }
            }
...
CodeChunk MaterialStageCompiler::evalInput(const MaterialGraphBlock* sourceBlock, StringID socketName, const CodeChunk& defaultValue)
{
    const MaterialGraphBlock* outputBlock = nullptr;
    StringID outputName, outputSwizzle;
...
void MaterialStageCompiler::includeHeader(StringView name)
{
    if (!name.empty())
        m_includes.pushBackUnique(StringBuf(name));
}
