...
public:
    ...
    virtual StringBuf content(RefT data, int colIndex = 0) const = 0;
...
private:
    struct ElemHolder : public IReferencable
    {
        T data;
        ModelIndex index;
        int displayIndex = 0;
    ...
    Array<RefPtr<ElemHolder>> m_elements;
...
{
public:
    virtual StringBuf content(RefT data, int colIndex = 0) const override
    {
        StringBuilder txt;
        txt.append(data);
        return txt.toString();
...
void SimpleTypedListModel<T, TV>::children(const ModelIndex& parent, Array<ModelIndex>& outChildrenIndices) const
{
    if (!parent)
    {
...
template< typename T, typename TV >
bool SimpleTypedListModel<T, TV>::hasChildren(const ModelIndex& parent) const
{
...
template< typename T, typename TV >
StringBuf SimpleTypedListModel<T, TV>::displayContent(const ModelIndex& id, int colIndex) const
{
...
class SimpleStringListModel : public SimpleTypedListModel<StringBuf>
{
public:
    virtual bool compare(const StringBuf& a, const StringBuf& b, int colIndex) const override
    {
        return a < b;
    }

    virtual bool filter(const StringBuf& data, const SearchPattern& filter, int colIndex) const override
    {
        return filter.testString(data);
    }

    virtual StringBuf content(const StringBuf& data, int colIndex) const override
    {
        return data;
    }
};
