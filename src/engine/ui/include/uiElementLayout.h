...
class ENGINE_UI_API ElementArea
{
public:
    ElementArea();
    ElementArea(const Position& absolutePosition, const Size& size);
    ElementArea(const Rect& pixelRect);
    ...
    {
        return Rect((int)m_absolutePosition.x, (int)m_absolutePosition.y, (int)(m_absolutePosition.x + m_size.x), (int)(m_absolutePosition.y + m_size.y));
    }
    ...
    INLINE bool contains(const Point& pos) const
    {
	...
	INLINE XForm2D placement() const
	{
		return XForm2D(m_absolutePosition.x, m_absolutePosition.y);
	}
...
class ENGINE_UI_API ElementLayout
{
public:
...
class ENGINE_UI_API ArrangedChildren
{
public:
    ArrangedChildren();
    ...
    INLINE ConstArrayIterator<Entry> end() const
    {
...
private:
    typedef InplaceArray<Entry, 20> TElements;
...
extern ENGINE_UI_API void ArrangeAxis(float requiredSize, float allowedSize, float minSize, float maxSize, ElementVerticalLayout mode, float& outSize, float& outOffset);

// align on horizontal axis
extern ENGINE_UI_API void ArrangeAxis(float requiredSize, float allowedSize, float minSize, float maxSize, ElementHorizontalLayout mode, float& outSize, float& outOffset);

// arrange element into provided area using the element style
extern ENGINE_UI_API ElementArea ArrangeElementLayout(const ElementArea& incomingInnerArea, const ElementLayout& layout, const Size& totalSize);

// arrange element into provided area using the element style
extern ENGINE_UI_API ElementArea ArrangeAreaLayout(const ElementArea& incomingInnerArea, const Size& size, ElementVerticalLayout verticalAlign, ElementHorizontalLayout horizontalAlign);
