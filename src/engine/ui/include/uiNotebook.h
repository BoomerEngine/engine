...
typedef DirectFlags<NotebookFlagBit> NotebookFlags;
...
class ENGINE_UI_API Notebook : public IElement
{
...
public:
    ...
    // list of all tabs
    INLINE const Array<IElement*>& tabs() const { return m_tabs; }

    // active tab
    INLINE IElement* activeTab() const { return m_activeTab; }

    // list of all tabs with given type (NOTE: do not cache directly)
    template< typename T >
    INLINE const Array<T*>& typedTabsRef() const
    {
        m_tempTabs.reserve(m_tabs.size());
        m_tempTabs.reset();
        for (auto* tab : m_tabs)
            if (auto* typedTab = rtti_cast<T>(tab))
                m_tempTabs.pushBack(tab);
        return (const Array<T*>&) m_tempTabs;
    }
    ...
    INLINE T* tab() const
    {
        return rtti_cast<T>(m_activeTab);
    }
...
private:
    ...
    Array<IElement*> m_tabs;

    Array<IElement*> m_customHeaderElements;

    mutable Array<IElement*> m_tempTabs;
...
protected:
    ...
    virtual StringBuf tabTitle(IElement* tab);
