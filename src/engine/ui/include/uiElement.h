...
class ENGINE_UI_API ElementDynamicSizing
{
public:
...
class ENGINE_UI_API ElementChildIterator
{
public:
...
struct ENGINE_UI_API ElementCachedChildrenLayout : public NoCopy
{
    struct Placement
    {
        Position m_localPos;
    ...
    Array<Placement> m_placements; // placements of the items
};
...
struct ElementCachedStyle : public NoCopy
{
...
struct ElementCachedGeometry : public NoCopy
{
    RTTI_DECLARE_POOL(POOL_UI_CANVAS)

public:
    canvas::Geometry* shadow = nullptr; // rendered with parent clip rect
    canvas::Geometry* background = nullptr; // rendered with widget client clip rect
    canvas::Geometry* foreground = nullptr; // rendered with widget client clip rect
    canvas::Geometry* overlay = nullptr; // rendered with parent clip rect
...
struct ElementDrawListToken : public NoCopy
{
...
/// widget timer
class ENGINE_UI_API Timer : public NoCopy
{
public:
    Timer(IElement* owner, StringID name = StringID::EMPTY()); // starts in disabled state
    Timer(IElement* owner, float timeInterval, bool oneShot = false, StringID name = StringID::EMPTY()); // starts in initialized state
    ~Timer();
    ...
    NativeTimePoint nextTick() const;
...
private:
    StringID m_name; // name of the timer, does not have to be set but if it's set the handleTimer is called on element as well
    IElement* m_owner = nullptr;
    Renderer* m_renderer = nullptr;
    NativeTimePoint m_nextTick; // when should be tick it
    NativeTimePoint m_scheduleTime; // when was the timer scheduled
    NativeTimeInterval m_tickInterval; // if not set it was a one shot timer
...
class ENGINE_UI_API IElement : public IObject
{
    RTTI_DECLARE_POOL(POOL_UI_OBJECTS)
    RTTI_DECLARE_VIRTUAL_CLASS(IElement, IObject);
...
public:
    ...
    void tooltip(StringView tooltip);
    ...
    EventFunctionBinder bind(StringID name, IElement* owner = nullptr);
    ...
    void customBackgroundColor(Color color);

    // set custom foreground style (text)
    void customForegroundColor(Color color);
    ...
    void customStyleVariant(StringID name, Variant value);

    // set a custom style override
    template< typename T >
    INLINE void customStyle(StringID name, T value)
    {
        ...
        customStyleVariant(name, CreateVariant(value));
    }

    // remove custom style override
    void removeCustomStyle(StringID name);

    /// check if the window has a given style class specified
    bool hasStyleClass(StringID styleClassName) const;
    ...
    void addStyleClass(StringID name);

    /// remove style class from the element
    /// causes full invalidation
    void removeStyleClass(StringID name);
    ...
    void styleType(StringID type);

    /// get the additional style type for element
    StringID styleType() const;

    //---

    /// evaluate style value, local styles take precedence over the common styles
    template< typename T >
    INLINE const T& evalStyleRef(StringID name, const T& defaultValue = T()) const
    {
    ...
    /// evaluate style value, local styles take precedence over the common styles
    template< typename T >
    INLINE T evalStyleValue(StringID name, T defaultValue = T()) const
    {
        if (m_customLocalStyles)
        {
            if (const auto* val = m_customLocalStyles->findVariant(name))
    ...
    /// evaluate style value, local styles take precedence over the common styles
    template< typename T >
    INLINE bool evalStyleValueIfPresent(StringID name, T& outVal) const
    {
        if (m_customLocalStyles)
        {
            if (const auto* val = m_customLocalStyles->findVariant(name))
    ...
    /// evaluate style value, local styles take precedence over the common styles
    template< typename T >
    INLINE const T* evalStyleValueIfPresentPtr(StringID name) const
    {
        if (m_customLocalStyles)
        {
            if (const auto* val = m_customLocalStyles->findVariant(name))
    ...
    virtual bool handleTimer(StringID name, float elapsedTime);
    ...
    virtual void prepareBoundaryGeometry(DataStash& stash, const ElementArea& drawArea, float pixelScale, canvas::GeometryBuilder& builder, float inset) const;

    /// prepare shadow geometry for this element
    virtual void prepareShadowGeometry(DataStash& stash, const ElementArea& drawArea, float pixelScale, canvas::GeometryBuilder& builder) const;

    /// prepare background geometry for this element
    virtual void prepareBackgroundGeometry(DataStash& stash, const ElementArea& drawArea, float pixelScale, canvas::GeometryBuilder& builder) const;

    /// prepare foreground geometry for this element
    virtual void prepareForegroundGeometry(DataStash& stash, const ElementArea& drawArea, float pixelScale, canvas::GeometryBuilder& builder) const;

    /// prepare overlay geometry for this element
    virtual void prepareOverlayGeometry(DataStash& stash, const ElementArea& drawArea, float pixelScale, canvas::GeometryBuilder& builder) const;
    ...
    virtual void renderShadow(DataStash& stash, const ElementArea& drawArea, canvas::Canvas& canvas, float mergedOpacity);

    /// render background geometry, by default rendered the cached client geometry
    virtual void renderBackground(DataStash& stash, const ElementArea& drawArea, canvas::Canvas& canvas, float mergedOpacity);

    /// render foreground geometry
    virtual void renderForeground(DataStash& stash, const ElementArea& drawArea, canvas::Canvas& canvas, float mergedOpacity);

    /// render overlay elements
    virtual void renderOverlay(DataStash& stash, const ElementArea& drawArea, canvas::Canvas& canvas, float mergedOpacity);
    ...
    /// render element
    void render(HitCache& hitCache, DataStash& stash, const ElementArea& outerArea, const ElementArea& outerClipArea, canvas::Canvas& canvas, float mergedOpacity, const ElementDynamicSizing* parentDynamicSizing);
    ...
    bool callGeneric(IElement* originator, StringID name, const Variant& data);

    // call event with specific data
    template< typename T >
    INLINE bool call(IElement* originator, StringID name, const T& data)
    {
        static_assert(!std::is_same<T, Variant>::value, "To call with a variant use callGeneric");
        return callGeneric(originator, name, CreateVariant(data));
    }

    /// call event on this element, notifies any registered callbacks, no data version
    INLINE bool call(StringID name)
    {
        return callGeneric(this, name, Variant::EMPTY());
    }

    /// call event on this element, notifies any registered callbacks, generic data version
    INLINE bool callGeneric(StringID name, const Variant& data)
    {
    ...
    // call event with specific data
    template< typename T >
    INLINE bool call(StringID name, const T& data)
    {
        static_assert(!std::is_same<T, Variant>::value, "To call with a variant use callGeneric");
        return callGeneric(name, CreateVariant(data));
    }

    /// call event on this element, notifies any registered callbacks, no data version
    INLINE bool call(IElement* originator, StringID name)
    {
        return callGeneric(originator, name, Variant::EMPTY());
    }
    ...
    INLINE RefPtr<T> createChild(Args&& ... args)
    {
        auto child = RefNew<T>(std::forward< Args >(args)...);
        attachChild(child);
        return child;
    }
    ...
    /// create and attach a child element
    template< typename T = IElement, typename... Args >
    INLINE RefPtr<T> createNamedChild(StringID name, Args&& ... args)
    {
        auto child = RefNew<T>(std::forward< Args >(args)...);
        child->name(name);
        attachChild(child);
    ...
    /// create and attach a child element
    template< typename T = IElement, typename... Args >
    INLINE RefPtr<T> createChildWithType(StringID typeName, Args&& ... args)
    {
        auto child = RefNew<T>(std::forward< Args >(args)...);
    ...
    /// create and attach a child element
    template< typename T = IElement, typename... Args >
    INLINE RefPtr<T> createNamedChildWithType(StringID name, StringID typeName, Args&& ... args)
    {
        auto child = RefNew<T>(std::forward< Args >(args)...);
        child->name(name);
        child->styleType(typeName);
    ...
    bool bindNamedProperties(const HashMap<StringView, IElement*>& namedElements);
    ...
    template< typename T >
    INLINE T* findParent(StringID name = StringID::EMPTY()) const
    {
        return rtti_cast<T>(findParent(T::GetStaticClass(), name));
    }

    /// find parent element implementing given class
    IElement* findParent(SpecificClassType<IElement> baseClass, StringID name = StringID::EMPTY()) const;

    /// find child element by name, if name is not specified any matching element will be returned
    IElement* findChildByName(const char* name, SpecificClassType<IElement> baseClass, bool recrusive = true) const;

    /// find child element by name, if name is not specified any matching element will be returned
    IElement* findChildByName(StringID name, SpecificClassType<IElement> baseClass, bool recrusive = true) const;

    /// find child element by name, if name is not specified any matching element will be returned
    template< typename T >
    INLINE T* findChildByName(const char* name, bool recrusive = true) const
    {
        return rtti_cast<T>(findChildByName(name, T::GetStaticClass(), recrusive));
    }

    /// find child element by name, if name is not specified any matching element will be returned
    template< typename T >
    INLINE T* findChildByName(StringID name, bool recrusive=true) const
    {
        return rtti_cast<T>(findChildByName(name, T::GetStaticClass(), recrusive));
    }
...
protected:
    ...
    bool hasStylePseudoClass(StringID name) const;

    /// add a style pseudo class to the widget
    void addStylePseudoClass(StringID name);

    /// remove style pseudo class from the widget
    void removeStylePseudoClass(StringID name);
    ...
    virtual void renderCustomOverlayElements(HitCache& hitCache, DataStash& stash, const ElementArea& outerArea, const ElementArea& outerClipArea, canvas::Canvas& canvas, float mergedOpacity);
    ...
    virtual bool adjustBackgroundStyle(canvas::RenderStyle& outStyle, float& outBorderWidth) const;

    /// adjust dynamically border style before rendering
    virtual bool adjustBorderStyle(canvas::RenderStyle& outStyle, float& outBorderWidth) const;
    ...
    /// create and attach a child element
    template< typename T = IElement, typename... Args >
    INLINE RefPtr<T> createInternalChild(Args&& ... args)
    {
        auto child = RefNew<T>(std::forward< Args >(args)...);
    ...
    /// create and attach a child element
    template< typename T = IElement, typename... Args >
    INLINE RefPtr<T> createInternalNamedChild(StringID name, Args&& ... args)
    {
        auto child = RefNew<T>(std::forward< Args >(args)...);
        child->name(name);
        attachElementToChildList(child);
    ...
    /// create and attach a child element
    template< typename T = IElement, typename... Args >
    INLINE RefPtr<T> createInternalNamedChildWithType(StringID name, StringID typeName, Args&& ... args)
    {
        auto child = RefNew<T>(std::forward< Args >(args)...);
        child->name(name);
        child->styleType(typeName);
    ...
    /// create and attach a child element
    template< typename T = IElement, typename... Args >
    INLINE RefPtr<T> createInternalChildWithType(StringID typeName, Args&& ... args)
    {
        auto child = RefNew<T>(std::forward< Args >(args)...);
        child->styleType(typeName);
        attachElementToChildList(child);
...
private:
    ...
    // local custom styles
    VariantTable* m_customLocalStyles = nullptr;
    ...
    typedef HashSet<IElement*> TChildrenUpdateList;
    ...
    UniquePtr<ElementCachedChildrenLayout> m_cachedChildrenPlacement;

    // overlay elements, valid only if we have overlay element ;)
    Array<IElement*>* m_overlayElements = nullptr;
...
extern ENGINE_UI_API ElementPtr Layout(LayoutMode mode, const std::initializer_list<ElementPtr>& elements);

// make an element from all child elements arranged vertically
extern ENGINE_UI_API ElementPtr LayoutVertically(const std::initializer_list<ElementPtr>& elements);

// make an element from all child elements arranged vertically
extern ENGINE_UI_API ElementPtr LayoutHorizontally(const std::initializer_list<ElementPtr>& elements);
