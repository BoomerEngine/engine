...
public:
    ...
    const Array<T*>& linearizedList() const
    {
...
class ENGINE_UI_API ItemView : public AbstractItemView
{
...
public:
    ...
    void indicesTouchingArea(const ElementArea& area, Array<ModelIndex>& outItems) const;
...
protected:
    ...
    {
        Array<ViewItem*> m_orderedChildren;
        Array<ViewItem*> m_displayOrder; // sorted, empty indicates no sorting needed
    ...
    struct ViewItem
    {
        ...
        //UntypedRefWeakPtr m_indexRefPtr;
    ...
    void buildSortedList(const Array<ViewItem*>& rawItems, Array<ViewItem*>& outSortedItems) const;
    ...
    bool buildIndicesFromDisplayRange(int firstDisplayIndex, int lastDisplayIndex, Array<ModelIndex>& outIndices) const;
    bool buildIndicesFromItemRange(const ViewItem* firstDisplayItem, const ViewItem* lastDisplayItem, Array<ModelIndex>& outIndices) const;
    bool buildIndicesFromIndexRange(const ModelIndex& firstIndex, const ModelIndex& lastIndex, Array<ModelIndex>& outIndices) const;
    ...
    INLINE void forEachVisualization(const ViewItemChildren& items, const std::function<void(T*)>& func) const
    {
        ...
        {
            if (auto elem = rtti_cast<T>(child->m_innerContent.get()))
                func(elem);
            else if (auto elem = rtti_cast<T>(child->m_content.get()))
                func(elem);
...
private:
    mutable Array<ViewItem*> m_tempStorage;
};
