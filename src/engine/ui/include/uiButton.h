...
typedef DirectFlags<ButtonModeBit> ButtonMode;

/// a simple button
class ENGINE_UI_API Button : public IElement
{
    RTTI_DECLARE_VIRTUAL_CLASS(Button, IElement);

public:
    Button(ButtonMode mode = ButtonModeBit::EventOnClickRelease);
    Button(StringView text, ButtonMode mode = ButtonModeBit::EventOnClickRelease);
