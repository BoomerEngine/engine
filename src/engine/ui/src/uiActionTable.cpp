...
class ActionKeyMap : public ISingleton
{
...
KeyShortcut::KeyShortcut()
    : m_key(input::KeyCode::KEY_INVALID)
    , m_shift(false)
    , m_ctrl(false)
    , m_alt(false)
{}
...
bool KeyShortcut::matches(const input::KeyEvent& evt) const
{
    if (evt.keyCode() != m_key)
        return false;
    if (evt.keyMask().mask().test(input::KeyMaskBit::ANY_ALT) != m_alt)
        return false;
    if (evt.keyMask().mask().test(input::KeyMaskBit::ANY_SHIFT) != m_shift)
        return false;
    if (evt.keyMask().mask().test(input::KeyMaskBit::ANY_CTRL) != m_ctrl)
        return false;
    return true;
}
...
void ActionTable::remove(StringID name)
{
...
void ActionTable::bindShortcut(StringID name, KeyShortcut shortcut)
{
...
const ActionInfo* ActionTable::find(StringID name) const
{
...
ActionInfo* ActionTable::get(StringID name)
{
    DEBUG_CHECK_EX(name, "Invalid action name");
    ActionInfo* ret = nullptr;
    if (name)
...
EventFunctionBinder ActionTable::bindCommand(StringID name)
{
...
EventFunctionBinder ActionTable::bindFilter(StringID name)
{
    if (auto* info = get(name))
...
EventFunctionBinder ActionTable::bindToggle(StringID name)
{
    if (auto* info = get(name))
...
ActionStatusFlags ActionTable::status(StringID name) const
{
...
bool ActionTable::run(StringID name, IElement* source) const
{
    if (const auto* info = find(name))
    {
        if (info->runFunc)
...
bool ActionTable::processKeyEvent(const input::KeyEvent& evt) const
{
