    ...
    : m_pos(0.5f)
{
    ...
    if (displayCaption)
        m_text = createNamedChild<TextLabel>("ProgressCaption"_id, TempString("{}%", m_pos * 100.0f));
}

void ProgressBar::position(float pos, StringView customText)
{
    ...
    if (m_pos != clampedPos)
    {
        ...
        {
            if (customText.empty())
                m_text->text(TempString("{}%", Prec(pos * 100.0f, 1)));
            else
                m_text->text(TempString("{}%: {}", Prec(pos * 100.0f, 1), customText));
        }
