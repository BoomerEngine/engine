...
namespace Scintilla
{
    ...
    const FontInfo* FontCache::cacheFont(const FontParameters& fp)
    {
        ...
        auto lock = boomer::CreateLock(m_lock);
    ...
    static boomer::RefPtr<boomer::Font> SelectFont(bool bold, bool italic)
    {
        ...
        static const auto resFontItalic = boomer::LoadFontFromDepotPath("/engine/interface/fonts/DejaVuSansMono-Oblique.ttf");
        static const auto resFontBoldItalic = boomer::LoadFontFromDepotPath("/engine/interface/fonts/DejaVuSansMono-BoldOblique.ttf");
    ...
    {
		m_buildBuffer->builder.strokeColor(boomer::Color(fore.GetRed(), fore.GetGreen(), fore.GetBlue()));
    }

    void SurfaceImpl::FillColour(const ColourDesired &back)
    {
        m_buildBuffer->builder.fillColor(boomer::Color(back.GetRed(), back.GetGreen(), back.GetBlue()));
    }
    ...
    {
        // set colors
        m_buildBuffer->builder.strokeColor(boomer::Color(outline.GetRed(), outline.GetGreen(), outline.GetBlue(), alphaOutline));
        m_buildBuffer->builder.fillColor(boomer::Color(fill.GetRed(), fill.GetGreen(), fill.GetBlue(), alphaFill));
    ...
    void SurfaceImpl::DrawTextTransparent(PRectangle rc, Font &font_, XYPOSITION ybase, std::string_view text, ColourDesired fore)
    {
        ...
        if (nullptr != fontInfo)
        {
            ...
            // render
            m_buildBuffer->builder.fillColor(boomer::Color(fore.GetRed(), fore.GetGreen(), fore.GetBlue()));
