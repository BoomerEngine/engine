...
DockPanel* DockNotebook::activeTab() const
{
    return rtti_cast<DockPanel>(Notebook::activeTab());
}
...
{
    if (auto dockPanel = rtti_cast<DockPanel>(tab))
...
StringBuf DockNotebook::tabTitle(IElement* tab)
{
    if (auto dockPanel = rtti_cast<DockPanel>(tab))
