...
ConfigProperty<float> cvDefaultNotificationTime("Editor.Notification", "DefaultNotificationTime", 5.0f);
...
void NotificationPanel::postNotification(StringID group, IElement* notification, float visibleFor /*= 0.0f*/, StringID additionalFrameStyle)
{
    if (notification)
    {
        if (group.empty())
        ...
        auto lock = CreateLock(m_notificationLock);
        ...
        n->content = RefPtr<IElement>(AddRef(notification));
        n->timeLeft = visibleFor;
        m_notifications.pushBack(n);
    }
}

void NotificationPanel::postTextNotification(StringID group, StringView txt, MessageType type /*= MessageType::Info*/, float visibleFor /*= 0.0f*/)
{
    if (txt)
    {
        StringID frameStyle;
        ...
        auto data = RefNew<TextLabel>(txt);
        postNotification(group, data, visibleFor, frameStyle);
    }
}

void NotificationPanel::postLargeTextNotification(StringID group, StringView txt, MessageType type /*= MessageType::Info*/, float visibleFor /*= 0.0f*/)
{
...
void PostWindowMessage(IElement* owner, MessageType type, StringID group, StringView txt)
{
    auto safeOwner = RefWeakPtr<IElement>(owner);
    auto safeText = StringBuf(txt);
    ...
    RunSync("NotifyWindow") << [safeOwner, type, group, safeText](FIBER_FUNC)
    {
        if (auto owner = safeOwner.lock())
        {
            ...
            {
                auto notificationPanel = window->findChildByName<NotificationPanel>(StringID::EMPTY(), false);
...
void PostNotificationMessage(IElement* owner, MessageType type, StringID group, StringView txt)
{
    auto safeOwner = RefWeakPtr<IElement>(owner);
    auto safeText = StringBuf(txt);
    ...
    RunSync("NotifyWindow") << [safeOwner, type, group, safeText](FIBER_FUNC)
    {
        if (auto ownerPtr = safeOwner.lock())
        {
            ...
            while (owner)
            {
                if (auto notificationPanel = owner->findChildByName<NotificationPanel>(StringID::EMPTY(), false))
                    notificationPanel->postTextNotification(group, safeText, type);
