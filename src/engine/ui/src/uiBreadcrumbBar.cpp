...
bool BreadcrumbBar::navigate(uint32_t parentBreadcrumb, const Array<StringBuf>& childrenNames, bool activateNewBreadcrumb)
{
    // empty
    ...
    Array<BreadcrumbPtr> newBreadcrumbs;
    for (const auto& childName : childrenNames)
    {
    ...
    {
        auto ptr = RefNew<BreadcrumbContainer>(this, newBreadCrumb, m_breadcrumbs.size());
...
void BreadcrumbBar::root(const BreadcrumbPtr& breadcrumb)
{
    ...
    if (breadcrumb)
    {
        auto ptr = RefNew<BreadcrumbContainer>(this, breadcrumb, 0);
        m_breadcrumbs.pushBack(ptr);
        attachChild(ptr);
    }
}
