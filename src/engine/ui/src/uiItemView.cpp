...
void ItemView::columnCount(uint32_t count)
{
    ...
    {
        m_columnCount = count;
        layoutMode(count ? LayoutMode::Vertical : LayoutMode::Icons);
...
void ItemView::indicesTouchingArea(const ElementArea& area, Array<ModelIndex>& outItems) const
{
    // build/get a linearized list of items in their display order
    const auto &linearizedList = m_displayList.linearizedList();
...
bool ItemView::buildIndicesFromDisplayRange(int firstDisplayIndex, int lastDisplayIndex, Array<ModelIndex>& outIndices) const
{
...
bool ItemView::buildIndicesFromItemRange(const ViewItem* firstDisplayItem, const ViewItem* lastDisplayItem, Array<ModelIndex>& outIndices) const
{
...
bool ItemView::buildIndicesFromIndexRange(const ModelIndex& firstIndex, const ModelIndex& lastIndex, Array<ModelIndex>& outIndices) const
{
    ...
    if (evt.leftClicked() || evt.rightClicked())
    {
        ...
        if (evt.leftClicked() || !selection().contains(clickedItem))
        {
            if (evt.keyMask().isShiftDown())
            {
                ...
                {
                    InplaceArray<ModelIndex, 64> selectedIndices;
...
ElementPtr ItemView::queryTooltipElement(const Position& absolutePosition, ElementArea& outArea) const
{
