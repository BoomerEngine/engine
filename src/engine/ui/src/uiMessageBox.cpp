...
ButtonPtr MakeButton(Window* window, IElement* container, MessageButton type, const MessageBoxSetup& setup)
{
    ...
    auto button = container->createChildWithType<Button>("PushButton"_id, caption);
...
MessageButton ShowMessageBox(IElement* owner, const MessageBoxSetup& setup)
{
    ...
    // message area
    {
        auto box = window->createChild<IElement>();
        ...
        box->createChild<TextLabel>(iconName)->customMargins(20, 20, 10, 10);

        auto text = box->createChild<TextLabel>(setup.m_message);
