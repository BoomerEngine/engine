...
namespace prv
{
    ...
    class LayutDataParser : public NoCopy
    {
    public:
        LayutDataParser(StringView txt)
        {
            curPtr = txt.data();
    ...
    private:
        ...
        INLINE static const char* ParseMarkup(const char* cur, const char* endPtr, StringView& outMarkup, StringView& outArg)
        {
            if (*cur == '[')
            {
                cur += 1;
                ...
                if (cur < endPtr)
                {
                    outMarkup = StringView(start, cur);
        ...
        InplaceArray<Color, 10> colorStack;
        InplaceArray<Color, 10> tagStack;
        InplaceArray<LayoutVerticalAlign, 10> alignStack;
        InplaceArray<float, 10> sizeStack;
        ...
        void finishTextRegion(bool newLine = false)
        {
            if (codes.size() > regionStartChar)
            {
                ...
                region.color = colorStack.empty() ? Color::WHITE : colorStack.back();
                region.tagColor = tagStack.empty() ? Color(0, 0, 0, 0) : tagStack.back();
        ...
        bool parseColor(StringView txt, Color& outColor) const
        {
            return Color::Parse(txt, outColor);
        }

        void pushColor(StringView txt)
        {
            Color color;
        ...
        void pushTagColor(StringView txt)
        {
            Color color;
            if (parseColor(txt, color))
        ...
        bool processMarkup(StringView markup, StringView arg)
        {
            if (markup == "b")
            {
                finishTextRegion();
            ...
            else if (markup == "img")
            {
                ...
                auto& region = regions.emplaceBack();
                region.imageName = StringID(arg);
                region.valign = alignStack.empty() ? LayoutVerticalAlign::Middle : alignStack.back();
                region.tagColor = tagStack.empty() ? Color(0, 0, 0, 0) : tagStack.back();
                region.color = colorStack.empty() ? Color(255, 255, 255, 255) : colorStack.back();
                region.flagTag = !tagStack.empty();
                region.flagColor = !colorStack.empty();
                return true;
            }
        ...
        void sliceText()
        {
            ...
            while (curPtr < endPtr)
            {
                ...
                // eat UTF8 char
                auto code = utf8::NextChar(curPtr, endPtr);
                if (code < 32)
                {
                    if (code == '\n')
                    {
    ...
    LayoutData::LayoutData(const StringBuf& txt)
        : text(txt)
    {
    ...
    void LayoutData::render(DataStash& stash, const style::FontFamily& fonts, float pixelScale, float fontSize, bool bold, bool italic, Color baseColor, float wrapWidth, LayoutDisplayData& outData) const
    {
