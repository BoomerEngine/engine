...
DockLayoutNode::DockLayoutNode(DockContainer* container, DockLayoutNode* parent, float splitNode /*= 0.0f*/)
    ...
    , m_parent(parent)
{
    m_notebook = RefNew<DockNotebook>(this);
}

DockLayoutNode& DockLayoutNode::top(float split /*= 0.0f*/)
{
    if (!m_topNode)
        m_topNode = RefNew<DockLayoutNode>(m_container, this, split);
    return *m_topNode;
}
...
{
    if (!m_leftNode)
        m_leftNode = RefNew<DockLayoutNode>(m_container, this, split);
    return *m_leftNode;
}
...
{
    if (!m_bottomNode)
        m_bottomNode = RefNew<DockLayoutNode>(m_container, this, split);
    return *m_bottomNode;
}
...
{
    if (!m_rightNode)
        m_rightNode = RefNew<DockLayoutNode>(m_container, this, split);
    return *m_rightNode;
}
    ...
    if (m_rightNode)
    {
        if (auto content = m_rightNode->visualize())
        {
            if (ret)
            {
                ...
                auto splitter = RefNew<Splitter>(Direction::Vertical, splitFactor);
    ...
    if (m_bottomNode)
    {
        if (auto content = m_bottomNode->visualize())
        {
            if (ret)
            {
                ...
                auto splitter = RefNew<Splitter>(Direction::Horizontal, splitFactor);
                splitter->attachChild(ret);
                splitter->attachChild(content);
                ret = splitter;
            }
...
void DockLayoutNode::fillViewMenu(MenuButtonContainer* menu)
{
    ...
    InplaceArray<DockPanel*, 10> persistentPanels;
