...
Window::Window(WindowFeatureFlags flags, StringView title)
    : m_title(title)
    , m_flags(flags)
{
...
void Window::requestTitleChange(StringView newTitle)
{
    if (m_title != newTitle)
    {
        m_title = StringBuf(newTitle);
        ...
        requests->requestedTitle = StringBuf(newTitle);
    }
}
...
void Window::queryInitialPlacementSetup(WindowInitialPlacementSetup& outSetup) const
{
    ...
    outSetup.owner = rtti_cast<Window>(m_modalOwner.lock());
