...
void CanvasArea::addElement(ICanvasAreaElement* elem, Position initialPlacement, StringID kind)
{
    if (elem)
    {
...
void CanvasArea::filterElement(StringID kind, bool visible)
{
    if (kind)
    {
        if (visible)
...
Position CanvasArea::absoluteToVirtual(const Point& absolutePos) const
{
    return absoluteToVirtual(absolutePos.toVector());
}

void CanvasArea::scrollToOffset(Vector2 offset)
{
...
void CanvasArea::processScheduledZoomChange()
{
    ...
    if (1 == m_scheduledZoomChangeMode || 3 == m_scheduledZoomChangeMode)
    {
        ...
        Position minPos = Position::INF();
        Position maxPos = -Position::INF();
    ...
    else if (2 == m_scheduledZoomChangeMode)
    {
        ...
        setZoom(Vector2(1, 1), center);
    }

    m_scheduledZoomChangeMode = 0;
}

void CanvasArea::setZoom(Vector2 zoom, Position pos)
{
...
void CanvasArea::statusMessage(StringID id, StringView txt, Color color /*= Color::WHITE*/, float decayTime /*= 3.0f*/)
{
        ...
        if (const auto fonts = evalStyleValueIfPresentPtr<style::FontFamily>("font-family"_id))
        {
            if (fonts->normal)
            {
                ...
                for (const auto& msg : m_statusMessages)
                {
                    ...
                    FontMetrics metrics;
                    fonts->normal->measureText(styleParams, msg.text, metrics);
