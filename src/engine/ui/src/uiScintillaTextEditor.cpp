...
ScintillaTextEditor::ScintillaTextEditor()
{
    ...
    // create scrollbar
    m_verticalScrollBar = RefNew<Scrollbar>();

    // create internal editor
    m_scintilla = RefNew<ScintillaInnerWidget>(this, m_verticalScrollBar);
    attachChild(m_scintilla);
    attachChild(m_verticalScrollBar);

    // allow to take whole space
    //m_verticalScrollBar->customProportion(1.0f);
    m_verticalScrollBar->customVerticalAligment(ElementVerticalLayout::Expand);
    m_scintilla->customVerticalAligment(ElementVerticalLayout::Expand);
    m_scintilla->customHorizontalAligment(ElementHorizontalLayout ::Expand);
...
void ScintillaTextEditor::text(const StringBuf& txt)
{
...
StringBuf ScintillaTextEditor::text() const
{
    // allocate buffer
    Array<char> buffer;
    ...
    return StringBuf(buffer.typedData());
}
...
StringBuf ScintillaTextEditor::selectedText() const
{
    auto start = m_scintilla->WndProc(SCI_GETSELECTIONSTART, 0, 0);
    auto end = m_scintilla->WndProc(SCI_GETSELECTIONEND, 0, 0);
    if (end <= start)
