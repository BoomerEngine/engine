...
public:
	...
	const T& ValueAt(Sci::Position position) const {
		...
		Sci::Position startPartition = starts->PositionFromPartition(partition);
	...
	void InsertSpace(Sci::Position position, Sci::Position insertLength) {
		...
		Sci::Position startPartition = starts->PositionFromPartition(partition);
		if (startPartition == position) {
			bool positionOccupied = values->ValueAt(partition) != T();
	...
	void DeletePosition(Sci::Position position) {
		...
		if (startPartition == position) {
			if (partition == 0) {
				...
				ClearValue(partition);
                fprintf(stderr, "SparseVector: deleting end partition.\n");
			} else {
				ClearValue(partition);
				starts->RemovePartition(partition);
				values->Delete(partition);
	...
	void Check() const {
		if (Length() < 0) {
            fprintf(stderr, "SparseVector: Length can not be negative.\n");
		}
		if (starts->Partitions() < 1) {
            fprintf(stderr,"SparseVector: Must always have 1 or more partitions.\n");
		}
		if (starts->Partitions() != values->Length() - 1) {
            fprintf(stderr,"SparseVector: Partitions and values different lengths.\n");
		}
		// The final element can not be set
		if (values->ValueAt(values->Length() - 1) != T()) {
            fprintf(stderr,"SparseVector: Unused style at end changed.\n");
		}
	}
};
