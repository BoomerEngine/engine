...
struct ENGINE_WORLD_API NodeTemplatePrefabSetup
{
...
struct ENGINE_WORLD_API NodeTemplateBehaviorEntry
{
...
class ENGINE_WORLD_API NodeTemplate : public IObject
{
...
struct ENGINE_WORLD_API NodeTemplateCompiledData : public IReferencable
{
    RTTI_DECLARE_POOL(POOL_WORLD_OBJECTS)
...
ENGINE_WORLD_API NodeTemplatePtr UnpackTopLevelPrefabs(const NodeTemplate* rootNode);

// compile flattened node (resolve and embed prefabs referenced in the node, NOTE: only prefabs from root node are embedded, use "Explode" to embed all prefabs)
// NOTE: may return NULL if node contains NO DATA and could be discarded for all practical purposes
ENGINE_WORLD_API NodeTemplatePtr CompileWithInjectedBaseNodes(const NodeTemplate* rootNode, const Array<const NodeTemplate*>& additionalBaseNodes);

// compile an entity from list of templates, may return NULL if no entity data is present in any template
// NOTE: it's not recursive
ENGINE_WORLD_API CAN_YIELD EntityPtr CompileEntity(const Array<const NodeTemplate*>& templates, Transform* outEntityLocalTransform = nullptr);
