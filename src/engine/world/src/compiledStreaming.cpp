...
RTTI_BEGIN_TYPE_CLASS(WorldObserverInfo);
    RTTI_PROPERTY(position);
    RTTI_PROPERTY(velocity);
RTTI_END_TYPE();

WorldObserverInfo::WorldObserverInfo()
{}

///---

CompiledStreamingIslandInfo::CompiledStreamingIslandInfo()
{}

///---

static uint32_t CountIslands(CompiledStreamingIsland* island)
{
    uint32_t ret = 1;
...
static uint32_t CountTotalIslands(const CompiledWorldData* scene)
{
    uint32_t ret = 0;
...
static void ExtractIslands(CompiledStreamingIslandInfo* extarctedIslands, uint32_t& index, CompiledStreamingIslandInfo* parent, const CompiledStreamingIsland* data)
{
...
RTTI_BEGIN_TYPE_NATIVE_CLASS(CompiledStreamingTask);
RTTI_END_TYPE();

CompiledStreamingTask::CompiledStreamingTask(const CompiledWorldStreaming* data, const Array<WorldObserverInfo>& observers, const Array<CompiledStreamingIslandInfo>& islands, const Array<uint32_t>& attachedIslands, const BitSet<>& attachedIslandsMask)
...
void CompiledStreamingTask::requestCancel()
{

}

static bool CheckStreamingRange(const Array<WorldObserverInfo>& observers, const Box& box)
{
...
void CompiledStreamingTask::process()
{
...
RTTI_BEGIN_TYPE_NATIVE_CLASS(CompiledWorldStreaming);
RTTI_END_TYPE();

CompiledWorldStreaming::CompiledWorldStreaming(const CompiledWorldData* world)
    : m_world(AddRef(world))
{
    const auto numIslands = CountTotalIslands(world);
    ...
    uint32_t index = 0;
    for (const auto& rootIsland : world->rootIslands())
        ExtractIslands(m_islands.typedData(), index, nullptr, rootIsland);
}

CompiledWorldStreaming::~CompiledWorldStreaming()
{}

void CompiledWorldStreaming::unbindEntities()
{
...
WorldStreamingTaskPtr CompiledWorldStreaming::createStreamingTask(const Array<WorldObserverInfo>& observers) const
{
    return RefNew<CompiledStreamingTask>(this, observers, m_islands, m_attachedIslands, m_attachedIslandsMask);
}

void CompiledWorldStreaming::applyStreamingTask(World* world, const IWorldStreamingTask* anyTask)
{
    const auto* task = rtti_cast<CompiledStreamingTask>(anyTask);
    DEBUG_CHECK_RETURN_EX(task, "Invalid task");
