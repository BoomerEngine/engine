...
void Prefab::setup(RawEntity* root)
{
    if (root)
    {
        m_root = AddRef(root);
...
EntityPtr Prefab::compile(StringID appearance, Array<EntityPtr>& outAllEntities, bool loadImports) const
{
    if (!m_root)
        return nullptr;

    EntityStaticIDBuilder path;
    if (auto data = CompileEntityHierarchy(path, m_root, loadImports))
    {
