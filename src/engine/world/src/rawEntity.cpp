...
RTTI_BEGIN_TYPE_STRUCT(RawEntityPrefabSetup);
RTTI_PROPERTY(enabled);
RTTI_PROPERTY(prefab);
RTTI_PROPERTY(appearance);
RTTI_END_TYPE();

RawEntityPrefabSetup::RawEntityPrefabSetup()
{}

RawEntityPrefabSetup::RawEntityPrefabSetup(const PrefabRef& prefab_, bool enabled_ /*= true*/)
    : prefab(prefab_)
    , enabled(enabled_)
{}
...
RTTI_BEGIN_TYPE_CLASS(RawEntityBehaviorEntry);
RTTI_PROPERTY(name);
RTTI_PROPERTY(data);
RTTI_END_TYPE();

RawEntityBehaviorEntry::RawEntityBehaviorEntry()
{}

//--
            
RTTI_BEGIN_TYPE_CLASS(RawEntity);
...
RawEntityCompiledData::RawEntityCompiledData()
{}
...
static void CollectComponentTemplates(const Array<const RawEntity*>& templates, ComponentTemplateList& outTemplates)
{
...
EntityPtr CompileEntity(const Array<const RawEntity*>& templates, bool loadImports, Transform* outEntityLocalTransform /*= nullptr*/)
{
    PC_SCOPE_LVL1(CreateEntity);

    // collect valid entity templates to build an entity from
    ObjectIndirectTemplateCompiler entityTemplateCompiler(loadImports);
    for (const auto& dataTemplate : templates)
    {
...
{
    StringID name;
    InplaceArray<const RawEntity*, 10> templatesToMerge;
    const RawEntity* originalRootNode = nullptr;
};

static void SuckInPrefab(const RawEntityPrefabSetup& prefabEntry, HashSet<const Prefab*>& allVisitedPrefabs, Array<const RawEntity*>& outPrefabRoots)
{
...
static RawEntityPtr CompileMergedNode(const NodeMergeStack& stack)
{
    auto ret = RefNew<RawEntity>();
    ret->m_name = stack.name;
    DEBUG_CHECK(ret->m_name);
    ...
    {
        HashMap<StringID, Array<const RawEntity*>> childrenNodes;
        for (const auto* ptr : stack.templatesToMerge)
...
RawEntityPtr CompileWithInjectedBaseNodes(const RawEntity* rootNode, const Array<const RawEntity*>& additionalBaseNodes)
{
...
RawEntityPtr UnpackTopLevelPrefabs(const RawEntity* rootNode)
{
    // always merge the original editable node - if there are no prefabs we will get a copy of data
    NodeMergeStack mergeStack;
...
RefPtr<HierarchyEntity> ProcessSingleEntity(int depth, EntityStaticIDBuilder& path, const Array<const RawEntity*>& inputTemplates, const Transform& parentToWorld, bool loadImports)
{
    auto ret = RefNew<HierarchyEntity>();
    ret->id = path.toID();
    ret->name = inputTemplates[0]->m_name;
    DEBUG_CHECK(ret->name);
    TRACE_INFO("HierarchyEntity, name: '{}', path: '{}', ID: '{}'", ret->name, path.toString(), Hex(ret->id));

    InplaceArray<const RawEntity*, 20> templates;
    {
        HashSet<const Prefab*> visitedPrefabs;
    ...
    // compile entity data
    {
        ...
        ret->localToParent = compiler.compileTransform().toTransform();
        ret->localToWorld = ret->localToParent * parentToWorld;
        ...
        // extract other info
        ret->attachTransformToParentEntity = compiler.compileValueOrDefault<bool>("attachToParentEntity"_id, false);
    }
    ...
    if (depth < MAX_DEPTH)
    {
        ...
        for (auto pair : childrenNodes.pairs())
        {
            ...
            if (auto child = ProcessSingleEntity(depth + 1, path, pair.value, ret->localToWorld, loadImports))
...
RefPtr<HierarchyEntity> CompileEntityHierarchy(const EntityStaticIDBuilder& path, const RawEntity* rootTemplateNode, bool loadImports)
{
    InplaceArray<const RawEntity*, 10> nodeTemplates;
    if (rootTemplateNode)
        nodeTemplates.pushBack(rootTemplateNode);

    EntityStaticIDBuilder localPath(path);
    return ProcessSingleEntity(0, localPath, nodeTemplates, Transform::IDENTITY(), loadImports);
}
