...
{
    for (base::image::ImageViewSliceIterator z(src); z; ++z)
    {
        for (base::image::ImageViewRowIterator y(z); y; ++y)
        {
            const auto* src = (const ST*)y.data();
            for (base::image::ImageViewPixelIterator x(y); x; ++x, writePtr += N, src += N)
            {
                switch (N)
                {
...
{
    for (base::image::ImageViewSliceIterator z(dest); z; ++z)
    {
        for (base::image::ImageViewRowIterator y(z); y; ++y)
        {
            auto* dest = (DT*)y.data();
            for (base::image::ImageViewPixelIterator x(y); x; ++x, readPtr += N, dest += N)
            {
                switch (N)
                {
