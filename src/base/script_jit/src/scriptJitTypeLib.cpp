...
bool JITTypeLib::updateMemberLayout(JITType* type)
{
    ...
    for (auto& prop : type->fields)
    {
        ...
        prop.runtimeOffset = base::Align(newSize, propAlignment);
...
void JITTypeLib::printImports(base::IFormatStream &f) const
{
...
const JITType* JITTypeLib::createScriptedStruct(const base::script::StubClass *stub)
{
    JITType* ret = m_mem.create<JITType>();
    ret->native = false;
    ret->imported = false;
