...
struct ImVec4
{
    ...
    ImVec4(base::Color color) {
        x = color.r / 255.0f;
        y = color.g / 255.0f;
        z = color.b / 255.0f;
        w = color.a / 255.0f;
    }
    /*ImVec4(uint32_t colorValue) {
        base::Color color(colorValue);
        x = color.r / 255.0f;
        y = color.g / 255.0f;
        z = color.b / 255.0f;
        w = color.a / 255.0f;
...
namespace ImGui
{
    ...
    IMGUI_API void*         ImGuiAlloc(size_t size);
    IMGUI_API void          ImGuiFree(void* ptr);
...
#define IM_ALLOC(_SIZE)                     ImGui::ImGuiAlloc(_SIZE)
#define IM_FREE(_PTR)                       ImGui::ImGuiFree(_PTR)
#define IM_PLACEMENT_NEW(_PTR)              new(ImNewDummy(), _PTR)
#define IM_NEW(_TYPE)                       new(ImNewDummy(), ImGui::ImGuiAlloc(sizeof(_TYPE))) _TYPE
template<typename T> void IM_DELETE(T* p)   { if (p) { p->~T(); ImGui::ImGuiFree(p); } }
...
struct ImGuiIO
{
    ...
    int         MetricsActiveAllocations;       // Number of active allocations, updated by ImGuiAlloc/ImGuiFree based on current context. May be off if you have multiple imgui contexts.
...
struct ImDrawCmd
{
    ...
    ImDrawCmd() { ElemCount = 0; TextureId = 0; VtxOffset = IdxOffset = 0;  UserCallback = NULL; UserCallbackData = NULL; }
};
