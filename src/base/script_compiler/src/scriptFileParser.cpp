...
extern int bsc_parse(base::script::FileParsingContext& context, base::script::FileParsingTokenStream& tokens);
...
namespace base
{
    namespace script
    {
        ...
        bool FileParser::processCode(const StubFile* fileStub, const Buffer& code)
        {
            ...
            auto fileParser  = m_mem.create<base::parser::TextFilePreprocessor>(m_mem, base::parser::IIncludeHandler::GetEmptyHandler(), parserErrorHandler, base::parser::ICommentEater::StandardComments(), lang);
            if (!fileParser->processContent(code, fileStub->depotPath))
                return true; // errors will be reported via the interface

            //tokens.printFull(base::logging::ILogStream::GetStream());
