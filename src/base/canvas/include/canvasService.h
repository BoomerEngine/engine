...
public:
	...
	virtual void updateGlyphCache(rendering::GPUCommandWriter& cmd, const AtlasUpdate& update) = 0;

	// flush updates to atlas entries
	virtual void updateAtlas(rendering::GPUCommandWriter& cmd, ImageAtlasIndex atlasIndex, const AtlasUpdate& update) = 0;
};
...
public:
	...
	// sync asset changes
	void syncAssetChanges(rendering::GPUCommandWriter& cmd, uint64_t atlasMask, uint64_t glpyhMask, ICanvasAtlasSync& sync);
