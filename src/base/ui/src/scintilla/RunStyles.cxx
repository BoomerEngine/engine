...
void RunStyles<DISTANCE, STYLE>::Check() const {
	if (Length() < 0) {
        fprintf(stderr, "RunStyles: Length can not be negative.\n");
	}
	if (starts->Partitions() < 1) {
        fprintf(stderr, "RunStyles: Must always have 1 or more partitions.\n");
	}
	if (starts->Partitions() != styles->Length()-1) {
        fprintf(stderr, "RunStyles: Partitions and styles different lengths.\n");
	}
	DISTANCE start=0;
	while (start < Length()) {
		const DISTANCE end = EndRun(start);
		if (start >= end) {
            fprintf(stderr,"RunStyles: Partition is 0 length.\n");
		}
		start = end;
	}
	if (styles->ValueAt(styles->Length()-1) != 0) {
        fprintf(stderr,"RunStyles: Unused style at end changed.\n");
	}
	for (ptrdiff_t j=1; j<styles->Length()-1; j++) {
		if (styles->ValueAt(j) == styles->ValueAt(j-1)) {
            fprintf(stderr,"RunStyles: Style of a partition same as previous.\n");
		}
	}
}
