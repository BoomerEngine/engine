...
ScintillaInnerWidget::ScintillaInnerWidget(ScintillaTextEditor* owner, const base::RefPtr<Scrollbar>& scrollbar)
    : m_owner(owner)
...
bool ScintillaInnerWidget::handleCursorQuery(const ElementArea &area, const Position &absolutePosition, base::input::CursorType &outCursorType) const
{
...
public:
    ScintillaInnerWidgetMouseDragAction(IElement* owner)
        : MouseInputAction(owner, base::input::KeyCode::KEY_MOUSE0, true)
        , m_requestExit(false)
    {}
...
InputActionPtr ScintillaInnerWidget::handleMouseClick(const ElementArea &area, const base::input::MouseClickEvent &evt)
{
...
void ScintillaInnerWidget::renderForeground(DataStash& stash, const ElementArea& drawArea, base::canvas::Canvas& canvas, float mergedOpacity)
{
