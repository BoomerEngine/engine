...
namespace base
{
    ...
    static simplecpp::TokenString DEFINE("define");
    static simplecpp::TokenString UNDEF("undef");

    static simplecpp::TokenString INCLUDE("include");

    static simplecpp::TokenString ERROR("error");
    static simplecpp::TokenString WARNING("warning");

    static simplecpp::TokenString IF("if");
    static simplecpp::TokenString IFDEF("ifdef");
    static simplecpp::TokenString IFNDEF("ifndef");
    static simplecpp::TokenString DEFINED("defined");
    static simplecpp::TokenString ELSE("else");
    static simplecpp::TokenString ELIF("elif");
    static simplecpp::TokenString ENDIF("endif");

    static simplecpp::TokenString PRAGMA("pragma");
    static simplecpp::TokenString ONCE("once");
    ...
    static unsigned short andSkipBOM(std::istream &istr)
    {
        int ch1 = istr.peek();
    ...
    void simplecpp::TokenList::readfile(std::istream &istr, const std::string &filename, OutputList *outputList)
    {
        ...
        const unsigned short bom = andSkipBOM(istr);
        ...
        while (istr.good()) {
            ...
            // comment
                ...
                std::string::size_type pos = currentToken.find_last_not_of(" \t");
            ...
            // string / char literal
                ...
                if (ch == '\"' && !prefix.empty() && *cback()->str().rbegin() == 'R') {
                    ...
                    std::string endOfRawString(')' + delim + currentToken);
    ...
    static bool isFloatSuffix(const simplecpp::Token *tok)
    {
        if (!tok || tok->str().size() != 1U)
            return false;
        char c = (char)std::tolower(tok->str()[0]);
        return c == 'f' || c == 'l';
    }
    ...
    void simplecpp::TokenList::combineOperators()
    {
        ...
        for (Token *tok = front(); tok; tok = tok->next) {
            ...
            // match: [0-9.]+E [+-] [0-9]+
            char lastChar = tok->str()[tok->str().size() - 1];
    ...
    static std::string NOTEQ("not_eq");
    ...
    static std::string BITAND("bitand");
    static std::string BITOR("bitor");
    static std::string XOR("xor");
    ...
    std::string simplecpp::TokenList::readUntil(std::istream &istr, const Location &location, char start, char end, OutputList *outputList, unsigned int bom)
    {
        std::string ret;
        ret += start;
    ...
    namespace simplecpp {
            ...
            const Token * expand(TokenList * const output,
                ...
                while (output2.cback() && rawtok) {
                    ...
                    if (macro == macros.end() || !macro->second.functionLike())
                        break;
                    TokenList rawtokens2(inputFiles);
                    Location loc(macro2tok->location);
                    while (macro2tok) {
        ...
        private:
            ...
            unsigned int argNum(const TokenString &str) const {
                unsigned int par = 0;
                while (par < args.size()) {
                    if (str == args[par])
                        return par;
            ...
            std::vector<const Token *> macroParameters(const Token *nameTokInst, bool calledInDefine) const {
            ...
            const Token * expand(TokenList * const output, const Location &loc, const Token * const nameTokInst, const std::map<TokenString,Macro> &macros, std::set<TokenString> expandedmacros) const {
                ...
                bool calledInDefine = (loc.fileIndex != nameTokInst->location.fileIndex ||
                                             loc.line < nameTokInst->location.line);

                std::vector<const Token*> parametertokens1(macroParameters(nameTokInst, calledInDefine));
            ...
            const Token *expandToken(TokenList *output, const Location &loc, const Token *tok, const std::map<TokenString,Macro> &macros, const std::set<TokenString> &expandedmacros, const std::vector<const Token*> &parametertokens) const {
                ...
                if (it != macros.end() && expandedmacros.find(tok->str()) == expandedmacros.end()) {
                    ...
                    if (defToken) {
                        bool def = (macros.find(defToken->str()) != macros.end());
            ...
            bool expandArg(TokenList *output, const Token *tok, const std::vector<const Token*> &parametertokens) const {
                ...
                const unsigned int argnr = argNum(tok->str());
                if (argnr >= args.size())
                    return false;
            ...
            const Token *expandHashHash(TokenList *output, const Location &loc, const Token *tok, const std::map<TokenString, Macro> &macros, const std::set<TokenString> &expandedmacros, const std::vector<const Token*> &parametertokens) const {
                ...
                bool varargs = variadic && args.size() >= 1U && B->str() == args[args.size()-1U];
    ...
    public:
        ...
        bool cacheEntry(const std::string& path, std::string* returnPath) {
    ...
    static bool realFileName(const std::string &f, std::string *result)
    {
        ...
        if (!realFileNameMap.cacheEntry(f, result)) {
    ...
    static std::string realFilename(const std::string &f)
    {
        ...
        if (realFilePathMap.cacheEntry(f, &ret))
            return ret;
    ...
    namespace simplecpp {
        ...
        std::string simplifyPath(std::string path)
        {
            ...
            while ((pos = path.find("/..", pos)) != std::string::npos) {
                ...
                std::string::size_type pos1 = path.rfind('/', pos - 1U) + 1U;
                std::string previousSubPath = path.substr(pos1, pos-pos1);
                if (previousSubPath == "..") {
                    // don't simplify
                    ++pos;
                } else {
    ...
    static std::string openHeader(std::ifstream &f, const simplecpp::DUI &dui, const std::string &sourcefile, const std::string &header, bool systemheader)
    {
        ...
        if (!systemheader) {
            if (sourcefile.find_first_of("\\/") != std::string::npos) {
                std::string s = sourcefile.substr(0, sourcefile.find_last_of("\\/") + 1U) + header;
    ...
    static std::string fileName(const std::map<std::string, simplecpp::TokenList *> &filedata, const std::string &sourcefile, const std::string &header, const simplecpp::DUI &dui, bool systemheader)
    {
        if (filedata.empty()) {
            return "";
        }
        ...
        if (!systemheader) {
            if (sourcefile.find_first_of("\\/") != std::string::npos) {
                std::string s(simplecpp::simplifyPath(sourcefile.substr(0, sourcefile.find_last_of("\\/") + 1U) + header));
    ...
    {
        return !fileName(filedata, sourcefile, header, dui, systemheader).empty();
    }
    ...
    std::map<std::string, simplecpp::TokenList*> simplecpp::load(const simplecpp::TokenList &rawtokens, std::vector<std::string> &fileNumbers, const simplecpp::DUI &dui, simplecpp::OutputList *outputList)
    {
        ...
        for (const Token *rawtok = rawtokens.cfront(); rawtok || !filelist.empty(); rawtok = rawtok ? rawtok->next : NULL) {
            ...
            std::string header(realFilename(htok->str().substr(1U, htok->str().size() - 2U)));
            if (hasFile(ret, sourcefile, header, dui, systemheader))
                continue;

            std::ifstream f;
            std::string header2 = openHeader(f,dui,sourcefile,header,systemheader);
            if (!f.is_open())
                continue;
    ...
    void simplecpp::preprocess(simplecpp::TokenList &output, const simplecpp::TokenList &rawtokens, std::vector<std::string> &files, std::map<std::string, simplecpp::TokenList *> &filedata, const simplecpp::DUI &dui, simplecpp::OutputList *outputList, std::list<simplecpp::MacroUsage> *macroUsage)
    {
        ...
        for (std::list<std::string>::const_iterator it = dui.defines.begin(); it != dui.defines.end(); ++it) {
            ...
            std::string::size_type eq = macrostr.find('=');
            std::string::size_type par = macrostr.find('(');
            std::string macroname = macrostr.substr(0, std::min(eq,par));
            if (dui.undefined.find(macroname) != dui.undefined.end())
                continue;
            std::string lhs(macrostr.substr(0,eq));
            std::string rhs(eq==std::string::npos ? std::string("1") : macrostr.substr(eq+1));
            Macro macro(lhs, rhs, files);
        ...
        for (const Token *rawtok = NULL; rawtok || !includetokenstack.empty();) {
            ...
            if (rawtok->op == '#' && !sameline(rawtok->previous, rawtok)) {
                ...
                if (rawtok->str() == DEFINE) {
                    ...
                    bool systemheader = (inctok->op == '<');
                    std::string header(realFilename(inctok->str().substr(1U, inctok->str().size() - 2U)));
                    std::string header2 = fileName(filedata, rawtok->location.file(), header, dui, systemheader);
                    if (header2.empty()) {
                        // try to load file..
                        std::ifstream f;
