...
ConfigProperty<bool> cvDumpaST("Rendering.Compiler", "DumpaST", false);
...
bool CodeLibrary::parseCode(ITextErrorReporter& err)
{
    ...
    for (auto& entry : entries)
    {
        ...
        auto code  = Analyzecode(allocator(), err, entry.tokens, *this, entry.function, entry.program);
        if (!code)
        {
            valid = false;
            continue;
...
bool CodeLibrary::loadFile(TokenList& tokens, ITextErrorReporter& err)
{
    ...
    auto root  = Analyzefile(m_allocator, err, tokens);
    if (!root)
        return false;
