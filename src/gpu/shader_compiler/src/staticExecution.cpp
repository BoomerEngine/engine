...
ExecutionValue ExecutionValue::compositeElement(const StringID name) const
{
...
void ExecutionValue::print(IFormatStream& f) const
{
    if (isReference())
        m_ref.print(f);
    else
...
void ExecutionStack::buildProgramParams()
{
    ...
    InplaceArray<const Program*, 8> basePrograms;
...
ExecutionResult ExecutionStack::callFunction(const Array<ExecutionValue>& args, ExecutionValue& result)
{
...
DataValueRef ExecutionStack::resolveGlobalConstantValue(const DataParameter* param, parser::IErrorReporter& err)
{
    ...
    if (!param->initializerCode)
    {
        err.reportError(param->loc, TempString("Constant {} has no initialization and cannot be used", param->name));
        return DataValueRef();
    }
    ...
    if (ret != ExecutionResult::Finished)
    {
        err.reportError(param->loc, TempString("Failed to evaluate constant {}", param->name));
        return DataValueRef();
    }
    ...
    {
        err.reportError(param->loc, TempString("Evaluation of constant {} did not produce a defined value, code may not be unequivocal", param->name));
        return DataValueRef();
    }
    ...
    {
        err.reportError(param->loc, TempString("Evaluation of constant {} did not produce a loadable value, code may not be unequivocal", param->name));
        return DataValueRef();
    }
...
{
    DEBUG_CHECK_EX(code->dataTypeResolved(), TempString("Code at '{}' was not properly type-checked before execution", code->location()));
    ...
    switch (op)
    {
        ...
        case OpCode::ParamRef:
        {
            ...
            {
                stack.errorReporter().reportWarning(code->location(), TempString("Unable to resolve reference to parameter '{}'", param->name));
        ...
        case OpCode::Call:
        {
            ...
            if (callFunction->program() != nullptr) // function requires a context
            {
                ...
                if (callContext.component(0).isProgramInstance())
                {
                    ...
                    if (!callFunction)
                    {
                        stack.errorReporter().reportWarning(code->location(), TempString("Missing function '{}' in '{}'", functionName, callProgramInstance->program()->name()));
                        return stack.returnCode(code->location(), ExecutionResult::Error);
                    }
                ...
                {
                    stack.errorReporter().reportWarning(code->location(), TempString("Trying to call function '{}' but no program context can be established", functionName));
                    return stack.returnCode(code->location(), ExecutionResult::Error);
                }
            }
        ...
        case OpCode::ProgramInstance:
        {
            ...
            for (auto childNode  : code->children())
            {
                ...
                {
                    stack.errorReporter().reportWarning(code->location(), TempString("Program instance variable '{}' should be known at compile time and it isn't", programParamName));
                    return stack.returnCode(code->location(), ExecutionResult::Error);
                }
                ...
                {
                    stack.errorReporter().reportWarning(code->location(), TempString("Loading error", programParamName));
                    return stack.returnCode(code->location(), ExecutionResult::Error);
                }
