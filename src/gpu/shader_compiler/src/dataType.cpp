...
void ArrayCounts::print(IFormatStream& f) const
{
    for (uint32_t i = 0; i < MAX_ARRAY_DIMS; ++i)
    {
        if (m_sizes[i])
...
bool ResourceType::calcTypeHash(CRC64& crc) const
{
    crc << (char)type;
...
void ResourceType::print(IFormatStream& f) const
{
    f << type;
...
void DataType::print(IFormatStream& ret) const
{
...
bool DataType::calcTypeHash(CRC64& crc) const
{
...
DataType DataType::applyFlags(const DirectFlags<TypeFlags>& flags) const
{
    DataType ret(*this);
    ret.m_flags = flags;
    return ret;
