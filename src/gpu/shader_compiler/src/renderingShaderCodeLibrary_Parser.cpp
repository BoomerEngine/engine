...
ConfigProperty<bool> cvDumpShaderAST("Rendering.Compiler", "DumpShaderAST", false);

//---

static bool ExtractVectorTypeSizes(StringView view, uint32_t& numCols, uint32_t& numRows)
{
    StringParser parser(view);
...
void CodeLibrary::findDescriptorEntry(const ResourceTable* table, StringID name, Array<ResolvedDescriptorEntry>& outEntries) const
{
...
void CodeLibrary::findUniformEntry(const ResourceTable* table, StringID name, Array<ResolvedDescriptorEntry>& outEntries) const
{
    for (const auto& entry : table->members())
    {
...
void CodeLibrary::findGlobalUniformEntry(StringID name, Array<ResolvedDescriptorEntry>& outEntries) const
{
    // TODO: cache
...
void CodeLibrary::findGlobalDescriptorEntry(StringID name, Array<ResolvedDescriptorEntry>& outEntries) const
{
    // TODO: cache
    for (const auto* table : m_typeLibrary->allResourceTables())
...
bool CodeLibrary::resolveTypeInner(const TypeReference* typeRef, DataType& outType, parser::IErrorReporter& err, bool reportErrors) const
{
    ...
    {
        if (reportErrors)
            err.reportError(typeRef->location, TempString("Undefined type used"));
        return false;
    }
    ...
    if (typeRef->name == "program" || typeRef->name == "shader")
    {
        ...
        {
            if (reportErrors)
                err.reportError(typeRef->location, TempString("Unrecognized {} {}", typeRef->name, typeRef->innerType));
            return false;
        }
    ...
    else if (typeRef->name.beginsWith("int"))
    {
        ...
        {
            if (reportErrors)
                err.reportError(typeRef->location, TempString("Invalid vector/matrix type '{}'", typeRef->name));
            return false;
        }
    ...
    // unresolved type
    if (reportErrors)
        err.reportError(typeRef->location, TempString("Unable to resolve type '{}'", typeRef->name));
    return false;
}
...
static bool ValidateFlags(const Element* elem, const ElementFlags flags, parser::IErrorReporter& err)
{
    ...
    for (uint32_t i=0; i<(uint32_t)ElementFlag::MAX; ++i)
    {
        auto flag = (ElementFlag)i;
        if (elem->flags.test(flag) && !flags.test(flag))
        {
            err.reportError(elem->location, TempString("Flag {} is not allowed on {}", flag, elem->name));
            valid = false;
        }
    }
...
bool CodeLibrary::createStructureElements(const Element* parent, CompositeType* structType, parser::IErrorReporter& err)
{
    ...
    {
        if (elem->type == ElementType::StructElement || elem->type == ElementType::DescriptorConstantTableElement)
        {
            // member must be unqiue
            auto memberName = StringID(elem->name);
            if (INDEX_NONE != structType->memberIndex(memberName))
            {
                err.reportError(elem->location, TempString("Structure '{}' already contains member '{}'", structType->name(), memberName));
                valid = false;
                continue;
            }
            ...
            {
                err.reportError(elem->location, TempString("Member '{}' does not have a valid type", memberName));
                valid = false;
                continue;
            }
            ...
            {
                err.reportError(elem->location, TempString("Member '{}' uses unsupported type (structures can only hold numerical data)", memberName));
                valid = false;
                continue;
            }

            // get decorations
            ElementFlags allowedFlags;
...
bool CodeLibrary::createDescriptorElements(const Element* parent, ResourceTable* table, parser::IErrorReporter& err)
{
    ...
    HashMap<StringID, const Element*> alreadyDeclaredParameters;
    for (Element* elem : parent->children)
    {
        if (elem->type == ElementType::DescriptorResourceElement)
        {
            if (INDEX_NONE != table->memberIndex(elem->name))
            {
                err.reportError(elem->location, TempString("Member '{}' of descriptor '{}' was already defined", elem->name, table->name()));
                valid = false;
                continue;
            }
            ...
            if (!type.valid())
            {
                err.reportError(elem->location, TempString("Resource type for '{}' in descriptor '{}' is invalid: {}", elem->name, table->name(), errorStr));
                valid = false;
                continue;
            }
			...
			if (type.resource().type == DeviceObjectViewType::SampledImage)
			{
				...
				{
					err.reportError(elem->location, TempString("Sampled image '{}' in descriptor '{}' should have sampler specified via attribute(sampler=xxx)", elem->name, table->name()));
					valid = false;
					continue;
				}
				...
				if (localSampler == -1)
				{
					staticSampler = typeLibrary().findStaticSampler(StringID::Find(samplerName));
					if (!staticSampler)
					{
						err.reportError(elem->location, TempString("Sampler '{}' used in entry '{}' in descriptor '{}' was not found", samplerName, elem->name, table->name()));
						valid = false;
						continue;
					}
				}
        ...
        else if (elem->type == ElementType::DescriptorConstantTable)
        {
            auto attributes = elem->gatherAttributes();
            ...
            if (auto packingValue = attributes.value("packing"_id))
            {
                ...
                {
                    err.reportError(elem->location, TempString("Unknown packing type '{}' for member '{}' of descriptor '{}'", packingValue, elem->name, table->name()));
                    valid = false;
                    continue;
                }
            }

            auto compositeName = StringID(TempString("_{}_Consts{}", table->name(), constantBufferIndex++));
            if (INDEX_NONE != table->memberIndex(compositeName))
            {
                err.reportError(elem->location, TempString("Member '{}' of descriptor '{}' already defined", compositeName, table->name()));
                valid = false;
                continue;
            }
            ...
            for (Element* memberElem : elem->children)
            {
                if (const auto* existingElem = alreadyDeclaredParameters.findSafe(memberElem->name, nullptr))
                {
                    err.reportError(memberElem->location, TempString("Member '{}' of constant buffer in descriptor '{}' was already defined somewhere in that descriptor", memberElem->name, table->name()));
                    valid = false;
                    continue;
                }
                else
...
bool CodeLibrary::createProgramElementsPass1(const Element* parent, Program* program, parser::IErrorReporter& err)
{
	...
	for (const auto& attr : program->attributes().attributes.pairs())
	{
		...
		{
			const auto* rs = typeLibrary().findStaticRenderStates(StringID::Find(attr.value));
			if (rs)
			{
				if (program->staticRenderStates().contains(rs))
				{
					err.reportWarning(program->location(), TempString("Render states '{}' were already included in this program", attr.value));
				}
				else
				{
			...
			{
				err.reportError(program->location(), TempString("Unable to find static render states '{}'", attr.value));
				valid = false;
				continue;
			}
		}
    ...
    for (Element* elem : parent->children)
    {
        if (elem->type == ElementType::Using)
        {
            // find the base program
            auto baseProgram  = findProgram(elem->name);
            if (!baseProgram)
            {
                err.reportError(elem->location, TempString("Unknown base program '{}'", elem->name));
                valid = false;
                continue;
            }
            ...
            {
                err.reportError(elem->location, TempString("Recursive program dependency between '{}' and '{}'", elem->name, program->name()));
                valid = false;
                continue;
            }
            ...
            {
                err.reportError(elem->location, TempString("Base program '{}' must be the same type as '{}'", elem->name, program->name()));
                valid = false;
                continue;
            }*/
            ...
            {
                err.reportWarning(elem->location, TempString("Base program '{}' already declared as base for '{}'", elem->name, program->name()));
                continue;
            }
...
bool CodeLibrary::createProgramElementsPass2(const Element* parent, Program* program, parser::IErrorReporter& err)
{
    ...
    for (Element* elem : parent->children)
    {
        if (elem->type == ElementType::Variable)
        {
            ...
            {
                err.reportError(elem->location, TempString("Unable to resolve type for variable '{}'", elem->name));
                valid = false;
                continue;
            }
            ...
            {
                err.reportError(elem->location, TempString("Can't start variable names with 'gl_'"));
                valid = false;
                continue;
            }
            ...
            if (existingVar)
            {
                err.reportError(elem->location, TempString("Variable '{}' is already defined at '{}'", elem->name, existingVar->loc));
                valid = false;
                continue;
            }
            ...
            {
                // determine type
				if (elem->flags.test(ElementFlag::In))
					param->scope = DataParameterScope::StageInput;
				else if (elem->flags.test(ElementFlag::Out))
					param->scope = DataParameterScope::StageOutput;
                else if (elem->flags.test(ElementFlag::Vertex))
                    param->scope = DataParameterScope::VertexInput;
                else if (elem->flags.test(ElementFlag::Const))
                    param->scope = DataParameterScope::GlobalConst;
                else if (elem->flags.test(ElementFlag::Shared))
                    param->scope = DataParameterScope::GroupShared;
                else
                {
                    err.reportError(elem->location, TempString("Variable '{}' has no scope defined, should be 'in', 'out', 'static' or 'uniform'", elem->name));
                    valid = false;
                    continue;
                }
            }
            ...
            {
                err.reportError(elem->location, TempString("Resource reference '{}' can only be declared as uniform", elem->name));
                valid = false;
            }
            else if (param->scope == DataParameterScope::StageInput || param->scope == DataParameterScope::StageOutput)
            {
                ...
                if (dataType.isArray())
                {
                    ...
                    {
                        err.reportError(elem->location, TempString("In/Out parameter '{}' can only be one dimensional array", elem->name));
                        valid = false;
                        continue;
                    }
                    ...
                    {
                        err.reportError(elem->location, TempString("In/Out parameter '{}' can only have undefined size (ie. []) but {} was specified", elem->name, dataType.arrayCounts().outermostCount()));
                        valid = false;
                        continue;
                    }
                }
                ...
                {
                    err.reportError(elem->location, TempString("In/Out parameter '{}' should be a numerical scalar or a simple vector", elem->name));
                    valid = false;
                    continue;
                }
            }
            else if (param->scope == DataParameterScope::VertexInput)
            {
                ...
                {
                    err.reportError(elem->location, TempString("Vertex input parameter '{}' must be a structure", elem->name));
                    valid = false;
                    continue;;
                }
                ...
                {
                    err.reportError(elem->location, TempString("Vertex input parameter '{}' must be a proper structure", elem->name));
                    valid = false;
                    continue;;
                }
                ...
                {
                    err.reportError(elem->location, TempString("Structure '{}' used for vertex input parameter '{}' must use vertex packing mode 'attribute(packing=vertex)'", dataType.composite().name(), elem->name));
                    valid = false;
                    continue;
                }
            }
            else if (param->scope == DataParameterScope::GlobalParameter)
            {
                ...
                {
                    err.reportError(elem->location, TempString("Uniform parameter '{}' should be a numerical value or a resource", elem->name));
                    valid = false;
                    continue;
                }
            }
            ...
            {
                err.reportError(elem->location, TempString("Constant '{}' should have default value specified", elem->name));
                valid = false;
            }
        ...
        else if (elem->type == ElementType::Function)
        {
            /*// if the function already defined ?
            if (auto localExistingFunction  = program->findFunction(elem->name, false))
            {
                err.reportError(elem->location, TempString("Function {} was already defined at {}", elem->name, localExistingFunction->location()));
                valid = false;
                continue;
            }*/
            ...
            {
                err.reportError(elem->location, TempString("Function '{}' has undefined return type", elem->name));
                valid = false;
                continue;
            }

            // resolve parameters
            InplaceArray<DataParameter*, 16> functionArgs;
            InplaceArray<StringID, 16> functionArgsNames;
            for (auto param  : elem->children)
            {
                auto name = StringID(param->name);

                // already defined ?
                if (functionArgsNames.contains(name))
                {
                    err.reportError(elem->location, TempString("Function '{}' argument '{}' was already declared", elem->name, name));
                    valid = false;
                    continue;
                }
                ...
                {
                    err.reportError(param->location, TempString("Function '{}' argument '{}' has undefined type", elem->name, name));
                    valid = false;
                    continue;
                }

                // if the types are out/inout than the value passed must be a l-value
                if (param->flags.test(ElementFlag::Out))
                {
                ...
                {
                    err.reportError(elem->location, TempString("Argument '{}' of type '{}' must be declared as \"const\" as it has to be known at the compile time", param->name, paramType));
                    valid = false;
                }*/
            ...
            /*FunctionFlags flags;
            if (elem->flags.test(ElementFlag::Override))
            ...
            if (baseFunction)
            {
                ...
                /*if (!elem->flags.test(ElementFlag::Override))
                {
                    err.reportError(elem->location, BaseTempString<1024>("Function '{}' was already defined at '{}' yet we are not marking it as override", elem->name, baseFunction->location()));
                    valid = false;
                    continue;
                }*/
                ...
                {
                    auto newName = StringID(TempString("{}_prev", elem->name));
                ...
                {
                    valid = false;
                    err.reportError(function->location(), BaseTempString<1024>("Function has different return type '{}' than base function from program {} ({}) defined at {}",
                ...
                {
                    valid = false;
                    err.reportError(function->location(), BaseTempString<1024>("Function has different argument count ({}) than base function from program '{}' ({}) defined at {}",
                ...
                else
                {
                    ...
                    for (uint32_t i = 0; i < baseFunction->inputParameters().size(); ++i)
                    {
                        ...
                        {
                            valid = false;
                            err.reportError(function->location(), BaseTempString<1024>("Function argument '{}' has different type ({}) than in base function from program '{}' ({}) defined at {}",
            ...
            /*else
            {
                ...
                if (elem->flags.test(ElementFlag::Override))
                {
                    err.reportError(elem->location, BaseTempString<1024>("Function '{}' is marked as override but there's no base function to override. Did you add proper 'implements XXX' ?", elem->name));
                    valid = false;
                    continue;
                }
            }*/
...
bool CodeLibrary::createElements(const Element* root, parser::IErrorReporter& err)
{
    ...
    // create stubs
    for (Element* elem : root->children)
    {
        if (elem->type == ElementType::Program)
        {
            // get the program
        ...
        else if (elem->type == ElementType::Descriptor)
        {
            // get the descriptor
            auto resourceTable  = m_typeLibrary->findResourceTable(elem->name);
            if (resourceTable)
            {
                err.reportError(elem->location, TempString("Descriptor '{}' was already defined", elem->name));
                valid = false;
            }
            else
            {
		...
		else if (elem->type == ElementType::StaticSampler)
		{
			// get the descriptor
			auto sampler = m_typeLibrary->findStaticSampler(elem->name);
			if (sampler)
			{
				err.reportError(elem->location, TempString("Static sampler '{}' was already defined", elem->name));
				valid = false;
			}
			else
			{
				...
				{
					ASSERT(child->type == ElementType::KeyValueParam);

					const auto key = StringID(child->name);
		...
		else if (elem->type == ElementType::RenderStates)
		{
			// get the descriptor
			auto sampler = m_typeLibrary->findStaticRenderStates(elem->name);
			if (sampler)
			{
				err.reportError(elem->location, TempString("Static render states '{}' were already defined", elem->name));
				valid = false;
			}
			else
			{
        ...
        else if (elem->type == ElementType::Struct)
        {
            // get the struct
            auto compoundType  = m_typeLibrary->findCompositeType(elem->name);
            if (compoundType)
            {
                err.reportError(elem->location, TempString("Structure '{}' was already defined", elem->name));
                valid = false;
            }
            else
            {
                ...
                if (auto layoutAttr  = elem->findAttribute("packing"))
                {
                    ...
                    {
                        err.reportError(elem->location, TempString("Structure '{}' uses invalid packing mode '{}'", elem->name, layoutAttr->stringData));
                        valid = false;
                    }
                }
    ...
    for (auto elem  : root->children)
    {
        if (elem->type == ElementType::Descriptor)
        {
            auto resourceTable  = m_typeLibrary->findResourceTable(elem->name);
            if (resourceTable)
    ...
    for (auto elem  : root->children)
    {
        if (elem->type == ElementType::Program)
        {
            if (elem->flags.test(ElementFlag::Export))
            {
                ...
                {
                    err.reportError(elem->location, TempString("Exported shader name '{}' must end in PS,VS,GS,DS,HS or CS to specify target pipeline", elem->name));
                }
                else
                {
                    ...
                    {
                        err.reportWarning(elem->location, TempString("Stage '{}' was already exported from shader '{}'", type, existingExport->name()));
                        err.reportWarning(existingExport->location(), TempString("See previous declaration of '{}' here", existingExport->name()));
                    }
                    else
                    {
        ...
        else if (elem->type == ElementType::Variable)
        {
            // variable can't start with "gl_"
            if (elem->name.beginsWith("gl_"))
            {
            ...
            if (existingConst)
            {
                err.reportError(elem->location, TempString("Global variable '{}' was already defined at {}", elem->name, existingConst->loc));
                valid = false;
                continue;
            }
            else if (auto existingFunction  = findGlobalFunction(elem->name))
            {
                err.reportError(elem->location, TempString("Global variable '{}' hides global function defined at {}", elem->name, existingFunction->location()));
                valid = false;
                continue;
            }

            // we should have the const flag
            if (!elem->flags.test(ElementFlag::Const))
            {
                err.reportError(elem->location, TempString("Global variable '{}' must be declared as const", elem->name));
                valid = false;
                continue;
            }
            if (elem->flags.test(ElementFlag::In) || elem->flags.test(ElementFlag::Out))
            {
                err.reportError(elem->location, TempString("Global variable '{}' cannot be declared with any other usage", elem->name));
                valid = false;
                continue;
            }
            ...
            {
                err.reportError(elem->location, TempString("Global constant '{}' uses invalid type (only numerical data is allowed)", elem->name));
                valid = false;
                continue;
            }
            ...
            {
                err.reportError(elem->location, TempString("Global constant '{}' requires initialization", elem->name));
                valid = false;
                continue;
            }
            ...
            if (existingConst != nullptr)
            {
                ...
                {
                    err.reportError(elem->location, TempString("Global constant override '{}' has different type '{}' than previously '{}' at {}", elem->name, dataType, existingConst->dataType, existingConst->loc));
                    valid = false;
                    continue;
                }
            }
        ...
        else if (elem->type == ElementType::Function)
        {
            ...
            {
                err.reportError(elem->location, TempString("Global function '{}' hides global constant defined at {}", elem->name, existingConst->loc));
                valid = false;
                continue;
            }
            else if (auto existingFunction  = findGlobalFunction(elem->name))
            {
                err.reportError(elem->location, TempString("Global function '{}' was already defined at {}", elem->name, existingFunction->location()));
                valid = false;
                continue;
            }
            ...
            if (!resolveType(elem->typeRef, returnType, err))
            {
                err.reportError(elem->location, TempString("Unable to resolve return type for function '{}'", elem->name));
                valid = false;
                continue;
            }
            ...
            {
                err.reportError(elem->location, TempString("Global function '{}' return type '{}' must be declared as \"const\" as it has to be known at the compile time", elem->name, returnType));
                valid = false;
            }*/
...
bool CodeLibrary::parseCode(parser::IErrorReporter& err)
{
    ...
    Array<CodeCompilationEntry> entries;
    {
    ...
    for (auto& entry : entries)
    {
        ...
        auto code  = AnalyzeShaderCode(allocator(), err, entry.tokens, *this, entry.function, entry.program);
        if (!code)
        {
            valid = false;
            continue;
        ...
        // mutate nodes
        InplaceArray<CodeNode*, 32> parentStack;
        ...
        {
            err.reportError(entry.loc, TempString("Unable to compile code for initialization of {}", entry.name));
            valid = false;
        }
    }
...
bool CodeLibrary::loadFile(parser::TokenList& tokens, parser::IErrorReporter& err)
{
    ...
    auto root  = AnalyzeShaderFile(m_allocator, err, tokens);
    if (!root)
        return false;
