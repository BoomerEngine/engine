...
bool CodeNode::MatchNodeType(CodeLibrary& lib, CodeNode*& nodePtr, DataType requiredType, bool userExplicitCast, ITextErrorReporter& err)
{
    ...
    {
        err.reportError(nodePtr->location(), TempString("Undetermined value"));
        return false;
    }
    ...
    {
        err.reportError(nodePtr->location(), TempString("Unable to convert from non-LValue '{}' to '{}'", currentType, requiredType));
        return false;
    }
    ...
    {
        err.reportError(nodePtr->location(), TempString("Unable to convert from '{}' to '{}'", nodePtr->dataType(), requiredType));
        return false;
    }
    ...
    {
        err.reportError(nodePtr->location(), TempString("Casting from '{}' to '{}' requires explicit casting", nodePtr->dataType(), requiredType));
        return false;
    }
...
CodeNode::ResolveResult CodeNode::ResolveIdent(CodeLibrary& lib, const Program* program, const Function* func, CodeNode* contextNode, const StringID name, ITextErrorReporter& err)
{
    ...
    if (name.view().beginsWith("gl_"))
    {
        ...
        if (nullptr == builtin)
        {
            err.reportError(contextNode->location(), TempString("Unrecognized built-in parameter '{}'", name));
    ...
    if (program)
    {
        ...
        {
            InplaceArray< ResolvedDescriptorEntry, 10> entries;
            lib.findGlobalDescriptorEntry(name, entries);
            lib.findGlobalUniformEntry(name, entries);

            if (entries.size() > 1)
            {
                err.reportError(contextNode->location(), TempString("Name '{}' matched multiple elements of descriptors", name));
                for (const auto& entry : entries)
                {
                    if (entry.member)
                        err.reportError(entry.member->location, TempString("Constant buffer entry in descriptor '{}'", entry.table->name()));
                    else
                        err.reportError(entry.entry->m_location, TempString("Resource entry in descriptor '{}'", entry.table->name()));
                }
                return CodeNode::ResolveResult();
            }
...
bool CodeNode::MutateNode(CodeLibrary& lib, const Program* program, const Function* func, CodeNode*& node, Array<CodeNode*>& parentStack, ITextErrorReporter& err)
{
    ...
    switch (op)
    {
        ...
        case OpCode::AccessMember:
        {
            ...
            if (node->m_children.size() == 1)
            {
                if (node->m_children[0]->opCode() == OpCode::ResourceTable)
                {
                    ...
                    InplaceArray<ResolvedDescriptorEntry, 10> resolvedEntries;
                    ...
                    {
                        err.reportError(node->location(), TempString("Descriptor '{}' does not contain '{}'", resourceTable->name(), value.m_name));
                        return false;
                    }
                    else if (resolvedEntries.size() != 1)
                    {
                        err.reportError(node->location(), TempString("Descriptor '{}' contains more than one '{}', name is ambiguous", resourceTable->name(), value.m_name));
                        return false;
                    }
                    ...
                    {
                        err.reportError(node->location(), TempString("Constant buffer '{}' in descriptor '{}' can't be accessed directly, only it members can", resolvedEntry.entry->m_name, resourceTable->name()));
                        return false;
                    }
                    ...
                    if (!paramRef)
                    {
                        err.reportError(node->location(), TempString("Unable to access '{}' in descriptor '{}'", value.m_name, resourceTable->name()));
                        return false;
                    }

					if (paramRef->dataType.isResource())
					{
						const auto key = StringID(TempString("res:{}.{}", resolvedEntry.table->name(), resolvedEntry.entry->m_name));
        ...
        case OpCode::VariableDecl:
        {
            ...
            {
                err.reportError(node->location(), TempString("Unable to declare '{}' without a proper parent scope", value.m_name));
                return false;
            }
            ...
            for (auto declaration  : value.m_parentScopeRef->m_declarations)
            {
                ...
                {
                    err.reportError(node->location(), TempString("Variable '{}' was already declared at {}", value.m_name, declaration->loc));
                    return false;
                }
            }
            ...
            {
                err.reportError(node->location(), TempString("Unable to declare '{}' without a proper type", value.m_name));
                return false;
            }
            ...
            {
                err.reportError(node->location(), TempString("Value '{}' was declared as constant and must be initialized right away", value.m_name));
                return false;
            }
			...
			if (node->m_children.empty() && localVar->dataType.isComposite())
            {
                ...
                if (numMembersWithInitialization && numMembersWithInitialization != compositeType.members().size())
                    err.reportWarning(node->location(), TempString("Not all members of '{}' are initialized", compositeType.name()));
...
bool CodeNode::HandleResourceArrayAccess(CodeLibrary& lib, CodeNode* node, ITextErrorReporter& err)
{
    ...
    if (res.type == DeviceObjectViewType::Image || res.type == DeviceObjectViewType::ImageWritable)
    {
        ...
        {
            err.reportError(node->location(), TempString("Texture has no format defined and cannot be accessed as array"));
            return false;
        }
        ...
        {
            err.reportError(node->location(), TempString("Unable to determine texel type for texture"));
            return false;
        }
		...
		|| res.type == DeviceObjectViewType::BufferStructuredWritable || res.type == DeviceObjectViewType::Buffer)
    {
        ...
        if (!node->m_dataType.valid())
        {
            err.reportError(node->location(), TempString("Unable to determine element type for buffer"));
            return false;
        }
    ...
    err.reportError(node->location(), TempString("This resource can't be accessed like an array"));
    return false;
}
...
bool CodeNode::ResolveTypes_Store(SHADER_RESOLVE_FUNC)
{
	...
	if (node->m_children[0]->m_op == OpCode::ReadSwizzle)
	{
		...
		if (!mask.isMask())
		{
			err.reportError(node->location(), TempString("Swizzle '{}' is not a valid write mask for '{}'", mask, targetType));
			return false;
		}
	...
	{
		err.reportError(node->location(), TempString("Cannot write to '{}', type is not a reference", targetType));
		return false;
	}
...
bool CodeNode::ResolveTypes_Ident(SHADER_RESOLVE_FUNC)
{
	...
	if (!resolvedIdent)
	{
		err.reportError(node->location(), TempString("Unknown reference to '{}'", paramName));
	...
	if (resolvedIdent.m_param)
	{
		...
		if (resolvedIdent.m_param->dataType.isResource())
		{
			...
			// make resource key
			const auto key = StringID(TempString("res:{}.{}", resolvedIdent.m_param->resourceTable->name(), resolvedIdent.m_param->resourceTableEntry->m_name));
			node->m_dataValue = DataValue(key);
		}
		else
		{
			...
			if (resolvedIdent.m_param->scope == DataParameterScope::GroupShared)
				node->m_dataType = node->m_dataType.makeAtomic();
		}
	}
...
bool CodeNode::ResolveTypes_AccessArray(SHADER_RESOLVE_FUNC)
{
	...
	{
		err.reportError(node->location(), TempString("Type '{}' has no operator[] defined", sourceType));
		return false;
	}
...
bool CodeNode::ResolveTypes_AccessMember(SHADER_RESOLVE_FUNC)
{
	...
	// well, no member
	err.reportError(node->location(), TempString("Type '{}' has no member '{}'", sourceType, memberName));
	return false;
}

bool CodeNode::ResolveTypes_AccessMember_Vector(const DataType& sourceType, StringID memberName, SHADER_RESOLVE_FUNC)
{
	// use the swizzle if possible
	ComponentMask mask;
	if (!ComponentMask::Parse(memberName.c_str(), mask))
	{
		err.reportError(node->location(), TempString("Unable to parse swizzle from '{}'", memberName));
		return false;
	}
	...
	{
		err.reportError(node->location(), TempString("Swizzle '{}' is not compatible with type '{}'", memberName, sourceType));
		return false;
	}
...
bool CodeNode::ResolveTypes_AccessMember_Matrix(const DataType& sourceType, StringID memberName, SHADER_RESOLVE_FUNC)
{
	err.reportError(node->location(), TempString("Matrices have no direct members, use the array operator to access COLUMNS"));
	return false;
}

bool CodeNode::ResolveTypes_AccessMember_Struct(const DataType& sourceType, StringID memberName, SHADER_RESOLVE_FUNC)
{
	// find member in the composite type
	auto memberIndex = sourceType.composite().memberIndex(memberName);
	if (memberIndex == -1)
	{
		err.reportError(node->location(), TempString("Structure '{}' has no member '{}'", sourceType, memberName));
		return false;
	}
...
bool CodeNode::ResolveTypes_AccessMember_Program(const DataType& sourceType, StringID memberName, SHADER_RESOLVE_FUNC)
{
	...
	// unknown
	err.reportError(node->location(), TempString("Member '{}' not found in program '{}'", memberName, targetProgram->name()));
	return false;
}

bool CodeNode::ResolveTypes_AccessMember_Scalar(const DataType& sourceType, StringID memberName, SHADER_RESOLVE_FUNC)
{
...
bool CodeNode::ResolveTypes_Call(SHADER_RESOLVE_FUNC)
{
	...
	if (callContextNode->m_op == OpCode::Ident)
	{
		...
		{
			err.reportError(node->location(), TempString("Failed to mutate function '{}'", funcName));
			return false;
		}
	...
	{
		err.reportError(node->location(), TempString("Context of call ({}) is not a function", callContextNode->dataType()));
		return false;
	}
	...
	{	
		...
		{
			err.reportError(node->location(), TempString("Missing argument {} needed by function call to {}", childIndex, functionName));
			return false;
		}

		if (!argumentTypes[i].valid())
		{
			err.reportError(node->location(), TempString("Argument {} of function call to {} takes undefined type", childIndex, functionName));
			return false;
		}
...
bool CodeNode::ResolveTypes_ProgramInstance(SHADER_RESOLVE_FUNC)
{
	...
	for (uint32_t i = 0; i < node->m_children.size(); ++i)
	{
		...
		if (!programParam)
		{
			err.reportError(node->location(), TempString("Parameter '{}' is not declared in program '{}' or any parent programs", programParamName, targetProgram->name()));
			return false;
		}
		...
		{
			err.reportError(node->location(), TempString("Parameter '{}' from program '{}' was not declared as constexpr", programParamName, targetProgram->name()));
			return false;
		}
...
bool CodeNode::ResolveTypes_Return(SHADER_RESOLVE_FUNC)
{
	...
	if (func->returnType().baseType() == BaseType::Void)
	{
		...
		{
			err.reportError(node->location(), TempString("Function '{}' does not return anything", func->name()));
			return false;
		}
	}
	else
...
bool CodeNode::ResolveTypes_CreateVector(SHADER_RESOLVE_FUNC)
{
	...
	for (uint32_t i = 0; i < numArgs; ++i)
	{
		...
		{
			err.reportError(node->location(), TempString("Vector constructor cannot accept '{}' as input type", argType));
			return false;
		}
		...
		if (!numComponents)
		{
			err.reportError(node->location(), TempString("Vector constructor cannot accept type '{}' as argument because the component count cannot be determined", argType));
			return false;
		}
	...
	{
		err.reportError(node->location(), TempString("Too much data in type constructor, expected {} components, {} provided", expectedComponentCount, numComponentsCollected));
		return false;
	}
	...
	{
		err.reportError(node->location(), TempString("Vector constructor expects {} components, {} provided", expectedComponentCount, numComponentsCollected));
		return false;
	}
...
bool CodeNode::ResolveTypes_CreateMatrix(SHADER_RESOLVE_FUNC)
{
	...
	// make sure all arguments are castable
	uint32_t numComponentsCollected = 0;
	InplaceArray<uint32_t, 16> argumentsComponentCounts;
	for (uint32_t i = 0; i < numArgs; ++i)
	{
		auto*& argNode = node->m_children[i];
		const auto& argType = argNode->dataType();
		...
		{
			err.reportError(node->location(), TempString("Matrix constructor cannot accept '{}' as input type", argType));
			return false;
		}
		...
		if (!numComponents)
		{
			err.reportError(node->location(), TempString("Matrix constructor cannot accept type '{}' as argument because the component count cannot be determined", argType));
			return false;
		}
	...
	if (numArgs != 1 && numArgs != maxComponents)
	{
		...
		{
			err.reportError(node->location(), TempString("Matrix constructor requires {} colum vectors", expectedColumnCount));
			return false;
		}
		...
		for (uint32_t i = 0; i < numArgs; ++i)
		{
			...
			{
				err.reportError(node->location(), TempString("Matrix constructor requires {} colum vectors of {} components each", expectedColumnCount, expectedRowCount));
				return false;
			}
		}
	}
	...
	if (numComponentsCollected > maxComponents)
	{
		if (numArgs != 1)
		{
			err.reportError(node->location(), TempString("Matrix constructor of this type can accept up to {} components, {} provided", maxComponents, numComponentsCollected));
			return false;
		}
		else
		{
			...
			{
				err.reportError(node->location(), TempString("Source matrix does not have enough row/columns for this construction"));
				return false;
			}
			else
			{
	...
	else if (numComponentsCollected < maxComponents)
	{
		if (numArgs == 1)
		{
			if (numComponentsCollected != 1)
			{
				...
				{
					err.reportError(node->location(), TempString("Matrix constructor of this type works only with squre matrices"));
					return false;
				}
			}
			else
...
bool CodeNode::ResolveTypes_CreateArray(SHADER_RESOLVE_FUNC)
{
	...
	{
		err.reportError(node->location(), TempString("Array constructor requires {} arguments but {} were provided", expectedElementCount, numArgs));
		return false;
	}
