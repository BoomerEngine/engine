...
const DataParameter* Program::createBuildinParameterReference(StringID name) const
{
	...
	{
		type = m_library->typeLibrary().floatType(4);
		ShaderBuiltIn = shader::ShaderBuiltIn::FragCoord;
	}
	else if (name == "gl_FrontFacing"_id)
	{
		type = m_library->typeLibrary().booleanType();
		ShaderBuiltIn = shader::ShaderBuiltIn::FrontFacing;
	}
    else if (name == "gl_PointCoord"_id)
	{
        type = m_library->typeLibrary().floatType(2);
		ShaderBuiltIn = shader::ShaderBuiltIn::PointCoord;
	}
	else if (name == "gl_SampleID"_id)
	{
		type = m_library->typeLibrary().integerType(1);
		ShaderBuiltIn = shader::ShaderBuiltIn::SampleID;
	}
	else if (name == "gl_SamplePosition"_id)
	{
		type = m_library->typeLibrary().floatType(2);
		ShaderBuiltIn = shader::ShaderBuiltIn::SamplePosition;
	}
	else if (name == "gl_SampleMaskIn"_id)
	{
		ShaderBuiltIn = shader::ShaderBuiltIn::SampleMaskIn;
		type = m_library->typeLibrary().integerType();
	}
	else if (name == "gl_SampleMask"_id)
	{
		ShaderBuiltIn = shader::ShaderBuiltIn::SampleMask;
		type = m_library->typeLibrary().integerType();
		assignable = true;
	}
	else if (name == "gl_Target0"_id)
	{
		ShaderBuiltIn = shader::ShaderBuiltIn::Target0;
		type = m_library->typeLibrary().floatType(4);
		assignable = true;
	}
	else if (name == "gl_Target1"_id)
	{
		ShaderBuiltIn = shader::ShaderBuiltIn::Target1;
		type = m_library->typeLibrary().floatType(4);
		assignable = true;
	}
	else if (name == "gl_Target2"_id)
	{
		ShaderBuiltIn = shader::ShaderBuiltIn::Target2;
		type = m_library->typeLibrary().floatType(4);
		assignable = true;
	}
	else if (name == "gl_Target3"_id)
	{
		ShaderBuiltIn = shader::ShaderBuiltIn::Target3;
		type = m_library->typeLibrary().floatType(4);
		assignable = true;
	}
	else if (name == "gl_Target4"_id)
	{
		ShaderBuiltIn = shader::ShaderBuiltIn::Target4;
		type = m_library->typeLibrary().floatType(4);
		assignable = true;
	}
	else if (name == "gl_Target5"_id)
	{
		ShaderBuiltIn = shader::ShaderBuiltIn::Target5;
		type = m_library->typeLibrary().floatType(4);
		assignable = true;
	}
	else if (name == "gl_Target6"_id)
	{
		ShaderBuiltIn = shader::ShaderBuiltIn::Target6;
		type = m_library->typeLibrary().floatType(4);
		assignable = true;
	}
	else if (name == "gl_Target7"_id)
	{
		ShaderBuiltIn = shader::ShaderBuiltIn::Target7;
		type = m_library->typeLibrary().floatType(4);
		assignable = true;
	}
	else if (name == "gl_FragDepth"_id)
	{
		ShaderBuiltIn = shader::ShaderBuiltIn::Depth;
		type = m_library->typeLibrary().floatType();
		assignable = true;
	}
	else if (name == "gl_NumWorkGroups"_id)
	{
		ShaderBuiltIn = shader::ShaderBuiltIn::NumWorkGroups;
		type = m_library->typeLibrary().unsignedType(3);
	}
	else if (name == "gl_GlobalInvocationID"_id)
	{
		ShaderBuiltIn = shader::ShaderBuiltIn::GlobalInvocationID;
		type = m_library->typeLibrary().unsignedType(3);
	}
	else if (name == "gl_LocalInvocationID"_id)
	{
		ShaderBuiltIn = shader::ShaderBuiltIn::LocalInvocationID;
		type = m_library->typeLibrary().unsignedType(3);
	}
	else if (name == "gl_WorkGroupID"_id)
	{
		ShaderBuiltIn = shader::ShaderBuiltIn::WorkGroupID;
		type = m_library->typeLibrary().unsignedType(3);
	}
	else if (name == "gl_LocalInvocationIndex"_id)
	{
		ShaderBuiltIn = shader::ShaderBuiltIn::LocalInvocationIndex;
    ...
    param->scope = scope;
    param->dataType = type;
	param->builtInVariable = ShaderBuiltIn;
