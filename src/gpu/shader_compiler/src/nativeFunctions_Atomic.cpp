...
public:
    ...
    virtual DataType determineReturnType(TypeLibrary& typeLibrary, uint32_t numArgs, DataType* argTypes, const TextTokenLocation& loc, ITextErrorReporter& err) const override final
    {
        ...
        {
            err.reportError(loc, TempString("Invalid number of function arguments, this function requires {} arguments", m_numArgs));
            return DataType();
        }
        ...
        {
            err.reportError(loc, TempString("Atomic functions require '{}' to be a reference to memory location/texel", argTypes[0]));
            return DataType();
        }
        ...
        {
            err.reportError(loc, TempString("Type '{}' does not support atomic operations", argTypes[0]));
            return DataType();
        }
