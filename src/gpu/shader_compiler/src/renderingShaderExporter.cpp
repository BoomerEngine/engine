...
ConfigProperty<bool> cvDumpShaderStubs("Rendering.Shader", "DumpCompiledStubs", true);
...
static bool ExtractShaderBundle(const ExportedShaders& exports, CodeLibrary& code, ShaderBunderSetup& outBundleSetup, parser::IErrorReporter& err)
{
	bool valid = true;
...
public:
	...
	shader::StubProgram* exportProgram(const ShaderBunderSetup& bundle, StringView contextPath, StringView contextOptions, parser::IErrorReporter& err)
	{
		clear();
		...
		// stages
		bool valid = true;
		InplaceArray<const shader::StubStage*, 10> shaderStages;
...
extern bool AssembleShaderStubs(mem::LinearAllocator& mem, CodeLibrary& lib, AssembledShader& outAssembledShader, StringView contextPath, StringView contextOptions, parser::IErrorReporter& err)
{
	// extract shaders
	...
	if (const auto* program = helper.exportProgram(bundle, contextPath, contextOptions, err))
	{
		...
		// export metadata
		const auto shaderKey = CRC64().append(outAssembledShader.blob.data(), outAssembledShader.blob.size()).crc();
