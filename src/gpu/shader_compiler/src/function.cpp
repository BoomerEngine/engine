...
void DataParameter::print(IFormatStream& ret) const
{
    switch (scope)
    {
...
void ComponentMask::print(IFormatStream& f) const
{
    for (uint32_t i = 0; i < MAX_COMPONENTS; ++i)
    {
        switch (m_bits[i])
...
void ComponentMask::componentsToWrite( InplaceArray<uint8_t, 4>& outComponents) const
{
    for (uint32_t i = 0; i < MAX_COMPONENTS; ++i)
    {
        if (m_bits[i] == ComponentMaskBit::NotDefined)
...
void Function::rename(StringID name)
{
    m_name = name;
}
