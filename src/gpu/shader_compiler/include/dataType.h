...
class GPU_SHADER_COMPILER_API ArrayCounts
{
public:
...
/// resource type
struct GPU_SHADER_COMPILER_API ResourceType
{
    ...
    bool calcTypeHash(CRC64& crc) const;
...
/// shader data type
struct GPU_SHADER_COMPILER_API DataType
{
public:
    DataType();
    ...
    // get custom flags
    INLINE const DirectFlags<TypeFlags>& flags() const
    {
        return m_flags;
    }
    ...
    DataType applyFlags(const DirectFlags<TypeFlags>& flags) const;
...
private:
    ...
    DirectFlags<TypeFlags> m_flags; // internal flags
