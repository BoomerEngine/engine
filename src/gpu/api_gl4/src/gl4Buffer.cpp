...
void Buffer::ensureCreated()
{
	...
	// fetch source atoms
	InplaceArray<ISourceDataProvider::SourceAtom, 1> sourceAtoms;
    if (m_initData)
...
void BufferTypedView::ensureCreated()
{
	...
	if (auto glBuffer = buffer()->object())
	{
		...
		GL_PROTECT(glObjectLabel(GL_BUFFER, m_glBufferView, -1, TempString("TypedView {} @{} of {}", setup().format, setup().offset, buffer()->setup().label).c_str()));
	}
}
