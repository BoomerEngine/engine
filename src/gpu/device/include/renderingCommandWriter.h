...
struct ResourceCurrentStateTrackingRecord : public NoCopy
{
...
struct StashedFrameBuffer
{
    ...
    InplaceArray<Rect, 1> viewports;
    InplaceArray<Vector2, 1> depthRanges;
};

/// helper class for writing the micro operations stream
/// NOTE: thread safe to write using multiple threads once inside a pass
class GPU_DEVICE_API CommandWriter : public NoCopy
{
public:
    CommandWriter(std::nullptr_t); // unattached
    CommandWriter(CommandBuffer* buffer, StringView scopeName = StringView()); // NOTE: buffer is reset, that's the only legal way we can write to it
    CommandWriter(StringView scopeName = StringView()); // NOTE: buffer is reset, that's the only legal way we can write to it
    ~CommandWriter();
    ...
    void opBeingPass(const FrameBuffer& frameBuffer, uint8_t viewportCount = 1, const Rect& drawArea = Rect::EMPTY());
    ...
    void opClearFrameBuffer(const FrameBuffer& frameBuffer, const Rect* area = nullptr);

    /// fill current color render target to single color
    void opClearPassRenderTarget(uint32_t index, const Vector4& color);
    ...
    /// begin a debug block
    void opBeginBlock(StringView name, StringView file="", uint32_t line=0);
    ...
    void opClearRenderTarget(const RenderTargetView* view, const Vector4& values, const Rect* rects = nullptr, uint32_t numRects = 0);

    /// clear buffer with custom value or with zero if no value was provided
    void opClearDepthStencil(const RenderTargetView* view, bool doClearDepth, bool doClearStencil, float clearDepth, uint32_t clearStencil, const Rect* rects = nullptr, uint32_t numRects = 0);

    //---

    /// bind vertex buffer at specified slot
    void opBindVertexBuffer(StringID name, const BufferObject* buffer, uint32_t offset = 0);

    /// unbind vertex buffer from specified slot
    void opUnbindVertexBuffer(StringID name);
    ...
    void opSetViewportRect(uint8_t viewportIndex, const Rect& viewportRect, float depthMin = 0.0f, float depthMax = 1.0f);
    void opSetViewportRect(uint8_t viewportIndex, int x, int y, int w, int h, float depthMin = 0.0f, float depthMax = 1.0f);

    /// set blend constant
    void opSetBlendConstant(const Color& color);
    void opSetBlendConstant(const Vector4& color);
    ...
    void opSetScissorRect(uint8_t viewportIndex, const Rect& scissorRect);
    ...
    void opCopyRenderTarget(const RenderTargetView* source, const RenderTargetView* dest, uint16_t sourceSlice = 0, uint16_t destSlice = 0, bool flipY = false, Rect sourceRect = Rect(), Rect destRect = Rect());
    ...
    void opBindDescriptorEntries(StringID bindingID, const DescriptorEntry* entries, uint32_t count);

    /// bind new descriptor entries specified in a table
    template< typename T >
    INLINE void opBindDescriptor(StringID bindingID, const T& data)
    {
...
private:
    ...
    HashMap<StringID, uint32_t> m_currentVertexBufferRemainingSize;
    HashMap<StringID, BufferObjectPtr> m_currentVertexBuffers;
#endif

    HashMap<StringID, DescriptorID> m_currentParameterBindings;

#ifdef VALIDATE_DESCRIPTOR_BOUND_RESOURCES			
    HashMap<StringID, const DescriptorEntry*> m_currentParameterData;
    HashSet<DeviceObjectViewPtr> m_trackedObjectViews;
#endif
    ...
    mutable HashMap<ObjectID, ResourceCurrentStateTrackingRecord*> m_currentResourceState;
    ...
    void* allocConstants(uint32_t size, const OpUploadConstants*& outCommand);
...
struct CommandWriterBlock : public NoCopy
{
public:
    INLINE CommandWriterBlock(CommandWriter& cmd, StringView name)
    {
        if (name)
        {
            m_writer = &cmd;
