...
struct GPU_DEVICE_API ShaderSelector
{
...
public:
	...
	int get(StringID key) const;

	// set value for a key, setting a value of 0 removes it
	void set(StringID key, int value);
	...
	void defines(HashMap<StringID, StringBuf>& outDefines) const;

	// parse from string
	static bool Parse(StringView text, ShaderSelector& outSelectors);
...
private:
	...
	bool setNoSort(StringID key, int value);
	void sort();
};
