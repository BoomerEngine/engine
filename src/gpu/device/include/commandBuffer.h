...
class GPU_DEVICE_API CommandBuffer : public NoCopy
{
    RTTI_DECLARE_POOL(POOL_GPU_COMMAND_BUFFER)
...
private:
    ...
    InplaceArray<DownloadDataSinkPtr, 16> m_downloadSinks;

    HashMap<StringID, DescriptorID> m_activeParameterBindings;

#ifdef VALIDATE_DESCRIPTOR_BOUND_RESOURCES
    HashMap<StringID, const DescriptorEntry*> m_activeParameterData;
#endif
...
extern GPU_DEVICE_API void ValidateCommandBuffer(CommandBuffer* buffer);
