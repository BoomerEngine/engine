...
static uint32_t CalcUpdateMemorySize(ImageFormat format, const ResourceCopyRange& range)
{
	...
	if (formatInfo.compressed)
	{
		...
		const auto alignedSizeX = Align<uint32_t>(range.image.sizeX, 4);
		const auto alignedSizeY = Align<uint32_t>(range.image.sizeY, 4);
...
CommandWriter::CommandWriter(CommandBuffer* buffer, StringView scopeName /*= StringView()*/)
{
    attachBuffer(buffer);
...
CommandWriter::CommandWriter(StringView scopeName /*= StringView()*/)
{
...
bool CommandWriter::validateFrameBuffer(const FrameBuffer& frameBuffer, uint32_t* outWidth /*= nullptr*/, uint32_t* outHeight /*= nullptr*/)
{
	...
	/*for (uint32_t i = 0; i < FrameBuffer::MAX_COLOR_TARGETS; ++i)
	{
		if (frameBuffer.color[i].viewPtr)
		{
			...
			{
				DEBUG_CHECK_RETURN_EX_V(expectedFormat == format, TempString("Color target {} in frame buffer has layout {} but {} is expected by pass layout", i, format, expectedFormat), false);
			}
			else
			{
				DEBUG_CHECK_RETURN_EX_V(ImageFormat::UNKNOWN == format, TempString("Color target {} in frame buffer is bound with format '{}' but pass layout expects it's not", i, format), false);
			}
		}
		else
		{
			DEBUG_CHECK_RETURN_EX_V(layout->layout().color[i].format == ImageFormat::UNKNOWN, TempString("Color target {} in frame buffer is not specified but it's expected by pass layout", i), false);
		}
	}
	...
	if (frameBuffer.depth.viewPtr)
	{
		...
		if (expectedFormat != ImageFormat::UNKNOWN)
		{
			DEBUG_CHECK_RETURN_EX_V(expectedFormat == format, TempString("Depth target in frame buffer has layout {} but {} is expected by pass layout", format, expectedFormat), false);
		}
		else
		{
			DEBUG_CHECK_RETURN_EX_V(ImageFormat::UNKNOWN == format, TempString("Deth target in frame buffer is bound with format '{}' but pass layout expects it's not", format), false);
		}
	}
	else
	{
	...
	DEBUG_CHECK_RETURN_EX_V(frameBuffer.samples() == layout->layout().samples, TempString("Frame buffer has {} samples per pixel while layout expects {}", frameBuffer.samples(), layout->layout().samples), false);*/
	...
	for (uint32_t i = 0; i < FrameBuffer::MAX_COLOR_TARGETS; ++i)
	{
		if (frameBuffer.color[i].viewPtr)
		{
			...
			if (auto* owningImage = rtti_cast<ImageObject>(frameBuffer.color[i].viewPtr->object()))
	...
	{
		// NOTE: render target may NOT be owned by image but then the layout does not change much
		if (auto* owningImage = rtti_cast<ImageObject>(frameBuffer.depth.viewPtr->object()))
...
void CommandWriter::opBeingPass(const FrameBuffer& frameBuffer, uint8_t viewportCount, const Rect& drawArea)
{
	...
	if (!drawArea.empty())
		...
		op->renderArea = Rect(0, 0, width, height);
...
void CommandWriter::opClearFrameBuffer(const FrameBuffer& frameBuffer, const Rect* area/* = nullptr*/)
{
	...
	if (area)
		...
		op->customArea = Rect(0, 0, width, height);
}

//--

void CommandWriter::opBeginBlock(StringView name, StringView file, uint32_t line)
{
...
void CommandWriter::opSetViewportRect(uint8_t viewport, const Rect& viewportRect, float depthMin, float depthMax)
{
...
{
    opSetViewportRect(viewport, Rect(x, y, x + w, y + h), depthMin, depthMax);
}

void CommandWriter::opSetBlendConstant(const Color& color)
{
	opSetBlendConstant(color.toVectorLinear());
}

void CommandWriter::opSetBlendConstant(const Vector4& color)
{
	DEBUG_CHECK_RETURN(m_currentPass != nullptr);
...
void CommandWriter::opSetScissorRect(uint8_t viewportIndex, const Rect& scissorRect)
{
    DEBUG_CHECK_RETURN(m_currentPass != nullptr);
...
{
    opSetScissorRect(viewportIndex, Rect(x, y, x + w, y + h));
}

void CommandWriter::opSetScissorBounds(uint8_t viewportIndex, int x0, int y0, int x1, int y1)
{
    opSetScissorRect(viewportIndex, Rect(x0, y0, x1, y1));
}
...
void CommandWriter::opClearPassRenderTarget(uint32_t index, const Vector4& color)
{
    DEBUG_CHECK_RETURN(m_currentPass != nullptr);
...
void CommandWriter::opClearRenderTarget(const RenderTargetView * view, const Vector4 & values, const Rect * rects /*= nullptr*/, uint32_t numRects /*= 0*/)
{
	...
	const auto payloadDataSize = sizeof(Rect) * numRects;
...
void CommandWriter::opClearDepthStencil(const RenderTargetView* view, bool doClearDepth, bool doClearStencil, float clearDepth, uint32_t clearStencil, const Rect* rects /*= nullptr*/, uint32_t numRects /*= 0*/)
{
	DEBUG_CHECK_RETURN_EX(view, "Invalid render target view");
...
void CommandWriter::opCopyRenderTarget(const RenderTargetView* source, const RenderTargetView* dest, uint16_t sourceSlice /*= 0*/, uint16_t destSlice /*= 0*/, bool flipY /*= false*/, Rect sourceRect /*= Rect()*/, Rect destRect /*= Rect()*/)
{
...
void CommandWriter::opBindVertexBuffer(StringID bindpoint, const BufferObject* buffer, uint32_t offset /*= 0*/)
{
...
void CommandWriter::opUnbindVertexBuffer(StringID bindpoint)
{
    DEBUG_CHECK_RETURN(bindpoint);// , "Invalid bind point");
...
bool CommandWriter::validateDrawVertexLayout(const ShaderMetadata* meta, uint32_t requiredVertexCount, uint32_t requiredInstanceCount)
{
    ...
    for (const auto& stream : meta->vertexStreams)
    {
		...
		DEBUG_CHECK_RETURN_EX_V(found, TempString("Missing vertex buffer '{}' for current draw call", stream.name), false);
                    
		if (stream.instanced)
		{
			auto sizeNeeded = stream.size * requiredInstanceCount;
			DEBUG_CHECK_RETURN_EX_V(dataSizeAtVertexBinding >= sizeNeeded, TempString("Instance vertex buffer '{}' has not enough data for current draw call {} < {}", stream.name, dataSizeAtVertexBinding, sizeNeeded), false);
		}
		else
		{
			auto sizeNeeded = stream.size * requiredVertexCount;
			DEBUG_CHECK_RETURN_EX_V(dataSizeAtVertexBinding >= sizeNeeded, TempString("Vertex buffer '{}' has not enough data for current draw call {} < {}", stream.name, dataSizeAtVertexBinding, sizeNeeded), false);
		}
...
bool CommandWriter::validateParameterBindings(const ShaderMetadata* meta)
{
	...
	for (const auto& desc : meta->descriptors)
    {
		...
		DEBUG_CHECK_RETURN_EX_V(currentLayout != nullptr, TempString("Selected shader requires descriptor '{}' that is not bound", desc.name), false);

        DEBUG_CHECK_RETURN_EX_V(currentLayout->layout().size() == desc.elements.size(), TempString("Bound parameter layout '{}' requires {} elements but {} provided",
		...
		for (auto i : desc.elements.indexRange())
		{
			...
			const auto& boundElemViewType = currentLayout->layout()[i];
 			...
 			DEBUG_CHECK_RETURN_EX_V(expectedElem.type == boundElemViewType, TempString("Element '{}' in descriptor '{}' is expected to be {} but in layout it's {}",
        ...
        if (!descriptorEntries)
            printActiveDescriptors();
		DEBUG_CHECK_RETURN_EX_V(descriptorEntries != nullptr, TempString("No actual descript data for descriptor '{}' found", desc.name), false);
		...
		for (auto i : desc.elements.indexRange())
		{
			...
			DEBUG_CHECK_RETURN_EX_V(expectedElem.type == descriptorEntry.type, TempString("Element '{}' in descriptor '{}' is expected to be {} bound in data it's {}",
			...
			switch (expectedElem.type)
			{
				case DeviceObjectViewType::ConstantBuffer:
				{
					if (descriptorEntry.inlinedConstants.sourceDataPtr) // way more common to have inlined constants
					{
						...
						DEBUG_CHECK_RETURN_EX_V(descriptorEntry.size >= expectedElem.number, TempString("Constant buffer '{}' in descriptor '{}' is expected to be of size {} but {} bytes were given.",
							expectedElem.name, desc.name, expectedElem.number, descriptorEntry.size), false);
					}
					else
					{
						const auto* view = rtti_cast<BufferConstantView>(descriptorEntry.viewPtr);
						...
						DEBUG_CHECK_RETURN_EX_V(view->size() >= expectedElem.number, TempString("Constant buffer '{}' in descriptor '{}' is expected to be of size {} but {} bytes are in the bound view.",
				...
				case DeviceObjectViewType::Buffer:
				{
					...
					const auto* view = rtti_cast<BufferView>(descriptorEntry.viewPtr);
					DEBUG_CHECK_RETURN_V(view != nullptr, false);

					DEBUG_CHECK_RETURN_EX_V(expectedElem.format == view->format(), TempString("Buffer view '{}' in descriptor '{}' is expected to be of format '{}' but '{}' is bound.",
				...
				case DeviceObjectViewType::BufferWritable:
				{
					...
					const auto* view = rtti_cast<BufferWritableView>(descriptorEntry.viewPtr);
					DEBUG_CHECK_RETURN_V(view != nullptr, false);

					DEBUG_CHECK_RETURN_EX_V(expectedElem.format == view->format(), TempString("Writable buffer view '{}' in descriptor '{}' is expected to be of format '{}' but '{}' is bound.",
						expectedElem.name, desc.name, expectedElem.format, view->format()), false);
				...
				case DeviceObjectViewType::BufferStructured:
				{
					...
					const auto* view = rtti_cast<BufferStructuredView>(descriptorEntry.viewPtr);
					DEBUG_CHECK_RETURN_V(view != nullptr, false);

					DEBUG_CHECK_RETURN_EX_V(expectedElem.number == view->stride(), TempString("Structured buffer view '{}' in descriptor '{}' is expected to have stride {} but buffer with stride {} is bound.",
				...
				case DeviceObjectViewType::BufferStructuredWritable:
				{
					...
					const auto* view = rtti_cast<BufferWritableStructuredView>(descriptorEntry.viewPtr);
					DEBUG_CHECK_RETURN_V(view != nullptr, false);

					DEBUG_CHECK_RETURN_EX_V(expectedElem.number == view->stride(), TempString("Writable structured buffer view '{}' in descriptor '{}' is expected to have stride {} but buffer with stride {} is bound.",
						expectedElem.name, desc.name, expectedElem.number, view->stride()), false);
				...
				case DeviceObjectViewType::Sampler:
				{
					...
					const auto* sampler = rtti_cast<SamplerObject>(descriptorEntry.objectPtr);
				...
				case DeviceObjectViewType::Image:
				{
					...
					const auto* view = rtti_cast<ImageReadOnlyView>(descriptorEntry.viewPtr);
					DEBUG_CHECK_RETURN_V(view != nullptr, false);

					DEBUG_CHECK_RETURN_EX_V(expectedElem.format == view->image()->format(), TempString("Writable image view '{}' in descriptor '{}' is expected to be of format '{}' but '{}' is bound.",
				...
				case DeviceObjectViewType::ImageWritable:
				{
					...
					const auto* view = rtti_cast<ImageWritableView>(descriptorEntry.viewPtr);
					DEBUG_CHECK_RETURN_V(view != nullptr, false);

					DEBUG_CHECK_RETURN_EX_V(expectedElem.format == view->image()->format(), TempString("Writable image view '{}' in descriptor '{}' is expected to be of format '{}' but '{}' is bound.",
						expectedElem.name, desc.name, expectedElem.format, view->image()->format()), false);

					DEBUG_CHECK_RETURN_EX_V(expectedElem.viewType == view->image()->type(), TempString("Writable image view '{}' in descriptor '{}' is expected to be '{}' but '{}' is bound.",
						expectedElem.name, desc.name, expectedElem.type, view->image()->type()), false);
...
void CommandWriter::opDispatchThreads(const ComputePipelineObject* co, uint32_t threadCountX /*= 1*/, uint32_t threadCountY /*= 1*/, uint32_t threadCountZ /*= 1*/)
{
	...
	if (validateParameterBindings(co->shaders()->metadata()))
	{
		auto op = allocCommand<OpDispatch>();
		op->pipelineObject = co->id();
		op->counts[0] = Align<uint32_t>(threadCountX, co->groupSizeX()) / co->groupSizeX();
		op->counts[1] = Align<uint32_t>(threadCountY, co->groupSizeY()) / co->groupSizeY();
		op->counts[2] = Align<uint32_t>(threadCountZ, co->groupSizeZ()) / co->groupSizeZ();
	}
}
...
{
	if (const auto* buffer = rtti_cast<BufferObject>(obj))
	{
	...
	else if (const auto* image = rtti_cast<ImageObject>(obj))
	{
		switch (incomingLayout)
		{
		case ResourceLayout::Common:
...
{
	if (const auto* imageObject = rtti_cast<ImageObject>(obj))
	{
	...
	else if (const auto* bufferObject = rtti_cast<BufferObject>(obj))
	{
		auto* entry = (ResourceCurrentStateTrackingRecord*)allocMemory(sizeof(ResourceCurrentStateTrackingRecord));
		memzero(entry, sizeof(ResourceCurrentStateTrackingRecord));
		entry->numSubResources = 1;
	...
	else if (const auto* outputObject = rtti_cast<IOutputObject>(obj))
	{
        auto* entry = (ResourceCurrentStateTrackingRecord*)allocMemory(sizeof(ResourceCurrentStateTrackingRecord));
        memzero(entry, sizeof(ResourceCurrentStateTrackingRecord));
        entry->numSubResources = 1;
	...
	if (entry->tracksSubResources && subImageRegion)
	{
		...
		for (uint32_t i = 0; i < subImageRegion->numSlices; ++i)
		{
			...
			for (uint32_t j = 0; j < subImageRegion->numMips; ++j, ++localEntryIndex)
			{
				...
				{
					DEBUG_CHECK_RETURN_EX_V(layoutEntry.currentLayout == layout, TempString("Sub-Resource {},{} of {} should be in layout {} but is in {}",
...
#ifdef VALIDATE_RESOURCE_LAYOUTS
	DEBUG_CHECK_RETURN_EX(LayoutCompatible(obj, incomingLayout), TempString("Object '{}' has incompatible source layout {}", *obj, incomingLayout));
	DEBUG_CHECK_RETURN_EX(LayoutCompatible(obj, outgoingLayout), TempString("Object '{}' has incompatible target layout {}", *obj, outgoingLayout));
...
void* CommandWriter::opUpdateDynamicPtr(const IDeviceObject* dynamicObject, const ResourceCopyRange& range)
{
	...
	if (const auto* dynamicBuffer = rtti_cast<BufferObject>(dynamicObject))
	{
	...
	else if (const auto* dynamicImage = rtti_cast<ImageObject>(dynamicObject))
	{ 
...
{
	if (const auto* srcImage = rtti_cast<ImageObject>(src))
	{
	...
	else if (const auto* srcBuffer = rtti_cast<BufferObject>(src))
	{
...
void CommandWriter::opDownloadData(const IDeviceObject* obj, const ResourceCopyRange& range, IDownloadDataSink* sink)
{
	...
	if (const auto* image = rtti_cast<ImageObject>(obj))
	{
	...
	else if (const auto* buffer = rtti_cast<BufferObject>(obj))
	{
...
void CommandWriter::opBindDescriptorEntries(StringID binding, const DescriptorEntry* entries, uint32_t count)
{
...
void* CommandWriter::allocConstants(uint32_t size, const OpUploadConstants*& outOffsetPtr)
{
    ASSERT_EX(size > 0, "Recording contants with zero size is not a good idea");

    // allocate space
    //auto uploadSpaceOffset = Align<uint32_t>(m_writeBuffer->m_gatheredState.totalConstantsUploadSize, 256);
    //m_writeBuffer->m_gatheredState.totalConstantsUploadSize = uploadSpaceOffset + Align<uint32_t>(size, 16);

    // align size to the vector size
    auto alignedSize = Align<uint32_t>(size, 16);
