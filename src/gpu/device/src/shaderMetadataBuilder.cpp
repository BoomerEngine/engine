...
static bool HasAttribute(const StubPseudoArray<shader::StubAttribute>& attributes, StringID name)
{
...
template< typename T >
static bool FindAttributeVal(const StubPseudoArray<shader::StubAttribute>& attributes, StringID name, T& outVal)
{
	for (const auto* attr : attributes)
		if (attr->name == name)
			return attr->value.match(outVal) == MatchResult::OK;
...
{
	auto ret = RefNew<ShaderMetadata>();
	...
	for (auto* stage : program->stages)
	{
		...
		for (const auto* descMember : stage->descriptorMembers)
		{
			...
			StringID samplerName, staticSamplerName;
