...
#include "managedBufferWithAllocator.h"
...
void ManagedBufferWithAllocator::update(CommandWriter& cmd)
{
    // record data uploads
    {
...
bool ManagedBufferWithAllocator::allocateBlock(uint32_t dataSize, ManagedBufferBlock& outBlock)
{
    ...
    {
        auto lock = CreateLock(m_bufferAllocatorLock);
        if (BlockAllocationResult::OK != m_bufferAllocator.allocateBlock(dataSize, m_alignment, allocatedBlock))
            return false;
    }

    // return block
    outBlock.block = allocatedBlock;
    outBlock.dataOffset = BlockPool::GetBlockOffset(allocatedBlock);
    outBlock.dataSize = dataSize;
    return true;
}

bool ManagedBufferWithAllocator::allocateBlock(const Buffer& data, ManagedBufferBlock& outBlock)
{
    MemoryBlock allocatedBlock = nullptr;
    ...
    {
        auto lock = CreateLock(m_bufferAllocatorLock);
        if (BlockAllocationResult::OK != m_bufferAllocator.allocateBlock(data.size(), m_alignment, allocatedBlock))
            return false;
    }

    // create the upload request
    {
        auto lcok = CreateLock(m_pendingUploadsLock);
        auto upload = new PendingUpload;
        upload->block = allocatedBlock;
        upload->offset = BlockPool::GetBlockOffset(allocatedBlock);;
