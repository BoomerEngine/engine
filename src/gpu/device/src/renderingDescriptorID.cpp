...
namespace helper
{
	...
	private:
		...
		HashMap<uint64_t, Entry> m_localMap;
	};
    ...
    class LayoutRegistry : public ISingleton
    {
    ...
    public:
        ...
        StringBuf entryName(uint16_t id) const
        {
            // take the lock
            auto lock = CreateLock(m_lock);
    ...
    private:
        ...
        {
            RTTI_DECLARE_POOL(POOL_GPU_PARAM_LAYOUT)
        ...
        HashMap<uint64_t, Entry*> m_entriesMap;
...
void DescriptorID::print(IFormatStream& f) const
{
...
DescriptorID DescriptorID::FromTypes(const DeviceObjectViewType* types, uint32_t numTypes, const DescriptorInfo** outInfoPtr /*= nullptr*/)
{
    ...
    info.m_hash = CRC64().append(info.m_entries.data(), info.m_entries.dataSize()).crc();
...
DescriptorID DescriptorID::Register(StringView layoutDesc, const DescriptorInfo** outInfoPtr /*= nullptr*/)
{
    DEBUG_CHECK_RETURN_V(layoutDesc, DescriptorID());

    InplaceArray<DeviceObjectViewType, 32> viewTypes;
    StringParser parser(layoutDesc);
