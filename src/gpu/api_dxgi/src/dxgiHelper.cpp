...
void DXGIHelper::enumMonitorAreas(Array<Rect>& outMonitorAreas) const
{
	outMonitorAreas = m_cachedMonitors;
}

void DXGIHelper::enumDisplays(Array<DisplayInfo>& outDisplayInfos) const
{
...
void DXGIHelper::enumResolutions(uint32_t displayIndex, Array<ResolutionInfo>& outResolutions) const
{
...
void DXGIHelper::enumerateAdapters()
{
	...
	for (;;)
	{
		...
		DEBUG_CHECK_EX(hRet == DXGI_ERROR_NOT_FOUND || hRet == S_OK, TempString("Unexpected result from EnumAdapters: {} {}", hRet, TranslateDXGIError(hRet)));
