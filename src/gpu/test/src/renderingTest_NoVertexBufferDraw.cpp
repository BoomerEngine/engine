...
#include "../../device/include/buffer.h"
...
static void PrepareTestGeometry(float x, float y, float w, float h, int test, Vector4* outVertices, Vector4* outColors)
{
    ...
    for (uint32_t py = 0; py < TRIANGLES_PER_SIDE; ++py)
    {
        ...
        for (uint32_t px = 0; px < TRIANGLES_PER_SIDE; ++px, vertexWriteIndex += 6, colorWriteIndex += 2)
        {
            ...
            auto d1 = Vector2(fx + 0.5f, fy + 0.5f).length();
            auto s1 = 0.5f + 0.5f * cos(d1 * TWOPI * 2.0f);
            auto d2 = Vector2(fx + 3.5f, fy - 3.5f).length();
            ...
            outVertices[vertexWriteIndex + 0] = Vector4(x0, y0, 0.5f, 1.0f);
            outVertices[vertexWriteIndex + 1] = Vector4(x0 + dx, y0, 0.5f, 1.0f);
            outVertices[vertexWriteIndex + 2] = Vector4(x0 + dx, y0 + dy, 0.5f, 1.0f);

            outVertices[vertexWriteIndex + 3] = Vector4(x0, y0, 0.5f, 1.0f);
            outVertices[vertexWriteIndex + 4] = Vector4(x0 + dx, y0 + dy, 0.5f, 1.0f);
            outVertices[vertexWriteIndex + 5] = Vector4(x0, y0 + dy, 0.5f, 1.0f);
...
{
    Vector4 positionBuffer[MAX_VERTICES];
    Vector4 colorBuffer[MAX_TRIANGLES];
...
void RenderingTest_NoVertexBufferDraw::render(CommandWriter& cmd, float time, const RenderTargetView* backBufferView, const RenderTargetView* backBufferDepthView )
{
    FrameBuffer fb;
