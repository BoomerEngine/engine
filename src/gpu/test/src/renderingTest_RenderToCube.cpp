...
public:
	...
	virtual void queryInitialCamera(Vector3& outPosition, Angles& outRotation) override
	{
		outRotation = Angles(40.0f, 0.0f, 0);
		outPosition = Vector3(0.8f, 0.8f, 0.0f) - outRotation.forward() * 1.2f;
	}
...
private:
	...
	Vector3 m_reflectionCenter;
	Vector3 m_initialReflectionCenter;
...
{
	m_initialReflectionCenter = Vector3(0.8f, 0.8f, 0.3f);
...
{
    Matrix WorldToShadowmap;
};

void RenderingTest_RenderToCube::render(CommandWriter& cmd, float time, const RenderTargetView* backBufferView, const RenderTargetView* backBufferDepth)
{
	// rotate the teapots
	for (uint32_t i=1; i< m_scene->m_objects.size(); ++i)
	{
		...
		teapot.m_params.LocalToWorld = Matrix::BuildRotation(Angles(0.0f, rot, 0.0f));
    ...
    m_scene->m_lightPosition = Angles(-40.0f, angle, 0.0f).forward();

    // render shadowmap
    {
        SceneCamera camera;
		camera.setupDirectionalShadowmap(Vector3::ZERO(), m_scene->m_lightPosition);
		camera.calcMatrices();
	...
	static const Vector4 CLEAR_COLORS[6] = {
