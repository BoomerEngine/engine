...
static void PrepareTestGeometry(float x, float y, float w, float h, Array<Simple3DVertex>& outVertices)
{
    ...
    for (uint32_t i = 0; i < 50; ++i)
    {
        ...
        auto color = Color::FromVectorLinear(Vector4(RandRange(0.2f, 1.0f), RandRange(0.2f, 1.0f), RandRange(0.2f, 1.0f), 1.0f));
...
void RenderingTest_MultiSampleBasic::initialize()
{
    ...
    if (m_showSamples)
        m_shaderPreview = loadGraphicsShader(TempString("MultiSampleTexturePreview{}.csl", subTestIndex() % 5));
    else
...
void RenderingTest_MultiSampleBasic::render(CommandWriter& cmd, float frameIndex, const RenderTargetView* backBufferView, const RenderTargetView* depth)
{
    ...
    FrameBuffer fb;
    fb.color[0].view(m_colorBufferRTV).clear(Vector4(0.0f, 0.0f, 0.2f, 1.0f));
