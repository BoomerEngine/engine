...
static void AddQuad(Simple3DVertex* vertices, float x0, float y0, float x1, float y1, Color color = Color::WHITE)
{
...
void RenderingTest_StencilModes::initialize()
{
    ...
    // allocate fill pattern to resolve the stencil value
    {
        ...
        colors[0] = Vector4(0, 0, 0, 0);
        colors[1] = Vector4(1, 0, 0, 0);
        colors[2] = Vector4(0, 1, 0, 0);
        colors[3] = Vector4(1, 1, 0, 0);
        colors[4] = Vector4(0, 0, 1, 0);
        colors[5] = Vector4(1, 0, 1, 0);
        colors[6] = Vector4(0, 1, 1, 0);
        colors[7] = Vector4(1, 1, 1, 0);

        colors[255] = Vector4(0.5f, 0, 0, 0);
        colors[254] = Vector4(0, 0.5f, 0, 0);
        colors[253] = Vector4(0.5f, 0.5f, 0, 0);
        colors[252] = Vector4(0, 0, 0.5f, 0);
        colors[251] = Vector4(0.5f, 0, 0.5f, 0);
        colors[250] = Vector4(0, 0.5f, 0.5f, 0);
        colors[249] = Vector4(0.5f, 0.5f, 0.5f, 0);

        Simple3DVertex vertices[256 * 6];

        for (uint32_t i = 0; i < 256; ++i)
            AddQuad(vertices + 6 * i, -1, -1, 1, 1, Color::FromVectorLinear(colors[i]));
...
void RenderingTest_StencilModes::render(CommandWriter& cmd, float time, const RenderTargetView* backBufferView, const RenderTargetView* backBufferDepthView )
{
    FrameBuffer fb;
