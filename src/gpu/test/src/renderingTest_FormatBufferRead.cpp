...
void PrepareTestGeometry(float x, float y, float w, float h, uint32_t count, Array<Simple3DVertex>& outVertices, Array<Vector4>& outBufferData)
{
    for (uint32_t py = 0; py <= count; ++py)
    {
        ...
        for (uint32_t px = 0; px <= count; ++px)
        {
            ...
            auto d = Vector2(fx - 0.5f, fy - 0.5f).length();
            ...
            outBufferData.pushBack(Vector4(0, fy, fx, s));
        }
    }
}
...
void RenderingTest_FormatBufferRead::render(CommandWriter& cmd, float time, const RenderTargetView* backBufferView, const RenderTargetView* depth)
{
	FrameBuffer fb;
