...
static void PrepareTestTexels(uint32_t sideCount, Array<Color>& outColors)
{
	...
	for (uint32_t y = 0; y < sideCount; y++)
	{
		...
		for (uint32_t x = 0; x < sideCount; x++)
		{
			...
			outColors.pushBack(Color::FromVectorLinear(Vector4(0.0f, f, 1.0f - f, 1.0f)));
		}
	}
}

static void PrepareTestTexels2(uint32_t sideCount, Array<Color>& outColors)
{
	outColors.reserve(sideCount * sideCount);
	for (uint32_t y = 0; y < sideCount; y++)
	{
		...
		for (uint32_t x = 0; x < sideCount; x++)
		{
			...
			outColors.pushBack(Color::FromVectorLinear(Vector4(1.0f - f, 0, f, 1.0f)));
		}
	}
}
...
void RenderingTest_BufferCopyRect::render(CommandWriter& cmd, float time, const RenderTargetView* backBufferView, const RenderTargetView* backBufferViewDepth)
{
	// copy part
	{
