...
private:
    ...
    void computeHistogram(CommandWriter& cmd, uint32_t componentType, const ImageReadOnlyView* view, const BufferWritableView* outHistogram);
	void normalizeHistograms(CommandWriter& cmd, BufferWritableView* histograms[4]);
    void drawHistogram(CommandWriter& cmd, const BufferView* histogram, Color lineColor);
};
...
void RenderingTest_ComputeHistogram::computeHistogram(CommandWriter& cmd, uint32_t componentType, const ImageReadOnlyView* view, const BufferWritableView* outHistogram)
{
...
void RenderingTest_ComputeHistogram::normalizeHistograms(CommandWriter& cmd, BufferWritableView* histograms[4])
{
	struct
	{
		uint32_t numberOfBuckets = 0;
...
void RenderingTest_ComputeHistogram::drawHistogram(CommandWriter& cmd, const BufferView* histogram, Color lineColor)
{
	struct
	{
		uint32_t numberOfBuckets = 0;
...
void RenderingTest_ComputeHistogram::render(CommandWriter& cmd, float frameIndex, const RenderTargetView* backBufferView, const RenderTargetView* depth)
{
	// clear buffers
    ...
    // draw
    {
		...
		// draw the histograms
		drawHistogram(cmd, m_redHistogramBufferSRV, Color::RED);
		drawHistogram(cmd, m_greenHistogramBufferSRV, Color::GREEN);
		drawHistogram(cmd, m_blueHistogramBufferSRV, Color::BLUE);
