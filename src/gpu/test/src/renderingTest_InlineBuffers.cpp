...
static void GenerateTempGeometry(float x, float y, float w, float h, float time, Array<Simple3DVertex>& outVertices, Array<uint16_t>& outIndices)
{
    ...
    for (uint32_t py = 0; py <= GRID_SIZE; ++py)
    {
        ...
        for (uint32_t px = 0; px <= GRID_SIZE; ++px)
        {
            ...
            float d = Vector2(fx - 0.5f, fy - 0.5f).length();
            float ro = d * 2.0f;
            float rs = d * 10.0f;
            ...
            v.VertexColor = Color(cv, 255-cv, 0, 255);
...
void RenderingTest_InlineBuffers::render(CommandWriter& cmd, float time, const RenderTargetView* backBufferView, const RenderTargetView* backBufferDepthView )
{
    Array<Simple3DVertex> tempVertices;
    ...
    BufferObjectPtr vertexBuffer;
    {
		...
		auto source = RefNew<SourceDataProviderBuffer>(tempVertices.createBuffer());
    ...
    BufferObjectPtr indexBuffer;
    {
		...
		auto source = RefNew<SourceDataProviderBuffer>(tempIndices.createBuffer());
