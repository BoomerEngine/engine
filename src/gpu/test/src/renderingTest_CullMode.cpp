...
void RenderingTest_CullMode::initialize()
{
    // create vertex buffer with a single triangle
    {
        ...
        float x = -0.85f;
        float y = -0.5f;
        AddQuad(x, y, 0.2f, 0.8f, true, Color::RED, writePtr); x += 0.2f;
        AddQuad(x, y, 0.2f, 0.8f, false, Color::GREEN, writePtr); x += 0.3f;
        AddQuad(x, y, 0.2f, 0.8f, true, Color::RED, writePtr); x += 0.2f;
        AddQuad(x, y, 0.2f, 0.8f, false, Color::GREEN, writePtr); x += 0.3f;
        AddQuad(x, y, 0.2f, 0.8f, true, Color::RED, writePtr); x += 0.2f;
        AddQuad(x, y, 0.2f, 0.8f, false, Color::GREEN, writePtr); x += 0.3f;
...
void RenderingTest_CullMode::render(CommandWriter& cmd, float time, const RenderTargetView* backBufferView, const RenderTargetView* backBufferDepthView )
{
    FrameBuffer fb;
