...
private:
	...
	void recordRowRange(CommandWriter& cmd, uint32_t firstRow, uint32_t lastRow);
...
void RenderingTest_InlineParameters::initialize()
{
    ...
    // create vertex buffer with a single triangle
    {
        ...
        vertices[0].set(0, -1, 0.5f, 0, 0, Color::WHITE);
        vertices[1].set(-1, 1, 0.5f, 0, 0, Color::WHITE);
        vertices[2].set(1, 1, 0.5f, 0, 0, Color::WHITE);
        m_vertexBuffer = createVertexBuffer(sizeof(vertices), vertices);
    }
}

void RenderingTest_InlineParameters::recordRowRange(CommandWriter& cmd, uint32_t firstRow, uint32_t lastRow)
{
	//--

	struct InlineParameterConsts
	{
		Vector2 TestOffset = Vector2(0, 0);
		Vector2 TestScale = Vector2(1, 1);
	};
...
void RenderingTest_InlineParameters::render(CommandWriter& cmd, float time, const RenderTargetView* backBufferView, const RenderTargetView* backBufferDepthView )
{
	//--
