...
static void PrepareTestGeometry(float x, float y, float r, float phase, Vector2* outVertices)
{
    auto NUM_SEGS = 3;
    ...
    for (uint32_t i=0; i<NUM_SEGS; i++)
    {
        ...
        outVertices[i*2+0] = Vector2(ax,ay);
        outVertices[i*2+1] = Vector2(bx,by);
    }
}

void RenderingTest_MultipleBufferUpdates::initialize()
{
    m_vertexBuffer = createVertexBuffer(sizeof(Vector2) * 6, nullptr);
    m_vertexColorBuffer = createVertexBuffer(sizeof(Color) * 6, nullptr);
...
void RenderingTest_MultipleBufferUpdates::render(CommandWriter& cmd, float time, const RenderTargetView* backBufferView, const RenderTargetView* backBufferDepthView)
{
    auto GRID_SIZE = 16;
    ...
    for (uint32_t y = 0; y < GRID_SIZE; ++y)
    {
        for (uint32_t x = 0; x < GRID_SIZE; ++x)
        {
            ...
            // prepare positions
			{
                ...
                auto* updateVertices = cmd.opUpdateDynamicBufferPtrN<Vector2>(m_vertexBuffer, 0, 6);
            ...
            // prepare colors
            {
                ...
                const auto randomColor = Color(rand.next(), rand.next(), rand.next());
                auto* updateColors = cmd.opUpdateDynamicBufferPtrN<Color>(m_vertexColorBuffer, 0, 6);
