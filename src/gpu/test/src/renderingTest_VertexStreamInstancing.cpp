...
static void PrepareInstanceBuffer(float x, float y, float w, float h, uint32_t count, Array<InstanceDataTest>& outInstances)
{
    for (uint32_t py = 0; py < count; ++py)
    {
        for (uint32_t px = 0; px < count; ++px)
        {
            ...
            t.InstanceColor = Color::FromVectorLinear(Vector4(px / (float)(count - 1), py / (float)(count - 1), 0, 1));
...
void RenderingTest_VertexStreamInstancing::initialize()
{
    ...
    {
        Array<InstanceDataTest> instances;
...
void RenderingTest_VertexStreamInstancing::render(CommandWriter& cmd, float time, const RenderTargetView* backBufferView, const RenderTargetView* depth)
{
    FrameBuffer fb;
