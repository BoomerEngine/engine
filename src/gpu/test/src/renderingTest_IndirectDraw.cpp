...
static void PrepareTestGeometry(uint32_t count, Array<Simple3DVertex>& outVertices)
{
    for (uint32_t py = 0; py < count; ++py)
    {
		...
		v[0].set(x0, y0, 0.5f, 0, 0, Color::WHITE);
		v[1].set(x1, y0, 0.5f, 0, 0, Color::WHITE);
		v[2].set(x1, y1, 0.5f, 0, 0, Color::WHITE);
		v[3].set(x0, y0, 0.5f, 0, 0, Color::WHITE);
		v[4].set(x1, y1, 0.5f, 0, 0, Color::WHITE);
		v[5].set(x0, y1, 0.5f, 0, 0, Color::WHITE);
    }
}
...
void RenderingTest_IndirectDraw::render(CommandWriter& cmd, float time, const RenderTargetView* backBufferView, const RenderTargetView* depth)
{
	//--
