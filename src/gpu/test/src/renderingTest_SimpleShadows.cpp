...
public:
	...
	{
		outPosition = Vector3(-4.5f, 0.5f, 1.5f);
...
void RenderingTest_SimpleShadows::render(CommandWriter& cmd, float time, const RenderTargetView* backBufferView, const RenderTargetView* backBufferDepth)
{
    // create shadowmap
    ...
    {
        SceneCamera camera;
		camera.setupDirectionalShadowmap(Vector3(0, 0, 0), m_scene->m_lightPosition);
		camera.calcMatrices();
