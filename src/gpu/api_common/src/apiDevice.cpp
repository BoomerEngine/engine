...
Point IBaseDevice::maxRenderTargetSize() const
{
	InplaceArray<Rect, 8> displays;
...
OutputObjectPtr IBaseDevice::createOutput(const OutputInitInfo& info)
{
	if (auto* swapchain = m_thread->createOptimalSwapchain(info))
	{
		...
		return RefNew<OutputObjectProxy>(output->handle(), m_thread->objectRegistry(), swapchain->flipped(), swapchain->windowInterface());
	}
...
BufferObjectPtr IBaseDevice::createBuffer(const BufferCreationInfo& info, const ISourceDataProvider* sourceData)
{
	...
	{
		return RefNew<BufferObjectProxy>(obj->handle(), m_thread->objectRegistry(), setup);
	}
...
ShaderObjectPtr IBaseDevice::createShaders(const ShaderData* shader)
{
	if (auto* obj = m_thread->createOptimalShaders(shader))
		return RefNew<ShadersObjectProxy>(obj->handle(), m_thread->objectRegistry(), shader->metadata());
...
ImageObjectPtr IBaseDevice::createImage(const ImageCreationInfo& info, const ISourceDataProvider* sourceData)
{
	...
	{
		return RefNew<ImageObjectProxy>(obj->handle(), m_thread->objectRegistry(), setup);
	}
...
SamplerObjectPtr IBaseDevice::createSampler(const SamplerState& info)
{
	if (auto* obj = m_thread->createOptimalSampler(info))
		return RefNew<SamplerObject>(obj->handle(), m_thread->objectRegistry());
...
GraphicsRenderStatesObjectPtr IBaseDevice::createGraphicsRenderStates(const GraphicsRenderStatesSetup& states)
{
	...
	return RefNew<GraphicsRenderStatesObject>(obj->handle(), m_thread->objectRegistry(), states, obj->key());
}

//--

void IBaseDevice::enumMonitorAreas(Array<Rect>& outMonitorAreas) const
{
	return m_windows->enumMonitorAreas(outMonitorAreas);
}

void IBaseDevice::enumDisplays(Array<DisplayInfo>& outDisplayInfos) const
{
	return m_windows->enumDisplays(outDisplayInfos);
}

void IBaseDevice::enumResolutions(uint32_t displayIndex, Array<ResolutionInfo>& outResolutions) const
{
...
void IBaseDevice::submitWork(CommandBuffer* masterCommandBuffer, bool background)
{
