...
bool Output::prepare_ClientApi(IDeviceObject* owningObject, RenderTargetViewPtr* outColorRT, RenderTargetViewPtr* outDepthRT, Point& outViewport)
{
	...
	if (outColorRT)
	{
		if (state.colorFormat != ImageFormat::UNKNOWN)
		{
			...
			*outColorRT = RefNew<RenderTargetView>(m_colorTarget->handle(), owningObject, owner()->objectRegistry(), setup);
		}
		else
		{
			*outColorRT = nullptr;
	...
	if (outDepthRT)
	{
		if (state.depthFormat != ImageFormat::UNKNOWN)
		{
			...
			*outDepthRT = RefNew<RenderTargetView>(m_depthTarget->handle(), owningObject, owner()->objectRegistry(), setup);
		}
		else
		{
			*outDepthRT = nullptr;
...
OutputObjectProxy::OutputObjectProxy(ObjectID id, IDeviceObjectHandler* impl, bool flipped, INativeWindowInterface* window)
    : gpu::IOutputObject(id, impl, flipped, window)
{}

bool OutputObjectProxy::prepare(RenderTargetViewPtr* outColorRT, RenderTargetViewPtr* outDepthRT, Point& outViewport)
{
