...
WindowWinApi::WindowWinApi()
{
	...
	m_nextFPSCapture = NativeTimePoint::Now() + 1.0;
}
...
Point WindowWinApi::windowGetClientPlacement() const
{
	DEBUG_CHECK_EX(IsMainThread(), "Windows can only be touched on main thread");
	if (IsMainThread())
	{
...
Point WindowWinApi::windowGetClientSize() const
{
	DEBUG_CHECK_EX(IsMainThread(), "Windows can only be touched on main thread");
	if (IsMainThread())
	{
...
Point WindowWinApi::windowGetWindowPlacement() const
{
	DEBUG_CHECK_EX(IsMainThread(), "Windows can only be touched on main thread");
	if (IsMainThread())
	{
...
bool WindowWinApi::windowGetWindowDefaultPlacement(Rect& outWindowNormalRect) const
{
	WINDOWPLACEMENT plc;
...
Point WindowWinApi::windowGetWindowSize() const
{
	DEBUG_CHECK_EX(IsMainThread(), "Windows can only be touched on main thread");
	if (IsMainThread())
	{
...
{
	m_fullTitleString = TempString("{} (FPS: {})", m_currentTitle, m_numLastFramesRendered);
	SetWindowTextA(m_hWnd, m_fullTitleString.c_str());
}

void WindowWinApi::windowSetTitle(const StringBuf& title)
{
	DEBUG_CHECK_EX(IsMainThread(), "Windows can only be touched on main thread");
	if (IsMainThread())
	{
...
void WindowWinApi::windowAdjustClientPlacement(const Rect& clientRect)
{
	DEBUG_CHECK_EX(IsMainThread(), "Windows can only be touched on main thread");
	if (IsMainThread())
	{
...
void WindowWinApi::windowAdjustWindowPlacement(const Rect& windowRect)
{
	DEBUG_CHECK_EX(IsMainThread(), "Windows can only be touched on main thread");
	if (IsMainThread())
	{
...
void WindowManagerWinApi::updateWindows()
{
	...
	InplaceArray<HWND, 10> windowHandles;
...
{
	auto outList = (Array<Rect>*)lParam;
	...
	{
		auto monitorX = info.rcWork.left;
		auto monitorY = info.rcWork.top;
		auto monitorWidth = info.rcWork.right - info.rcWork.left;
		auto monitorHeight = info.rcWork.bottom - info.rcWork.top;
		float dpiScale = std::max<float>(mode.dmPelsWidth / (float)monitorWidth, mode.dmPelsHeight / (float)monitorHeight);

		Rect monitorRect;
		monitorRect.min.x = monitorX;
		monitorRect.min.y = monitorY;
		monitorRect.max.x = monitorX + mode.dmPelsWidth;
		monitorRect.max.y = monitorY + mode.dmPelsHeight;
...
void WindowManagerWinApi::enumMonitorAreas(Array<Rect>& outMonitorAreas) const
{
	outMonitorAreas = m_monitorRects;
}

void WindowManagerWinApi::enumDisplays(Array<DisplayInfo>& outDisplayInfos) const
{
	for (auto& info : m_cachedDisplayInfos)
		outDisplayInfos.pushBack(info.m_displayInfo);
}

void WindowManagerWinApi::enumResolutions(uint32_t displayIndex, Array<ResolutionInfo>& outResolutions) const
{
