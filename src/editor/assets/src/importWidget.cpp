...
void AssetFileImportWidget::updateStatus()
{
    ...
    if (m_checker->status(flags))
    {
        ...
        m_buttonReimport->enable(visualState.imported);
        m_buttonShowFileList->enable(visualState.imported);
        m_buttonRecheck->enable(true);

        StringBuilder text;
        text << visualState.tag;
        text << "[b]";
        text << visualState.icon;
        text << " ";
        text << visualState.caption;
        text << "[/b]";
        text << "[/color]";
        text << "[/tag]";

        m_statusText->text(text.view());
        m_statusText->tooltip(visualState.tooltip);
    }
}
