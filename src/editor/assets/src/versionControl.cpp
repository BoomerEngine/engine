...
RTTI_BEGIN_TYPE_ENUM(VersionControlFileLocalAction);
...
RTTI_BEGIN_TYPE_ENUM(VersionControlFileActionType);
...
void VersionControlFileState::print(IFormatStream& ret) const
{
    if (m_isOutdated)
        ret.append("Outdated ");
    else
    ...
    {
        case VersionControlFileLocalAction::CheckedOut:
            ret.append("Checkedout");
            break;

        case VersionControlFileLocalAction::MoveAdded:
            ret.append("MovedTo");
            break;

        case VersionControlFileLocalAction::MoveDeleted:
            ret.append("MovedOut");
            break;

        case VersionControlFileLocalAction::Deleted:
            ret.append("Deleted");
            break;

        case VersionControlFileLocalAction::Added:
            ret.append("Added");
            break;
    }
}

bool VersionControlFileState::canCheckout() const
{
    return m_isManaged && (m_localAction == VersionControlFileLocalAction::None);
}

bool VersionControlFileState::canCheckin() const
{
    return m_isManaged && (m_localAction != VersionControlFileLocalAction::None);
}

bool VersionControlFileState::canRevert() const
{
    return m_isManaged && (m_localAction != VersionControlFileLocalAction::None);
}

bool VersionControlFileState::canGetLatest() const
{
