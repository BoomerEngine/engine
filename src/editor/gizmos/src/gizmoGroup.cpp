...
GizmoGroup::GizmoGroup(const IGizmoHost* host, Array<GizmoPtr>&& gizmos)
    ...
    , m_hoverCursor(base::input::CursorType::Hand)
{
    alignGizmos();
}
...
void GizmoGroup::alignGizmos()
{
    if (m_host->gizmoHost_hasSelection())
    {
        ...
        const ui::CameraViewportSetup viewport(viewportCamera, viewportSize.x, viewportSize.y);
...
void GizmoGroup::render(rendering::scene::FrameParams& frame)
{
    // transform points
    alignGizmos();
...
bool GizmoGroup::updateHover(const base::Point& clientPoint)
{
    alignGizmos();
...
bool GizmoGroup::updateCursor(base::input::CursorType& outCursor)
{
    if (m_hoverGizmo)
    {
...
IGizmo* GizmoGroup::findGizmoUnderCursor(const base::Point& point, base::input::CursorType& outCursor) const
{
    const auto viewportSize = m_host->gizmoHost_viewportSize();
    const auto& viewportCamera = m_host->gizmoHost_camera();
    const ui::CameraViewportSetup viewport(viewportCamera, viewportSize.x, viewportSize.y);
