...
namespace ed
{
    namespace vsc
    {
        ...
        enum class FileActionType : uint8_t
        {
        ...
        /// file state
        struct EDITOR_COMMON_API FileState
        {
            FileLocalAction m_localAction; // intended local action for the file
            ...
            INLINE FileState()
                : m_localAction(FileLocalAction::None)
                , m_isManaged(false)
            ...
            INLINE bool operator==(const FileState& other) const
            {
        ...
        INLINE FilePaths MakeFileList(const FilePath &path)
        {
            FilePaths ret;
            ret.emplaceBack(path);
            return ret;
        }
        ...
        /// state of file locks
        struct EDITOR_COMMON_API FileLocks
        {
            FileLocks();

            using UserName = StringBuf;
            using UserNames = Array<UserName>;

            UserNames m_lockUser; // users that have lock on the file
            UserNames m_checkoutUsers; // users that have checked out the file
        };

        //------

        /// version control settings
        struct EDITOR_COMMON_API Settings
        {
            Settings();
        ...
        struct EDITOR_COMMON_API Result
        {
        public:
        ...
        class EDITOR_COMMON_API History
        {
        public:
            using HistoryEntries = Array<HistoryEntry>;

            FilePath m_file;
            HistoryEntries m_entries;
        ...
        class EDITOR_COMMON_API IChangelist : public IReferencable
        {
        public:
            IChangelist();
            virtual ~IChangelist();
            ...
            virtual Result files(FilePaths& outDepotPaths) const = 0;
        ...
        class EDITOR_COMMON_API IHostListener
        {
        public:
            virtual ~IHostListener();

            /// file under source control has changed its state
            virtual void VersionControl_FileStatusChanged(const FilePath& file, FileState fileState) {};

            /// source control action on a file has finished
            virtual void VersionControl_FileActionPerformed(const FilePath& file, FileActionType fileAction, bool wasSuccessful, const StringBuf& extraInfo) {};
        ...
        class EDITOR_COMMON_API IVersionControl
        {
        ...
        public:
            ...
            virtual Result moveFilesToChangelist(const FilePaths& files, const ChangelistPtr& targetChangelist) = 0;

            /// get changelist that contains given file
            virtual Result fileChangelist(const FilePaths& path, ChangelistPtr& outChangelist) = 0;

            ///---

            /// get history entry for given file
            virtual Result fileHistory(const FilePath& file, History& outHistory) const = 0;

            /// get source control state of a single file
            virtual Result fileInfo(const FilePath& file, FileState& outFileState) const = 0;

            /// get information about locks on the file
            virtual Result fileLocks(const FilePath&file, FileLocks& outFileLocks) const = 0;

            ///---

            /// get latest version of given files
            virtual Result latest(const FilePaths& files, bool force) = 0;

            /// get specific version of give files
            virtual Result revision(const FilePaths& files, uint32_t revision) = 0;

            /// refresh tracked state of files, state will be reported via host notifiers
            virtual Result refreshFiles(const FilePaths& files) = 0;

            ///---

            /// add existing files to changelist
            virtual Result checkoutFiles(const FilePaths& paths) = 0;

            /// add new files to changelist (they will be added to remote depot when changelist is submited)
            virtual Result addFiles(const FilePaths& paths) = 0;

            /// add existing files to be deleted in the remote depot
            virtual Result deleteFiles(const FilePaths& paths) = 0;

            /// move files from one location to other
            virtual Result renameFiles(const FilePaths& oldPaths, const FilePaths& newPaths) = 0;

            /// revert all local changes in files
            virtual Result revertFiles(const FilePaths& paths) = 0;
        };
