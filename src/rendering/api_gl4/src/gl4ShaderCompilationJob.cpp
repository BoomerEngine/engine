...
base::ConfigProperty<bool> cvDumpGLSL("Rendering.Shader", "DumpGLSL", true);
base::ConfigProperty<bool> cvBreakOnCompilationErrors("Rendering.Shader", "BreakOnCompilationErrors", true);

//--

ShaderCompilationJob::ShaderCompilationJob(base::Buffer data, const ShaderMetadata* metadata)
	: m_data(data)
...
void ShaderCompilationJob::print(base::IFormatStream& f) const
{
...
static void PrintNiceErrorMessage(base::StringView dumpPath, base::StringView errorText)
{
	base::InplaceArray<base::StringView, 64> lines;
	errorText.slice("\n", false, lines);
	...
	for (auto line : lines)
	{
		if (dumpPath && line.beginsWith("0("))
		{
			...
			if (numberText.match(lineNumber) == base::MatchResult::OK)
			{
				base::logging::Log::Print(base::logging::OutputLevel::Error, dumpPath.data(), lineNumber, "", "", base::TempString("{}", line.afterFirst(")")));
				continue;
			}
		}
...
{
	base::ScopeTimer stageTimer;
	base::StringBuilder code;
	...
	if (shaderCompiled != GL_TRUE)
	{
		...
		{
			base::logging::IErrorHandler::Assert(false, __FILE__, __LINE__, "GLSL compilation error", message, &cvBreakOnCompilationErrors.get());
		}
	...
	if (linkStatus != GL_TRUE)
	{
		...
		if (bufferSize > 0)
		{
			...
			if (cvBreakOnCompilationErrors.get())
			{
				base::logging::IErrorHandler::Assert(false, __FILE__, __LINE__, "shderCompiled != GL_TRUE",
					base::TempString("GLSL linking error!: {}", buffer.typedData()), &cvBreakOnCompilationErrors.get());
			}
...
void ShaderCompilationJob::process(volatile bool* vCancelFlag)
{
	...
	// unpack data
	base::StubLoader loader(shader::Stub::Factory(), 1, POOL_API_SHADERS);
	...
	base::InplaceArray<GLuint, 5> programs;
