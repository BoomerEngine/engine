...
void ShaderCodePrinter::printCode(base::StringBuilder& f)
{
	printHeader(f);
...
static base::StringView GetAttribute(const base::StubPseudoArray<shader::StubAttribute>& attributes, base::StringID key, base::StringView defaultValue)
{
...
static bool HasAttribute(const base::StubPseudoArray<shader::StubAttribute>& attributes, base::StringID key)
{
	for (const auto* attr : attributes)
		if (attr->name == key)
			return true;
...
static int GetAttributeInt(const base::StubPseudoArray<shader::StubAttribute>& attributes, base::StringID key, int defaultValue)
{
...
void ShaderCodePrinter::printHeader(base::StringBuilder& f)
{
...
void ShaderCodePrinter::printType(base::StringBuilder& f, const shader::StubTypeDecl* type, base::StringView varName)
{
	base::InplaceArray<int, 4> arrayCounts;
...
void ShaderCodePrinter::printVertexInputs(base::StringBuilder& f)
{
	...
	for (const auto* stream : m_program->vertexStreams)
	{
		for (const auto* element : stream->elements)
		{
			...
			auto name = base::StringBuf(base::TempString("_vi_{}_{}", stream->name, element->name));
...
void ShaderCodePrinter::printStructureDeclarations(base::StringBuilder& f)
{
...
static void PrintScalar(base::StringBuilder& f, shader::ScalarType type, const uint32_t*& data)
{
	switch (type)
	{
...
void ShaderCodePrinter::printConstant(base::StringBuilder& f, const shader::StubTypeDecl* type, const uint32_t*& data)
{
...
void ShaderCodePrinter::printGlobalConstants(base::StringBuilder& f)
{
	for (const auto* c : m_stage->globalConstants)
	{
		const auto name = base::StringBuf(base::TempString("_CONST_{}", c->index));
...
static void PrintTextureDeclaration(base::StringBuilder& f, ImageViewType type, bool ms, bool depth)
{
	f << "sampler";
...
static void PrintImageFormat(base::StringBuilder& f, ImageFormat format)
{
	auto& info = GetImageFormatInfo(format);
	f << info.shaderName;
}

static void PrintImageDeclaration(base::StringBuilder& f, ImageViewType type, ImageFormat format, bool ms, bool depth)
{
...
void ShaderCodePrinter::printResources(base::StringBuilder& f)
{
	for (const auto* m : m_stage->descriptorMembers)
	{
		...
		if (const auto* obj = m->asDescriptorMemberConstantBuffer())
		{
			...
			{
				auto name = base::StringBuf(base::TempString("_cb{}_x{}_x{}", binding, obj->descriptor->name, m->name)); // _cb0_ObjectParams_LocalToWorld
				m_paramNames[m] = name;
		...
		else if (const auto* obj = m->asDescriptorMemberSampledImage())
		{
			...
			auto name = base::StringBuf(base::TempString("_tex{}_x{}_x{}", binding, m->descriptor->name, m->name)); // _tex0_Material_DiffuseMap
			m_paramNames[m] = name;
		...
		else if (const auto* obj = m->asDescriptorMemberImage())
		{
			...
			auto name = base::StringBuf(base::TempString("_img{}_x{}_x{}", binding, m->descriptor->name, m->name)); // _img5_Lights_VoxelGrid
			m_paramNames[m] = name;
		...
		else if (const auto* obj = m->asDescriptorMemberFormatBuffer())
		{
			...
			auto name = base::StringBuf(base::TempString("_buf{}_x{}_x{}", binding, m->descriptor->name, m->name)); // _buf2_Test_TexelColors
			m_paramNames[m] = name;
		...
		else if (const auto* obj = m->asDescriptorMemberStructuredBuffer())
		{
			...
			auto name = base::StringBuf(base::TempString("_sbuf{}_x{}_x{}", binding, m->descriptor->name, m->name)); // _sbuf2_Lights_LightInfo
			m_paramNames[m] = name;
...
void ShaderCodePrinter::printBuiltIns(base::StringBuilder& f)
{
...
void ShaderCodePrinter::printStageInputs(base::StringBuilder& f)
{
	for (const auto* s : m_stage->inputs)
	{
		...
		m_paramNames[s] = base::StringBuf(s->name.view());
	}
...
void ShaderCodePrinter::printStageOutputs(base::StringBuilder& f)
{
...
void ShaderCodePrinter::printSharedMemory(base::StringBuilder& f)
{
...
void ShaderCodePrinter::printFunctionsForwardDeclarations(base::StringBuilder& f)
{
...
void ShaderCodePrinter::printFunctions(base::StringBuilder& f)
{
...
void ShaderCodePrinter::printHeader(base::StringBuilder& f, const shader::StubFunction* func)
{
...
void ShaderCodePrinter::printFunction(base::StringBuilder& f, const shader::StubFunction* func)
{
	printHeader(f, func);
	f << " {\n";
...
void ShaderCodePrinter::printVariableDeclaration(base::StringBuilder& f, const shader::StubOpcodeVariableDeclaration* var)
{
...
void ShaderCodePrinter::printStatement(base::StringBuilder& f, const shader::StubOpcode* baseOp, int depth)
{
...
void ShaderCodePrinter::printExpression(base::StringBuilder& f, const shader::StubOpcode* baseOp)
{
...
void ShaderCodePrinter::printExpressionStore(base::StringBuilder& f, const shader::StubOpcodeStore* op)
{
	// TODO: resource store
...
void ShaderCodePrinter::printExpressionNativeCall(base::StringBuilder& f, const shader::StubOpcodeNativeCall* op)
{
	base::StringView funcName = op->name.view();;
...
void ShaderCodePrinter::printExpressionCall(base::StringBuilder& f, const shader::StubOpcodeCall* op)
{
...
static void PrintBuiltIn(base::StringBuilder& f, shader::ShaderBuiltIn b)
{
	switch (b)
	{
...
void ShaderCodePrinter::printExpressionDataRef(base::StringBuilder& f, const shader::StubOpcodeDataRef* op)
{
...
void ShaderCodePrinter::printExpressionReadSwizzle(base::StringBuilder& f, const shader::StubOpcodeSwizzle* op)
{
	printExpression(f, op->value);
	if (!op->swizzle.empty())
	{
...
void ShaderCodePrinter::printExpressionAccessArray(base::StringBuilder& f, const shader::StubOpcodeAccessArray* op)
{
...
void ShaderCodePrinter::printExpressionAccessMember(base::StringBuilder& f, const shader::StubOpcodeAccessMember* op)
{
	printExpression(f, op->value);
	f.appendf(".{}", op->member->name);
}

void ShaderCodePrinter::printExpressionCreateVector(base::StringBuilder& f, const shader::StubOpcodeCreateVector* op)
{
...
void ShaderCodePrinter::printExpressionCreateMatrix(base::StringBuilder& f, const shader::StubOpcodeCreateMatrix* op)
{
	printType(f, op->typeDecl);
	f << "(";
...
void ShaderCodePrinter::printExpressionCreateArray(base::StringBuilder& f, const shader::StubOpcodeCreateArray* op)
{
...
void ShaderCodePrinter::printExpressionCast(base::StringBuilder& f, const shader::StubOpcodeCast* op)
{
...
void ShaderCodePrinter::printExpressionConstant(base::StringBuilder& f, const shader::StubOpcodeConstant* op)
{
	const auto* ptr = (const uint32_t*)op->data;
	printConstant(f, op->typeDecl, ptr);
}

void ShaderCodePrinter::printExpressionResourceLoad(base::StringBuilder& f, const shader::StubOpcodeResourceLoad* op)
{
	// use load function
	f << "imageLoad(  ";
...
void ShaderCodePrinter::printExpressionResourceStore(base::StringBuilder& f, const shader::StubOpcodeResourceStore* op)
{
	f << "imageStore(  ";
	printExpressionResourceRef(f, op->resourceRef),
	f << ",";
...
void ShaderCodePrinter::printExpressionResourceElement(base::StringBuilder& f, const shader::StubOpcodeResourceElement* op)
{
...
void ShaderCodePrinter::printExpressionResourceRef(base::StringBuilder& f, const shader::StubOpcodeResourceRef* op)
{
...
void ShaderCodePrinter::printFuncSaturate(base::StringBuilder& f, const shader::StubOpcodeNativeCall* op)
{
	f.appendf("clamp({}, 0.0, 1.0)", ARG(0));
}

void ShaderCodePrinter::printFuncCompareStd(base::StringBuilder& f, const shader::StubOpcodeNativeCall* op, const char* scalarOp, const char* vectorFunc)
{
...
void ShaderCodePrinter::printFuncMod(base::StringBuilder& f, const shader::StubOpcodeNativeCall* op)
{
...
void ShaderCodePrinter::printFuncSelect(base::StringBuilder& f, const shader::StubOpcodeNativeCall* op)
{
	f.appendf("({} ? {} : {})", ARG(0), ARG(1), ARG(2));
}

void ShaderCodePrinter::printFuncNotEqual(base::StringBuilder& f, const shader::StubOpcodeNativeCall* op)
{
	printFuncCompareStd(f, op, "!=", "notEqual");
}

void ShaderCodePrinter::printFuncEqual(base::StringBuilder& f, const shader::StubOpcodeNativeCall* op)
{
	printFuncCompareStd(f, op, "==", "equal");
}

void ShaderCodePrinter::printFuncGreaterEqual(base::StringBuilder& f, const shader::StubOpcodeNativeCall* op)
{
	printFuncCompareStd(f, op, ">=", "greaterThanEqual");
}

void ShaderCodePrinter::printFuncGreaterThen(base::StringBuilder& f, const shader::StubOpcodeNativeCall* op)
{
	printFuncCompareStd(f, op, ">", "greaterThan");
}

void ShaderCodePrinter::printFuncLessEqual(base::StringBuilder& f, const shader::StubOpcodeNativeCall* op)
{
	printFuncCompareStd(f, op, "<=", "lessThanEqual");
}

void ShaderCodePrinter::printFuncLessThen(base::StringBuilder& f, const shader::StubOpcodeNativeCall* op)
{
...
void ShaderCodePrinter::printFuncAtomicStd(base::StringBuilder& f, const shader::StubOpcodeNativeCall* op, base::StringView stem)
{
	// printable stem
	base::StringView functionStem = stem;
...
void ShaderCodePrinter::printFuncAtomicIncrement(base::StringBuilder& f, const shader::StubOpcodeNativeCall* op)
{
	printFuncAtomicStd(f, op, "tomicIncrement");
}

void ShaderCodePrinter::printFuncAtomicDecrement(base::StringBuilder& f, const shader::StubOpcodeNativeCall* op)
{
	printFuncAtomicStd(f, op, "tomicDecrement");
}

void ShaderCodePrinter::printFuncAtomicAdd(base::StringBuilder& f, const shader::StubOpcodeNativeCall* op)
{
	printFuncAtomicStd(f, op, "tomicAdd");
}

void ShaderCodePrinter::printFuncAtomicSubtract(base::StringBuilder& f, const shader::StubOpcodeNativeCall* op)
{
	printFuncAtomicStd(f, op, "tomicSubtract");
}

void ShaderCodePrinter::printFuncAtomicMin(base::StringBuilder& f, const shader::StubOpcodeNativeCall* op)
{
	printFuncAtomicStd(f, op, "tomicMin");
}

void ShaderCodePrinter::printFuncAtomicMax(base::StringBuilder& f, const shader::StubOpcodeNativeCall* op)
{
	printFuncAtomicStd(f, op, "tomicMax");
}

void ShaderCodePrinter::printFuncAtomicOr(base::StringBuilder& f, const shader::StubOpcodeNativeCall* op)
{
	printFuncAtomicStd(f, op, "tomicOr");
}

void ShaderCodePrinter::printFuncAtomicAnd(base::StringBuilder& f, const shader::StubOpcodeNativeCall* op)
{
	printFuncAtomicStd(f, op, "tomicAnd");
}

void ShaderCodePrinter::printFuncAtomicXor(base::StringBuilder& f, const shader::StubOpcodeNativeCall* op)
{
	printFuncAtomicStd(f, op, "tomicXor");
}

void ShaderCodePrinter::printFuncAtomicExchange(base::StringBuilder& f, const shader::StubOpcodeNativeCall* op)
{
	printFuncAtomicStd(f, op, "tomicExchange");
}

void ShaderCodePrinter::printFuncAtomicCompareSwap(base::StringBuilder& f, const shader::StubOpcodeNativeCall* op)
{
...
void ShaderCodePrinter::printFuncTextureGatherOffset(base::StringBuilder& f, const shader::StubOpcodeNativeCall* op)
{
	f.appendf("textureGatherOffset({}, {}, {}, 0)", ARG(0), ARG(1), ARG(2));
}

void ShaderCodePrinter::printFuncTextureGather(base::StringBuilder& f, const shader::StubOpcodeNativeCall* op)
{
...
void ShaderCodePrinter::printFuncTextureDepthCompare(base::StringBuilder& f, const shader::StubOpcodeNativeCall* op)
{
...
void ShaderCodePrinter::printFuncTextureDepthCompareOffset(base::StringBuilder& f, const shader::StubOpcodeNativeCall* op)
{
	const auto uvCompCount = CalcComponentCount(op->argumentTypes[1]);
	DEBUG_CHECK(uvCompCount >= 1 && uvCompCount <= 3);
