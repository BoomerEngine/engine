...
GPUCommandWriter::GPUCommandWriter(std::nullptr_t)
{
}

GPUCommandWriter::GPUCommandWriter(GPUCommandBuffer* buffer, base::StringView scopeName /*= base::StringView()*/)
{
    attachBuffer(buffer);
...
GPUCommandWriter::GPUCommandWriter(base::StringView scopeName /*= base::StringView()*/)
{
    attachBuffer(GPUCommandBuffer::Alloc());

    if (!scopeName.empty())
        opBeginBlock(scopeName);
}

GPUCommandWriter::~GPUCommandWriter()
{
    if (m_writeBuffer)
        detachBuffer(true);
}

void GPUCommandWriter::attachBuffer(GPUCommandBuffer* buffer)
{
    ...
    DEBUG_CHECK_EX(((GPUBaseOpcode*)m_writePtr)->op == GPUCommandCode::NewBuffer, "Trying to write to improperly maintained command buffer");
#ifndef PLATFORM_RELEASE
    DEBUG_CHECK_EX(((GPUBaseOpcode*)m_writePtr)->magic == GPUBaseOpcode::MAGIC, "Trying to write to improperly maintained command buffer");
#endif
}

void GPUCommandWriter::detachBuffer(bool finishRecording)
{
    ...
    if (m_writeBuffer)
    {
        ...
        endBuffer->setup(GPUCommandCode::NewBuffer);
...
GPUCommandBuffer* GPUCommandWriter::release(bool finishRecording /*= true*/)
{
    GPUCommandBuffer* ret = nullptr;
...
void GPUCommandWriter::ensureMemory(uint32_t size)
{
    if (m_writePtr + size <= m_writeEndPtr)
        return;

    // finish current buffer
    auto* newBuffer = GPUBaseOpcode::Alloc<OpNewBuffer>(m_writePtr, 0, m_lastCommand);
    ...
    m_lastCommand = GPUBaseOpcode::Alloc<OpNop>(m_writePtr);
    newBuffer->firstInNewBuffer = m_lastCommand;
}

AcquiredOutput GPUCommandWriter::opAcquireOutput(IOutputObject* output)
{
...
void GPUCommandWriter::opSwapOutput(IOutputObject* output, bool doSwap /*= true*/)
{
...
void GPUCommandWriter::opTriggerCapture()
{
    auto op  = allocCommand<OpTriggerCapture>();
}

bool GPUCommandWriter::validateFrameBuffer(const FrameBuffer& frameBuffer, uint32_t* outWidth /*= nullptr*/, uint32_t* outHeight /*= nullptr*/)
{
...
void GPUCommandWriter::opBeingPass(const FrameBuffer& frameBuffer, uint8_t viewportCount, const base::Rect& drawArea)
{
...
void GPUCommandWriter::opEndPass()
{
...
void GPUCommandWriter::opClearFrameBuffer(const FrameBuffer& frameBuffer, const base::Rect* area/* = nullptr*/)
{
...
void GPUCommandWriter::opBeginBlock(base::StringView name, base::StringView file, uint32_t line)
{
...
void GPUCommandWriter::opEndBlock()
{
...
GPUCommandBuffer* GPUCommandWriter::opCreateChildCommandBuffer(bool inheritParameters /*= true*/)
{
    GPUCommandBuffer* ret = GPUCommandBuffer::Alloc();
...
void GPUCommandWriter::opAttachChildCommandBuffer(GPUCommandBuffer* buffer)
{
...
void GPUCommandWriter::opSetViewportRect(uint8_t viewport, const base::Rect& viewportRect, float depthMin, float depthMax)
{
...
void GPUCommandWriter::opSetViewportRect(uint8_t viewport, int x, int y, int w, int h, float depthMin, float depthMax)
{
    opSetViewportRect(viewport, base::Rect(x, y, x + w, y + h), depthMin, depthMax);
}

void GPUCommandWriter::opSetBlendConstant(const base::Color& color)
{
	opSetBlendConstant(color.toVectorLinear());
}

void GPUCommandWriter::opSetBlendConstant(const base::Vector4& color)
{
	DEBUG_CHECK_RETURN(m_currentPass != nullptr);
...
void GPUCommandWriter::opSetBlendConstant(float r /*= 1.0f*/, float g /*= 1.0f*/, float b /*= 1.0f*/, float a /*= 1.0f*/)
{
	DEBUG_CHECK_RETURN(m_currentPass != nullptr);
...
void GPUCommandWriter::opSetLineWidth(float lineWidth)
{
    DEBUG_CHECK_RETURN(m_currentPass != nullptr);
...
void GPUCommandWriter::opSetScissorRect(uint8_t viewportIndex, const base::Rect& scissorRect)
{
    DEBUG_CHECK_RETURN(m_currentPass != nullptr);
...
void GPUCommandWriter::opSetScissorRect(uint8_t viewportIndex, int x, int y, int w, int h)
{
    opSetScissorRect(viewportIndex, base::Rect(x, y, x + w, y + h));
}

void GPUCommandWriter::opSetScissorBounds(uint8_t viewportIndex, int x0, int y0, int x1, int y1)
{
    opSetScissorRect(viewportIndex, base::Rect(x0, y0, x1, y1));
}

void GPUCommandWriter::opSetStencilReferenceValue(uint8_t value)
{
    DEBUG_CHECK_RETURN(m_currentPass != nullptr);
...
void GPUCommandWriter::opSetStencilReferenceValue(uint8_t frontValue, uint8_t backValue)
{
    DEBUG_CHECK_RETURN(m_currentPass != nullptr);
...
/*void GPUCommandWriter::opSetStencilCompareMask(uint8_t mask)
{
    DEBUG_CHECK_RETURN(m_currentPass != nullptr);
...
void GPUCommandWriter::opSetStencilCompareMask(uint8_t frontValue, uint8_t backValue)
{
    DEBUG_CHECK_RETURN(m_currentPass != nullptr);
...
void GPUCommandWriter::opSetStencilWriteMask(uint8_t mask)
{
    DEBUG_CHECK_RETURN(m_currentPass != nullptr);
...
void GPUCommandWriter::opSetStencilWriteMask(uint8_t front, uint8_t back)
{
    DEBUG_CHECK_RETURN(m_currentPass != nullptr);
...
void GPUCommandWriter::opSetDepthClip(float minBounds, float maxBounds)
{
	DEBUG_CHECK_RETURN(m_currentPass != nullptr);
...
/*void GPUCommandWriter::opSetDepthBias(float constant, float slopeFactor, float clampValue)
{
	DEBUG_CHECK_RETURN(m_currentPass != nullptr);
...
void GPUCommandWriter::opClearPassRenderTarget(uint32_t index, const base::Vector4& color)
{
    DEBUG_CHECK_RETURN(m_currentPass != nullptr);
...
void GPUCommandWriter::opClearPassDepthStencil(float depth, uint8_t stencil, bool doClearDepth /*= true*/, bool doClearStencil /*= true*/)
{
    DEBUG_CHECK_RETURN(m_currentPass != nullptr);
...
void GPUCommandWriter::opClearWritableBufferRects(const BufferWritableView* bufferView, const void* clearValue /*= nullptr*/, const ResourceClearRect* rects /*= nullptr*/, uint32_t numRects /*= 0*/)
{
...
void GPUCommandWriter::opClearWritableBuffer(const BufferWritableView* view, const void* clearValue /*= nullptr*/, uint32_t offset /*= 0*/, uint32_t size /*= INDEX_MAX*/)
{
...
void GPUCommandWriter::opClearWritableBuffer(const BufferWritableStructuredView* bufferView, uint32_t firstElement /*= 0*/, uint32_t numElements /*= INDEX_MAX*/)
{
    DEBUG_CHECK_RETURN_EX(bufferView != nullptr, "Missing buffer");
...
void GPUCommandWriter::opClearWritableImageRects(const ImageWritableView* imageView, const void* clearValue /*= nullptr*/, const ResourceClearRect* rects /*= nullptr*/, uint32_t numRects /*= 0*/)
{
...
void GPUCommandWriter::opClearWritableImage(const ImageWritableView* view, const void* clearValue)
{
	DEBUG_CHECK_RETURN(view);
...
void GPUCommandWriter::opClearRenderTarget(const RenderTargetView * view, const base::Vector4 & values, const base::Rect * rects /*= nullptr*/, uint32_t numRects /*= 0*/)
{
...
void GPUCommandWriter::opClearDepthStencil(const RenderTargetView* view, bool doClearDepth, bool doClearStencil, float clearDepth, uint32_t clearStencil, const base::Rect* rects /*= nullptr*/, uint32_t numRects /*= 0*/)
{
	DEBUG_CHECK_RETURN_EX(view, "Invalid render target view");
...
void GPUCommandWriter::opCopyRenderTarget(const RenderTargetView* source, const RenderTargetView* dest, uint16_t sourceSlice /*= 0*/, uint16_t destSlice /*= 0*/, bool flipY /*= false*/, base::Rect sourceRect /*= base::Rect()*/, base::Rect destRect /*= base::Rect()*/)
{
...
void GPUCommandWriter::opResolve(const ImageObject* msaaSource, const ImageObject* nonMsaaDest, uint8_t sourceMip /*= 0*/, uint8_t destMip /*= 0*/, uint16_t sourceSlice /*= 0*/, uint16_t destSlice /*= 0*/)
{
...
void GPUCommandWriter::opBindVertexBuffer(base::StringID bindpoint, const BufferObject* buffer, uint32_t offset /*= 0*/)
{
...
void GPUCommandWriter::opUnbindVertexBuffer(base::StringID bindpoint)
{
    DEBUG_CHECK_RETURN(bindpoint);// , "Invalid bind point");
...
void GPUCommandWriter::opBindIndexBuffer(const BufferObject* buffer, ImageFormat indexFormat, uint32_t offset /*= 0*/)
{
...
void GPUCommandWriter::opUnbindIndexBuffer()
{
    auto op  = allocCommand<OpBindIndexBuffer>();
    op->offset = 0;
...
bool GPUCommandWriter::validateDrawIndexLayout(uint32_t requiredElementCount)
{
#ifdef VALIDATE_VERTEX_LAYOUTS
...
bool GPUCommandWriter::validateDrawVertexLayout(const ShaderMetadata* meta, uint32_t requiredVertexCount, uint32_t requiredInstanceCount)
{
#ifdef VALIDATE_VERTEX_LAYOUTS
...
void GPUCommandWriter::printActiveDescriptors()
{
...
bool GPUCommandWriter::validateParameterBindings(const ShaderMetadata* meta)
{
...
void GPUCommandWriter::opDraw(const GraphicsPipelineObject* po, uint32_t firstVertex, uint32_t vertexCount)
{
    opDrawInstanced(po, firstVertex, vertexCount, 0, 1);
}

void GPUCommandWriter::opDrawInstanced(const GraphicsPipelineObject* po, uint32_t firstVertex, uint32_t vertexCount, uint16_t firstInstance, uint16_t numInstances)
{
    DEBUG_CHECK_RETURN(m_currentPass != nullptr);
...
void GPUCommandWriter::opDrawIndexed(const GraphicsPipelineObject* po, uint32_t firstVertex, uint32_t firstIndex, uint32_t indexCount)
{
    opDrawIndexedInstanced(po, firstVertex, firstIndex, indexCount, 0, 1);
}

void GPUCommandWriter::opDrawIndexedInstanced(const GraphicsPipelineObject* po, uint32_t firstVertex, uint32_t firstIndex, uint32_t indexCount, uint16_t firstInstance, uint16_t numInstances)
{
    DEBUG_CHECK_RETURN(m_currentPass != nullptr);
    DEBUG_CHECK_RETURN(po);
...
void GPUCommandWriter::opDispatchGroups(const ComputePipelineObject* co, uint32_t countX /*= 1*/, uint32_t countY /*= 1*/, uint32_t countZ /*= 1*/)
{
...
void GPUCommandWriter::opDispatchThreads(const ComputePipelineObject* co, uint32_t threadCountX /*= 1*/, uint32_t threadCountY /*= 1*/, uint32_t threadCountZ /*= 1*/)
{
	DEBUG_CHECK_RETURN(co);
...
bool GPUCommandWriter::validateIndirectDraw(const BufferObject* buffer, uint32_t offsetInBuffer, uint32_t commandStride)
{
...
void GPUCommandWriter::opDrawIndirect(const GraphicsPipelineObject* po, const BufferObject* buffer, uint32_t offsetInBuffer)
{
	DEBUG_CHECK_RETURN(m_currentPass != nullptr);
	DEBUG_CHECK_RETURN(po);
...
void GPUCommandWriter::opDrawIndexedIndirect(const GraphicsPipelineObject* po, const BufferObject* buffer, uint32_t offsetInBuffer)
{
	DEBUG_CHECK_RETURN(m_currentPass != nullptr);
	DEBUG_CHECK_RETURN(po);
...
void GPUCommandWriter::opDispatchGroupsIndirect(const ComputePipelineObject* po, const BufferObject* buffer, uint32_t offsetInBuffer)
{
	DEBUG_CHECK_RETURN(m_currentPass != nullptr);
	DEBUG_CHECK_RETURN(po);
...
void GPUCommandWriter::opTransitionFlushUAV(const ImageWritableView* imageView)
{
...
void GPUCommandWriter::opTransitionFlushUAV(const BufferWritableView* bufferView)
{
...
ResourceCurrentStateTrackingRecord* GPUCommandWriter::createResourceStateTrackingEntry(const IDeviceObject* obj)
{
...
bool GPUCommandWriter::ensureResourceState(const IDeviceObject* object, ResourceLayout layout, const SubImageRegion* subImageRegion, ResourceLayout newLayout)
{
...
void GPUCommandWriter::opTransitionLayout(const IDeviceObject* obj, ResourceLayout incomingLayout, ResourceLayout outgoingLayout)
{
...
void GPUCommandWriter::opTransitionImageRangeLayout(const ImageObject* obj, uint8_t firstMip, uint8_t numMips, ResourceLayout incomingLayout, ResourceLayout outgoingLayout)
{
	DEBUG_CHECK_RETURN(obj != nullptr);
	DEBUG_CHECK_RETURN(incomingLayout != outgoingLayout);
...
void GPUCommandWriter::opTransitionImageArrayRangeLayout(const ImageObject* obj, uint8_t firstMip, uint8_t numMips, uint32_t firstSlice, uint32_t numSlices, ResourceLayout incomingLayout, ResourceLayout outgoingLayout)
{
	DEBUG_CHECK_RETURN(obj != nullptr);
	DEBUG_CHECK_RETURN(incomingLayout != outgoingLayout);
	DEBUG_CHECK_RETURN(obj->subResourceLayouts());
...
void GPUCommandWriter::opTransitionImageArrayRangeLayout(const ImageObject* obj, uint32_t firstSlice, uint32_t numSlices, ResourceLayout incomingLayout, ResourceLayout outgoingLayout)
{
	DEBUG_CHECK_RETURN(obj != nullptr);
	DEBUG_CHECK_RETURN(incomingLayout != outgoingLayout);
	DEBUG_CHECK_RETURN(obj->subResourceLayouts());
...
void GPUCommandWriter::linkUpdate(OpUpdate* op)
{
...
void* GPUCommandWriter::opUpdateDynamicPtr(const IDeviceObject* dynamicObject, const ResourceCopyRange& range)
{
...
void GPUCommandWriter::opUpdateDynamicImage(const ImageObject* dynamicImage, const base::image::ImageView& sourceData, uint8_t mipIndex /*= 0*/, uint32_t sliceIndex /*= 0*/, uint32_t offsetX /*= 0*/, uint32_t offsetY /*= 0*/, uint32_t offsetZ /*= 0*/)
{
...
void GPUCommandWriter::opUpdateDynamicBuffer(const BufferObject* dynamicBuffer, uint32_t dataOffset, uint32_t dataSize, const void* dataPtr)
{
	ResourceCopyRange range;
...
void GPUCommandWriter::opCopy(const IDeviceObject* src, const ResourceCopyRange& srcRange, const IDeviceObject* dest, const ResourceCopyRange& destRange)
{
...
void GPUCommandWriter::opCopyBuffer(const BufferObject* src, uint32_t srcOffset, const BufferObject* dest, uint32_t destOffset, uint32_t size)
{
...
void GPUCommandWriter::opCopyBufferToImage(const BufferObject* src, uint32_t srcOffset, const ImageObject* dest, uint32_t offsetX, uint32_t offsetY, uint32_t offsetZ, uint32_t sizeX, uint32_t sizeY, uint32_t sizeZ, uint8_t mipIndex /*= 0*/, uint32_t sliceIndex /*= 0*/)
{
	ResourceCopyRange destRange;
...
void GPUCommandWriter::opCopyImageToBuffer(const ImageObject* src, uint32_t offsetX, uint32_t offsetY, uint32_t offsetZ, uint32_t sizeX, uint32_t sizeY, uint32_t sizeZ, const BufferObject* dest, uint32_t destOffset, uint8_t mipIndex /*= 0*/, uint32_t sliceIndex /*= 0*/)
{
	ResourceCopyRange srcRange;
...
void GPUCommandWriter::opDownloadData(const IDeviceObject* obj, const ResourceCopyRange& range, IDownloadDataSink* sink)
{
...
void GPUCommandWriter::opBindDescriptorEntries(base::StringID binding, const DescriptorEntry* entries, uint32_t count)
{
...
void* GPUCommandWriter::allocConstants(uint32_t size, const OpUploadConstants*& outOffsetPtr)
{
...
DescriptorEntry* GPUCommandWriter::uploadDescriptor(DescriptorID layoutID, const DescriptorInfo* layout, const DescriptorEntry* entries, uint32_t count)
{
    ...
    for (uint32_t i = 0; i < count; ++i)
    {
        ...
        if (entry.type == DeviceObjectViewType::ConstantBuffer && entry.inlinedConstants.sourceDataPtr)
        {
			...
			const OpUploadConstants* uploadCommand = nullptr;
