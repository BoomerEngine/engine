...
enum class GPUCommandCode : uint16_t
{
...
struct GPUBaseOpcode
{
    ...
    GPUCommandCode op = GPUCommandCode::Nop;
    uint16_t offsetToNext = 0; // offset to next from current

    INLINE GPUBaseOpcode* setup(GPUCommandCode op_)
    {
        op = op_;
        offsetToNext = 0;
#ifndef BUILD_RELEASE
    ...
    INLINE GPUBaseOpcode* setup(GPUCommandCode op_, GPUBaseOpcode* prev)
    {
        op = op_;
        offsetToNext = 0;
#ifndef BUILD_RELEASE
    ...
    template< typename T >
    static INLINE T* Alloc(uint8_t*& ptr, uint32_t extraSize, GPUBaseOpcode*& lastCommand)
    {
        auto* cmd = (T*)ptr;
    ...
    INLINE const GPUBaseOpcode* next() const
    {
...
#ifdef BUILD_RELEASE
static_assert(sizeof(GPUBaseOpcode) == 4, "Please don't make me bigger");
#else
static_assert(sizeof(GPUBaseOpcode) == 8, "Please don't make me bigger");
#endif

template< GPUCommandCode opcode, typename SelfT >
struct GPUBaseOpcodeT : public GPUBaseOpcode
{
...
#define RENDER_DECLARE_OPCODE_DATA(op_) struct Op##op_ : public GPUBaseOpcodeT<GPUCommandCode::op_, Op##op_>
...
{
    GPUBaseOpcode* firstInNewBuffer = nullptr;
};

RENDER_DECLARE_OPCODE_DATA(ChildBuffer)
{
    GPUCommandBuffer* childBuffer = nullptr;
...
INLINE const GPUBaseOpcode* GetNextCommand(const GPUBaseOpcode* op)
{
#ifndef BUILD_RELEASE
    ASSERT_EX(op->magic == GPUBaseOpcode::MAGIC, "Invalid opcode magic - buffer overrun");
#endif
    const auto* next = op->next();
    while (next && next->op == GPUCommandCode::NewBuffer)
