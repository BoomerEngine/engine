...
class RENDERING_DEVICE_API GPUCommandWriter : public base::NoCopy
{
public:
    GPUCommandWriter(std::nullptr_t); // unattached
    GPUCommandWriter(GPUCommandBuffer* buffer, base::StringView scopeName = base::StringView()); // NOTE: buffer is reset, that's the only legal way we can write to it
    GPUCommandWriter(base::StringView scopeName = base::StringView()); // NOTE: buffer is reset, that's the only legal way we can write to it
    ~GPUCommandWriter();

    //--

    // attach command buffer to empty writer
    void attachBuffer(GPUCommandBuffer* buffer);

    // release buffer from the writer, finished it as well
    GPUCommandBuffer* release(bool finishRecording = true);
    ...
    GPUCommandBuffer* opCreateChildCommandBuffer(bool inheritParameters=true);
    ...
    void opAttachChildCommandBuffer(GPUCommandBuffer* buffer);
...
private:
    ...
    GPUBaseOpcode* m_lastCommand = nullptr;

    uint8_t* m_writePtr = nullptr;
    uint8_t* m_writeEndPtr = nullptr;
    GPUCommandBuffer* m_writeBuffer = nullptr;
...
{
public:
    INLINE CommandWriterBlock(GPUCommandWriter& cmd, base::StringView name)
    {
        if (name)
        {
            m_writer = &cmd;
...
private:
    GPUCommandWriter* m_writer = nullptr;
};
