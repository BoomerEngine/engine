...
struct GPUCommandBufferGatheredState
{
...
class RENDERING_DEVICE_API GPUCommandBuffer : public base::NoCopy
{
...
public:
    ...
    INLINE const GPUBaseOpcode* commands() const { return m_commands; }

    // get gathered state that can be used to speed up recording of native command buffers
    INLINE const GPUCommandBufferGatheredState& gatheredState() const { return m_gatheredState; }
    ...
    bool enumChildren(const std::function<bool(GPUCommandBuffer * buffer)>& enumFunc);

    // visit the whole hierarchy in order
    bool visitHierarchy(const std::function<bool(GPUCommandBuffer * buffer)>& enumFunc);
...
private:
    ...
    const GPUBaseOpcode* m_commands = nullptr;
    ...
    GPUCommandBufferGatheredState m_gatheredState;
    ...
    friend class GPUCommandWriter;
};

//--

// validate command buffer
extern RENDERING_DEVICE_API void ValidateCommandBuffer(GPUCommandBuffer* buffer);
