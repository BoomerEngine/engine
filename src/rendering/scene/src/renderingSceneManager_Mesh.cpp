...
void ObjectManagerMesh::prepare(GPUCommandWriter& cmd, IDevice* dev, const FrameRenderer& frame)
{
...
void ObjectManagerMesh::renderChunkListStandalone(GPUCommandWriter& cmd, const base::Array<VisibleStandaloneChunk>& chunks, MaterialPass pass) const
{
    const auto* objects = m_localObjects.values().typedData();
