...
static bool HasTriggeredCapture(const GPUCommandBuffer* masterCommandBuffer)
{
	auto* cmd = masterCommandBuffer->commands();
	while (cmd)
	{
		if (cmd->op == GPUCommandCode::TriggerCapture)
		{
			return true;
		}
		else if (cmd->op == GPUCommandCode::ChildBuffer)
		{
...
base::UniquePtr<IFrameCapture> IFrameCapture::ConditionalStartCapture(GPUCommandBuffer* masterCommandBuffer)
{
