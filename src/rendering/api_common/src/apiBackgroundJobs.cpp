...
namespace rendering
{
	namespace api
	{
			...
			if (!m_completed.load())
			{
				...
				else
				{
					// ensure mutex exists
					{
						...
						if (!m_completed.load())
						{
							m_completionEvent = new base::Event(true);
						}
					}
		...
		base::ConfigProperty<uint32_t> cvBackgroundQueueThreads("Rendering", "BackgroundJobThreadCount", 1);
		base::ConfigProperty<uint32_t> cvBackgroundQueueTimeout("Rendering", "BackgroundQueueTimeout", 100);
		base::ConfigProperty<uint32_t> cvBackgroundQueueSemaphoreWait("Rendering", "BackgroundQueueSemaphoreWait", 5);

		bool IBaseBackgroundQueue::initialize(const base::app::CommandLine& cmdLine)
		{
		...
		void IBaseBackgroundQueue::JobEntry::print(base::IFormatStream& f) const
		{
			job->print(f);
		}
