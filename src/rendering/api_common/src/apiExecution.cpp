...
void IBaseFrameExecutor::execute(GPUCommandBuffer* commandBuffer)
{
...
void IBaseFrameExecutor::executeSingle(GPUCommandBuffer* commandBuffer)
{
	...
	while (cmd)
	{
		switch (cmd->op)
		{
#define RENDER_COMMAND_OPCODE(x) case GPUCommandCode::##x: { run##x(*static_cast<const Op##x*>(cmd)); break; }
