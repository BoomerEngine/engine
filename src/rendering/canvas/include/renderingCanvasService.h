...
struct RENDERING_CANVAS_API CanvasRenderAtlasImage
{
	...
	void updateImage(GPUCommandWriter& cmd, const base::image::ImageView& sourceImage, uint8_t pageIndex, const base::Rect& dirtyRect);
	void updateEntries(GPUCommandWriter& cmd, uint32_t firstEntry, uint32_t numEntries, const base::canvas::ImageAtlasEntryInfo* entiresData);
};
...
public:
	...
	void render(GPUCommandWriter& cmd, const base::canvas::Canvas& canvas);
...
private:
	...
	virtual void updateGlyphCache(rendering::GPUCommandWriter& cmd, const base::canvas::ICanvasAtlasSync::AtlasUpdate& update) override final;
	virtual void updateAtlas(rendering::GPUCommandWriter& cmd, base::canvas::ImageAtlasIndex atlasIndex, const base::canvas::ICanvasAtlasSync::AtlasUpdate& update) override final;
};
