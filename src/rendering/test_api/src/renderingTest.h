...
public:
    ...
    void drawChunk(GPUCommandWriter& cmd, const GraphicsPipelineObject* func, uint32_t chunkIndex) const;
    void drawMesh(GPUCommandWriter& cmd, const GraphicsPipelineObject* func) const;
};
...
public:
    ...
    virtual void render(GPUCommandWriter& cmd, float time, const RenderTargetView* backBufferView, const RenderTargetView* backBufferDepthView ) = 0;
    ...
    void drawQuad(GPUCommandWriter& cmd, const GraphicsPipelineObject* func, float x=-1.0f, float y = -1.0f, float w=2.0f, float h = 2.0f, float u0=0.0f, float v0 = 0.0f, float u1 = 1.0f, float v1 = 1.0f, base::Color color = base::Color::WHITE);

    // configure quad params
    void setQuadParams(GPUCommandWriter& cmd, float x, float y, float w, float h);
    void setQuadParams(GPUCommandWriter& cmd, const RenderTargetView* rt, const base::Rect& rect);
...
struct VertexIndexBunch
{
    ...
    void draw(GPUCommandWriter& cmd, const GraphicsPipelineObject* func, uint16_t firstInstance = 0, uint16_t numInstances = 1) const
    {
        if (m_indexBuffer)
        {
