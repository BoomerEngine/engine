...
private:
	...
	void recordRowRange(GPUCommandWriter& cmd, uint32_t firstRow, uint32_t lastRow);
...
void RenderingTest_InlineParameters::recordRowRange(GPUCommandWriter& cmd, uint32_t firstRow, uint32_t lastRow)
{
	//--
...
void RenderingTest_InlineParameters::render(GPUCommandWriter& cmd, float time, const RenderTargetView* backBufferView, const RenderTargetView* backBufferDepthView )
{
	//--
	...
	else
	{
		...
		for (uint32_t i = 0; i < numFibers; ++i)
		{
			...
			{
				GPUCommandWriter childWriter(childCommandBuffer);
