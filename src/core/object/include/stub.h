...
struct CORE_OBJECT_API IStub : public NoCopy
{
...
#define STUB_CLASS(x) \
static StubTypeValue StaticType() { return (StubTypeValue)(StubType::##x); } \
virtual StubTypeValue runtimeType() const override { return (StubTypeValue)(StubType::##x); } \
...
class CORE_OBJECT_API IStubWriter : public NoCopy
{
public:
	virtual ~IStubWriter();
	...
	{
		writeRefList((const IStub* const*) val.elems, val.size());
	}
};

// abstract loader/unpacker of stub data
class CORE_OBJECT_API IStubReader : public NoCopy
{
public:
