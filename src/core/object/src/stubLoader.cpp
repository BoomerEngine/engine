		...
		if (!m_hasErrors && TryReadCompressedInt(m_objectMemPtr, m_objectMemEndPtr, index))
		{
			...
			if (index > 0 && index <= m_tables.stubs.lastValidIndex())
			{
				...
				if (stub)
				{
					...
					DEBUG_CHECK_EX(!expectedType || stub->runtimeType() == expectedType, TempString("Expected stub of type {}, got {} ({})", expectedType, stub->runtimeType(), stub->debugName()));
					return stub;
				}
			}
		}
...
const IStub* StubLoader::unpack(const void* packedData, uint32_t packedDataSize)
{
	...
	DEBUG_CHECK_RETURN_EX_V(header->version >= m_minVersion, TempString("Shader blob is in version {} but minimal supported version is {}", header->version, m_minVersion), nullptr);
	...
	// load stub content
	{
		...
		for (uint32_t i = 1; i < header->numStubs; ++i)
		{
...

			{
				...
				DEBUG_CHECK_RETURN_EX_V(!reader.hasErrors(), TempString("Stub '{}' at index {}: read error at {} of size {}", stub->debugName(), i, sizeRead, size), nullptr);
				DEBUG_CHECK_RETURN_EX_V(sizeRead == size, TempString("Stub '{}' at index {}: read {} bytes of expected {}", stub->debugName(), i, sizeRead, size), nullptr);
			}
		}
	}
