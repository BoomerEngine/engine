...
MemoryWriterFileHandle::MemoryWriterFileHandle(uint64_t initialSize /*= 1U << 20*/)
{
    if (initialSize > 0)
    {
        ...
        m_base = (uint8_t*)AllocSystemMemory(initialSize, true);
        if (m_base)
        {
            m_pos = m_base;
...
bool MemoryWriterFileHandle::increaseCapacity(uint64_t minAdditionalSize)
{
    ...
    // allocate new data
    void* newBuffer = AllocSystemMemory(capcity, true);
    if (!newBuffer)
    {
    ...
    // release old data
    FreeSystemMemory(m_base, m_end - m_base);
