...
bool FunctionCode::makeIntoMatchingTypeFuncCall(script::IErrorHandler &err, script::FunctionNode *&node, const script::FunctionTypeInfo &requiredType, bool explicitCast)
{
    // convert into basic shit
    if (!makeIntoMatchingTypeNoRefChange(err, node, requiredType, explicitCast))
        return false;
