...
public:
    ...
    /// get raw vertices
    INLINE const Vertex* vertices() const { return valid() ? (const Vertex*)OffsetPtr(m_data.data(), header().vertexOffset) : nullptr; }

    /// get raw edges
    INLINE const ConvexEdge* edges() const { return valid() ? (const ConvexEdge*)OffsetPtr(m_data.data(), header().edgeOffset) : nullptr; }

    /// get raw planes
    INLINE const Plane* planes() const { return valid() ? (const Plane*)OffsetPtr(m_data.data(), header().planeOffset) : nullptr; }

    /// get raw faces
    INLINE const Face* faces() const { return valid() ? (const Face*)OffsetPtr(m_data.data(), header().faceOffset) : nullptr; }
