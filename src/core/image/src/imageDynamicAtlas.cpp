...
bool DynamicAtlas::placeImage(const ImageView& sourceImage, uint32_t padding, bool supportWrapping, DynamicAtlasEntry& outEntry)
{
	...
	if (supportWrapping)
	{
		...
		// top/bottom line
		image::Copy(sourceImage.subView(0, 0, w, my), fullView.subView(p, p+h, w, my));
		image::Copy(sourceImage.subView(0, h - my, w, my), fullView.subView(p, p-my, w, my));

		// left/right line
		image::Copy(sourceImage.subView(0, 0, mx, h), fullView.subView(p+w, p, mx, h));
		image::Copy(sourceImage.subView(w - mx, 0, mx, h), fullView.subView(p-mx, p, mx, h));

		// corners
		//image::Copy(sourceImage.subView(0, 0, m, m), ret->view().subView(fw - m, fh - m, m, m));
		//image::Copy(sourceImage.subView(w - m, h - m, m, m), ret->view().subView(0, 0, m, m));
		//image::Copy(sourceImage.subView(0, h - m, m, m), ret->view().subView(fw - m, 0, m, m));
		//image::Copy(sourceImage.subView(w - m, 0, m, m), ret->view().subView(0, fh - m, m, m));
	}
