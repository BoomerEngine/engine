...
ImageAtlas::ImageAtlas(uint32_t width, uint32_t height, uint8_t numChannels /*= 1*/, ImagePixelFormat format /*= ImagePixelFormat::Uint8_Norm*/)
    : m_width(width)
    , m_height(height)
...
bool ImageAtlas::placeImage(const ImageView& sourceImage, uint32_t padding, bool supportWrapping, ImageAtlasEntry& outEntry)
{
	...
	// if we want to support wrapping we need to manually repeat some part of the image around the tile
	...
	if (supportWrapping)
	{
		...
		// top/bottom line
		Copy(sourceImage.subView(0, 0, w, my), fullView.subView(p, p+h, w, my));
		Copy(sourceImage.subView(0, h - my, w, my), fullView.subView(p, p-my, w, my));

		// left/right line
		Copy(sourceImage.subView(0, 0, mx, h), fullView.subView(p+w, p, mx, h));
		Copy(sourceImage.subView(w - mx, 0, mx, h), fullView.subView(p-mx, p, mx, h));

		// corners
		//Copy(sourceImage.subView(0, 0, m, m), ret->view().subView(fw - m, fh - m, m, m));
		//Copy(sourceImage.subView(w - m, h - m, m, m), ret->view().subView(0, 0, m, m));
		//Copy(sourceImage.subView(0, h - m, m, m), ret->view().subView(fw - m, 0, m, m));
		//Copy(sourceImage.subView(w - m, 0, m, m), ret->view().subView(0, fh - m, m, m));
	}
