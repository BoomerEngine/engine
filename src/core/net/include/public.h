...
#include "core_net_glue.inl"

#include "core/system/include/timing.h"

#define RESPONSE_FUNC const http::RequestResult& result
#define REQUEST_FUNC const http::IncomingRequestPtr& request
...
struct CORE_NET_API MessageStats
{
public:
    MessageStats();
...
struct CORE_NET_API RequestResult
{
    uint32_t code;
