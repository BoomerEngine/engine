...
public:
    ...
    CORE_CONTAINERS_API ~BaseArrayBuffer();
    ...
    // release memory
    CORE_CONTAINERS_API void release();
    ...
    CORE_CONTAINERS_API void resize(Count newCapcity, uint64_t currentMemorySize, uint64_t newMemorySize, uint64_t memoryAlignment, const char* typeName);
...
public:
	...
	CORE_CONTAINERS_API Count changeSize(Count newSize);
	...
	CORE_CONTAINERS_API Count changeCapacity(Count newCapacity, uint64_t currentMemorySize, uint64_t newMemorySize, uint64_t memoryAlignment, const char* typeNameInfo="");

    //--

    // calculate NEXT best array buffer size - when resizing array from given size
    CORE_CONTAINERS_API static uint64_t CalcNextBufferSize(uint64_t currentSize);

protected:
    CORE_CONTAINERS_API void suckFrom(BaseArray& arr);

#ifndef BUILD_RELEASE
	CORE_CONTAINERS_API void checkIndex(Index index) const;
	CORE_CONTAINERS_API void checkIndexRange(Index index, Count count) const;
#else
