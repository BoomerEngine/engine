...
namespace prv
{
    ...
    struct CORE_CONTAINERS_API BaseHelper
    {
...
{
    CaseInsensitiveKey(BaseStringView<char> view)
    {
        m_data = StringBuf(view).toLower();
    }

    CaseInsensitiveKey(const StringBuf& txt)
    {
        m_data = txt.toLower();
    }
    ...
    static uint32_t CalcHash(BaseStringView<char> txt)
    {
        return prv::BaseHelper::StringHashNoCase(txt.data(), txt.data() + txt.length());
    }
    ...
    INLINE bool operator==(BaseStringView<char> txt) const
    {
