...
namespace helper
{
    static uint32_t CountGridElements(const StreamingGrid& grid)
    {
        uint32_t numTotalElems = 0;
        for ( uint32_t i=0; i<NUM_GRID_LEVELS; ++i )
        {
            StreamingGridDebugInfo data;
...
TEST(Streaming, StreamingGrid_10k_AddRemove)
{
    ...
    // create grid
    containers::StreamingGrid grid(NUM_GRID_LEVELS, NUM_GRID_BUCKETS);
...
TEST(Streaming, StreamingGrid_10k_TestPoints)
{
    ...
    for (uint32_t i=0; i<500; ++i)
    {
        ...
        // query the grid
        containers::StreamingGridCollectorStack<10000> collector;
