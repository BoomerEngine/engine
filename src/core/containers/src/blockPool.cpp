...
BlockAllocationResult BlockPool::allocateBlock(uint32_t size, uint32_t alignment, MemoryBlock& outBlock)
{
    ...
    auto splitStart = Align(freeBlock->offset, range_cast<uint64_t>(alignment));
...
void BlockPool::requestBlocksToMove(uint64_t maxBytesToMove, uint32_t maxBlocksToMove, Array<BlockToDefrag>& outBlocks)
{
    // todo
}
