...
namespace text
{
    ...
    struct ParameterToken : public NoCopy
    {
        template< typename T >
    ...
    /// NOTE: do not cache beyond the lifetime of ParameterFile
    class ParameterNode : public NoCopy
    {
    public:
        ...
        INLINE T tokenRead(uint32_t index, const T& defaultValue = T()) const
        {
            ...
            auto cur  = token(index);
            if (cur)
                StringView(cur->value).match(ret);
    ...
    class IParameterFileParserErrorReporter : public NoCopy
    {
    public:
