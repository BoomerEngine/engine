...
class CORE_RESOURCE_API ResourcePath
{
public:
    INLINE ResourcePath() {};
...
extern CORE_RESOURCE_API bool ApplyRelativePath(StringView contextPath, StringView relativePath, StringBuf& outPath);
...
extern CORE_RESOURCE_API bool ScanRelativePaths(StringView contextPath, StringView pathParts, uint32_t scanDepth, StringBuf& outPath, const std::function<bool(StringView)>& testFunc);

// given a base path and a second path (both absolute) build a relative path that takes you from base to second
extern CORE_RESOURCE_API bool BuildRelativePath(StringView basePath, StringView targetPath, StringBuf& outRelativePath);
...
extern CORE_RESOURCE_API bool IsValidPathChar(uint32_t ch);

// given a wanted name of something return file name that is safe to use in the file system
extern CORE_RESOURCE_API bool MakeSafeFileName(StringView text, StringBuf& outFixedFileName);
...
extern CORE_RESOURCE_API bool ValidateFileName(StringView fileName);

// check if given file name is a valid depot file name with extension
extern CORE_RESOURCE_API bool ValidateFileNameWithExtension(StringView fileName);

// check if given depot path is valid
extern CORE_RESOURCE_API bool ValidateDepotPath(StringView path, DepotPathClass pathClass = DepotPathClass::AbsoluteFilePath);

// classify path based on content
extern CORE_RESOURCE_API DepotPathClass ClassifyDepotPath(StringView path);
