...
class LocalVariableMapper : public NoCopy
{
public:
    ...
    uint16_t mapLocal(const StubOpcode* localOpcode)
    {
        ...
        if (!m_localVarRemapping.find(portableIndex, machineIndex))
        {
            ...
            localEntry.offset = Align(m_localDataStorageSize, localEntry.type->alignment());
...
private:
    HashMap<uint16_t, uint16_t> m_localVarRemapping;
...
public:
    ...
    INLINE void writeValue(const T& data)
    {
        ...
        //*(T*)OffsetPtr(m_stream.data(), offset) = data;
    }
