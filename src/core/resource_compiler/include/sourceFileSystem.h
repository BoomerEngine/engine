...
class CORE_RESOURCE_COMPILER_API ISourceAssetFileSystem : public IReferencable
{
public:
    ...
    virtual Buffer loadFileContent(StringView fileSystemPath, TimeStamp& outTimestamp, SourceAssetFingerprint& outFingerprint) const = 0;
    ...
    virtual CAN_YIELD SourceAssetStatus checkFileStatus(StringView fileSystemPath, const TimeStamp& lastKnownTimestamp, const SourceAssetFingerprint& lastKnownFingerprint, IProgressTracker* progress) const = 0;
...
class CORE_RESOURCE_COMPILER_API ISourceAssetFileSystemFactory : public IReferencable
{
