...
struct CORE_RESOURCE_COMPILER_API SourceAssetFingerprintTimestampEntry
{
    RTTI_DECLARE_NONVIRTUAL_CLASS(SourceAssetFingerprintTimestampEntry);

    uint64_t timestamp = 0;
    SourceAssetFingerprint fingerprint;
};

struct CORE_RESOURCE_COMPILER_API SourceAssetFingerprintCacheEntry
{
    RTTI_DECLARE_NONVIRTUAL_CLASS(SourceAssetFingerprintCacheEntry);

    StringBuf absolutePathUTF8; // conformed - lower case
    Array<SourceAssetFingerprintTimestampEntry> timestampEntries; // sorted by time
};
...
public:
    ...
    bool findEntry(StringView path, TimeStamp timestamp, SourceAssetFingerprint& outFingerprint);

    /// store entry for given path and timestamp
    void storeEntry(StringView path, TimeStamp timestamp, const SourceAssetFingerprint& fingerprint);

private:
    HashMap<StringBuf, uint32_t> m_entriesMap;
    Array<SourceAssetFingerprintCacheEntry> m_entries;
