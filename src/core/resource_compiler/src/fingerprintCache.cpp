...
RTTI_BEGIN_TYPE_CLASS(SourceAssetFingerprintTimestampEntry);
    RTTI_PROPERTY(timestamp);
    RTTI_PROPERTY(fingerprint);
RTTI_END_TYPE();

RTTI_BEGIN_TYPE_CLASS(SourceAssetFingerprintCacheEntry);
...
bool ImportFingerprintCache::findEntry(StringView path, TimeStamp timestamp, SourceAssetFingerprint& outFingerprint)
{
...
void ImportFingerprintCache::storeEntry(StringView path, TimeStamp timestamp, const SourceAssetFingerprint& fingerprint)
{
    DEBUG_CHECK_EX(path, "Invalid path");
    DEBUG_CHECK_EX(!timestamp.empty(), "Invalid timestamp");
