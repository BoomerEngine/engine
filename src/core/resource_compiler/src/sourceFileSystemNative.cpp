...
SourceAssetStatus SourceAssetFileSystem_LocalComputer::checkFileStatus(StringView fileSystemPath, const TimeStamp& lastKnownTimestamp, const SourceAssetFingerprint& lastKnownFingerprint, IProgressTracker* progress) const
{
    StringBuf path;
    if (!convertToAbsolutePath(fileSystemPath, path))
    ...
    const auto ret = GetService<SourceAssetFingerprintService>()->calculateFingerprint(path, true, progress, currentFingerprint);
...
Buffer SourceAssetFileSystem_LocalComputer::loadFileContent(StringView fileSystemPath, TimeStamp& outTimestamp, SourceAssetFingerprint& outFingerprint) const
{
    StringBuf path;
    if (!convertToAbsolutePath(fileSystemPath, path))
        return Buffer();
