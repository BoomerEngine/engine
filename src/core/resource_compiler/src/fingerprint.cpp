...
RTTI_BEGIN_CUSTOM_TYPE(SourceAssetFingerprint);
    RTTI_TYPE_TRAIT().zeroInitializationValid().noDestructor().fastCopyCompare();
    RTTI_BIND_NATIVE_BINARY_SERIALIZATION(SourceAssetFingerprint);
    RTTI_BIND_NATIVE_PRINT(SourceAssetFingerprint);
RTTI_END_TYPE();
...
void SourceAssetFingerprint::print(IFormatStream& f) const
{
    if (m_value != 0)
    {
...
void SourceAssetFingerprint::writeBinary(SerializationWriter& stream) const
{
    stream.writeTypedData<uint8_t>(0);
    stream.writeTypedData(m_value);
}

void SourceAssetFingerprint::readBinary(SerializationReader& stream)
{
    uint8_t type = 0;
...
uint32_t SourceAssetFingerprint::CalcHash(const SourceAssetFingerprint& entry)
{
...
FingerpintCalculationStatus CalculateMemoryFingerprint(const void* data, uint64_t size, IProgressTracker* progress, SourceAssetFingerprint& outFingerpint)
{
    ...
    outFingerpint = SourceAssetFingerprint(calc.crc());
    return FingerpintCalculationStatus::OK;
}

FingerpintCalculationStatus CalculateFileFingerprint(IReadFileHandle* file, IProgressTracker* progress, SourceAssetFingerprint& outFingerpint)
{
...
CAN_YIELD extern CORE_RESOURCE_COMPILER_API FingerpintCalculationStatus CalculateFileFingerprint(IAsyncFileHandle* file, bool childFiber, IProgressTracker* progress, SourceAssetFingerprint& outFingerpint)
{
    ...
    if (state.status == FingerpintCalculationStatus::OK)
        outFingerpint = SourceAssetFingerprint(state.calc.crc());
