...
void SymbolName::set(const char* txt)
{
    strcpy_s(m_txt, txt);
}

void SymbolName::append(const char* txt)
{
    strcat_s(m_txt, txt);
}
...
void Callstack::push(uint64_t address)
{
...
void Callstack::print(IFormatStream& f, const char* lineSearator) const
{
	if (empty())
	{
	...
	{
		debug::SymbolName name, file;
		debug::TranslateSymbolName(m_frames[i], name, file);
...
uint64_t Callstack::uniqueHash() const
{
    TempCRC crc;
    crc.append4(m_size);
    for (uint32_t i = 0; i < m_size; ++i)
...
public:
    ...
    uint32_t capture(uint32_t skipInitialFunctions)
    {
        ...
        if (GrabCallstack(skipInitialFunctions + 2, nullptr, callstack))
        {
            ...
            m_capturedStacks.push_back(new Callstack(callstack));
    ...
    const Callstack& resolve(uint32_t id)
    {
        {
            std::lock_guard<std::mutex> guard(m_lock);
        ...
        static Callstack theEmptyStack;
        return theEmptyStack;
    }

    std::mutex m_lock;
    std::vector<Callstack*> m_capturedStacks;
...
const Callstack& ResolveCapturedCallstack(uint32_t id)
{
