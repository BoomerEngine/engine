...
BEGIN_BOOMER_NAMESPACE_EX(profiler)
...
/// block information
struct CORE_SYSTEM_API BlockInfo
{
public:
...
END_BOOMER_NAMESPACE_EX(profiler)
...
//--
    ...
    #define PC_SCOPE(name, lvl, color) static thread_local profiler::BlockInfo __profiling_block_##name(#name, PC_UNIQUE_LINE_ID, __FILE__, __LINE__, lvl, color); \
            profiler::Block __profiling_##name(__profiling_block_##name)

    // profiling macros
    #define PC_SCOPE_LVL0(name, ...) PC_SCOPE(name, 0, profiler::ExtractColor(__VA_ARGS__))
    #define PC_SCOPE_LVL1(name, ...) PC_SCOPE(name, 1, profiler::ExtractColor(__VA_ARGS__))
    #define PC_SCOPE_LVL2(name, ...) PC_SCOPE(name, 2, profiler::ExtractColor(__VA_ARGS__))
    #define PC_SCOPE_LVL3(name, ...) PC_SCOPE(name, 3, profiler::ExtractColor(__VA_ARGS__))
    #define PC_EVENT(name)
    #define PC_DATA(name, val)
