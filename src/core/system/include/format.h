...
/// formatting stream
class CORE_SYSTEM_API IFormatStream : public NoCopy
{
public:
...
{
public:
    FoldableFormatBlock(IFormatStream& f, const char* txt = "")
        : m_log(f)
    {
        m_log << "[#" << txt << "\n";
    }
...
template< typename T >
HexWrap<T> Hex(const T& data)
{
    return HexWrap<T>(data);
}

//--

template< typename T >
PrecWrap<T> Prec(const T& data, int digits)
{
    return PrecWrap<T>(data, digits);
}

//--

template< typename T >
MemSizeWrap<T> MemSize(const T& data)
{
    return MemSizeWrap<T>(data);
}

//--

template< typename T >
TimeWrap<T> TimeInterval(const T& data)
{
    return TimeWrap<T>(data);
}

//--

template< typename T >
IndirectWrap<T> IndirectPrint(const T* data)
{
    return IndirectWrap<T>(data);
}
