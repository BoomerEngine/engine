...
BEGIN_BOOMER_NAMESPACE_EX(debug)

//---

class CORE_SYSTEM_API SymbolName
{
public:
...
class CORE_SYSTEM_API Callstack
{
public:
...
extern CORE_SYSTEM_API void Break();

// Grab current callstack, returns number of function grabbed (0 if error)
extern CORE_SYSTEM_API bool GrabCallstack(uint32_t skipInitialFunctions, const void* exptr, Callstack& outCallstack);

// Get human readable function name
extern CORE_SYSTEM_API bool TranslateSymbolName(uint64_t functionAddress, SymbolName& outSymbolName, SymbolName& outFileName);

// Check if debugger is present (windows mostly)
extern CORE_SYSTEM_API bool DebuggerPresent();

//--

// Get indexed callstack (as an easy to save number)
extern CORE_SYSTEM_API uint32_t CaptureCallstack(uint32_t skipInitialFunctions);

// Resolve callstack by ID, can be used to print it
extern CORE_SYSTEM_API const Callstack& ResolveCapturedCallstack(uint32_t id);

//--

END_BOOMER_NAMESPACE_EX(debug)
