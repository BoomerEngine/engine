...
class CORE_SYSTEM_API ILogSink : public NoCopy
{
public:
    virtual ~ILogSink();
...
class CORE_SYSTEM_API LocalLogSink : public ILogSink
{
public:
    LocalLogSink();
...
class CORE_SYSTEM_API GlobalLogSink : public ILogSink
{
public:
    GlobalLogSink();
...
//! logging "system"
class CORE_SYSTEM_API Log : public NoCopy
{
public:
...
class CORE_SYSTEM_API IErrorHandler
{
public:
...
#define TRACE_STREAM( lvl ) ::boomer::logging::Log::Stream(lvl, TRACE_MAKE_TEXT2(PROJECT_NAME), __FILE__, __LINE__)
#define TRACE_STREAM_SPAM() TRACE_STREAM(::boomer::logging::OutputLevel::Spam)
#define TRACE_STREAM_INFO() TRACE_STREAM(::boomer::logging::OutputLevel::Info)
#define TRACE_STREAM_WARNING() TRACE_STREAM(::boomer::logging::OutputLevel::Warning)
#define TRACE_STREAM_ERROR() TRACE_STREAM(::boomer::logging::OutputLevel::Error)
...
#define FATAL_ERROR(x) ::boomer::logging::IErrorHandler::FatalError(__FILE__, __LINE__, x)
...
#define DEBUG_CHECK( expr )                                                                                              \
    {                                                                                                               \
        ...
        if (isEnabled && !(expr))                                                                                   \
            ::boomer::logging::IErrorHandler::Assert(false, __FILE__, __LINE__, #expr, NULL, &isEnabled);           \
    }
...
#define DEBUG_CHECK_EX( expr, msg )                                                                                      \
    {                                                                                                               \
        static bool isEnabled = true;                                                                               \
        if (isEnabled && !(expr))                                                                                   \
            ::boomer::logging::IErrorHandler::Assert(false,  __FILE__, __LINE__, #expr, msg, &isEnabled);           \
    }
...
#define DEBUG_CHECK_RETURN( expr )                                                                                  \
    {                                                                                                               \
        ...
        if (!(expr)) {                                                                                              \
            if (isEnabled)                                                                                          \
                ::boomer::logging::IErrorHandler::Assert(false,  __FILE__, __LINE__, #expr, "Required data is missing", &isEnabled);           \
...
#define ASSERT( expr )                                                                                        \
    {                                                                                                               \
        if (!(expr))                                                                                                \
            ::boomer::logging::IErrorHandler::Assert(true, __FILE__, __LINE__, #expr, nullptr, nullptr);            \
    }

#define ASSERT_EX( expr, msg )                                                                                \
    {                                                                                                               \
        if (!(expr))                                                                                                \
            ::boomer::logging::IErrorHandler::Assert(true,  __FILE__, __LINE__, #expr, msg, nullptr);               \
    }

#define FATAL_CHECK(expr) \
    {                                                                                                               \
        if (!(expr))                                                                                                \
            ::boomer::logging::IErrorHandler::Assert(true,  __FILE__, __LINE__, #expr, NULL, nullptr);               \
    }
