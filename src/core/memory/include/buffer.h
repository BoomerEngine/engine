...
extern CORE_MEMORY_API void* Compress(CompressionType ct, const void* uncompressedDataPtr, uint64_t uncompressedSize, uint64_t& outCompressedSize, PoolTag pool);

// compress a memory block to buffer
extern CORE_MEMORY_API Buffer Compress(CompressionType ct, const void* uncompressedDataPtr, uint64_t uncompressedSize, PoolTag pool);

// compress a memory block to buffer
extern CORE_MEMORY_API Buffer Compress(CompressionType ct, const Buffer& uncompressedData, PoolTag pool);

// compress a memory block in-place, output buffer must be at least as big as the input data
extern CORE_MEMORY_API bool Compress(CompressionType ct, const void* uncompressedDataPtr, uint64_t uncompressedSize, void* compressedDataPtr, uint64_t compressedDataMaxSize, uint64_t& outCompressedRealSize);

// decompress a memory block into a buffer
extern CORE_MEMORY_API bool Decompress(CompressionType ct, const void* compressedDataPtr, uint64_t compressedSize, void* decompressedDataPtr, uint64_t decompressedSize);

// decompress a memory block into a buffer
extern CORE_MEMORY_API Buffer Decompress(CompressionType ct, const void* compressedDataPtr, uint64_t compressedSize, uint64_t decompressedSize, PoolTag pool);
