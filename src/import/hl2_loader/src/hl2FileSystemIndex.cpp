...
#pragma pack()
    ...
    public:
        ...
        uint32_t mapArchive(const base::StringBuf& dirPath, int archiveIndex)
        {
            ...
            auto& entry = m_data.m_packages.emplaceBack();
            entry.m_name = mapString(base::TempString("{}", packageName));
            entry.m_numFiles = 0;
            entry.m_timeStamp = timeStamp.value();
    ...
    int FileSystemIndex::findDirectoryEntry(base::StringView path) const
    {
        if (m_directories.empty())
            return -1;
    ...
    int FileSystemIndex::findFileEntry(base::StringView path) const
    {
        if (m_files.empty())
            return -1;
