...
namespace application
{
    ...
    bool ParseGameStartInfo(const base::app::CommandLine& commandline, game::GameStartInfo& outData)
    {
        bool validInfo = false;

        const auto startuScenePath = commandline.singleValue("cookedScenePath");
        if (ValidateDepotPath(startuScenePath, base::DepotPathClass::AbsoluteFilePath))
        {
            outData.scenePath = base::StringBuf(startuScenePath);
    ...
    bool LauncherApp::createGame(const base::app::CommandLine& commandline)
    {
        ...
        game::GameStartInfo initData;
        if (ParseGameStartInfo(commandline, initData))
        {
            game->scheduleNewWorldTransition(initData);
        }
        else
        {
