...
namespace application
{
    ...
    public:
        ...
        {
            if (level == base::logging::OutputLevel::Warning)
                m_numWarnings += 1;

            if (level == base::logging::OutputLevel::Error)
            {
                m_numErrors += 1;
...

            {
                ...
                if (!m_allErrors.empty())
                {
                    ...
                    for (const auto* msg : m_allErrors)
                        base::logging::Log::Print(base::logging::OutputLevel::Error, msg->file.c_str(), msg->line, "", msg->context.c_str(), msg->text.c_str());
                }
            }
