    ...
    namespace compiler
    {
        ...
        public:
            virtual const INativeFunction* mutateFunction(TypeLibrary& typeLibrary, uint32_t numArgs, DataType* argTypes, const base::parser::Location& loc, base::parser::IErrorReporter& err) const override final
            {
                if (numArgs == 2)
                {
                    ...
                    err.reportError(loc, base::TempString("Unable to determine the nature of multiplication with types '{}' and '{}'", leftType, rightType));
                    return nullptr;
                }
                else
                {
        ...
        public:
            virtual DataType determineReturnType(TypeLibrary& typeLibrary, uint32_t numArgs, DataType* argTypes, const base::parser::Location& loc, base::parser::IErrorReporter& err) const override final
            {
                ...
                {
                    err.reportError(loc, base::TempString("Matrix/Vector multiplication works only with numerical types, type '{}' is not compatible", argTypes[0]));
                    return DataType();
                }
                else  if (!argTypes[1].isNumericalMatrixLikeOperand())
                {
                    err.reportError(loc, base::TempString("Matrix/Vector multiplication works only with numerical types, type '{}' is not compatible", argTypes[1]));
                    return DataType();
                }
                ...
                if (leftRows == 1 || leftCols == 1 || rightRows == 1 || leftRows == 1)
                    err.reportWarning(loc, base::TempString("Matrix multiplication function called with semi-vectors: {}x{} and {}x{}", leftRows, leftCols, rightRows, rightCols));

                // well, check the math rules
                if (leftCols != rightRows)
                {
                    err.reportError(loc, base::TempString("Matrix {}x{} cannot be multiplied with matrix {}x{} (blame math, not me)", leftRows, leftCols, rightRows, rightCols));
                    return DataType();
                }
        ...
        public:
            virtual DataType determineReturnType(TypeLibrary& typeLibrary, uint32_t numArgs, DataType* argTypes, const base::parser::Location& loc, base::parser::IErrorReporter& err) const override final
            {
                ...
                {
                    err.reportError(loc, base::TempString("Vector with {} components cannot be multiplied with matrix {}x{} (blame math, not me)", vectorSize, rightRows, rightCols));
                    return DataType();
                }
        ...
        public:
            virtual DataType determineReturnType(TypeLibrary& typeLibrary, uint32_t numArgs, DataType* argTypes, const base::parser::Location& loc, base::parser::IErrorReporter& err) const override final
            {
                ...
                {
                    err.reportError(loc, base::TempString("Matrix {}x{} cannot be multiplied with vector with {} components (blame math, not me)", leftRows, leftCols, vectorSize));
                    return DataType();
                }
