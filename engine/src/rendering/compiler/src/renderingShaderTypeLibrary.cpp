...
namespace rendering
{
    namespace compiler
    {
        ...
        CompositeType::CompositeType(base::StringID name, CompositePackingRules packingRules, CompositeTypeHint hint)
            : m_name(name)
            , m_hint(hint)
            , m_typeHash(0)
            , m_scalarCount(0)
        ...
        DataType CompositeType::memberType(const base::StringID name) const
        {
        ...
        int CompositeType::memberIndex(const base::StringID name) const
        {
        ...
        base::StringID CompositeType::memberName(uint32_t index) const
        {
            if (index < m_members.size())
        ...
        void CompositeType::addMember(const base::parser::Location& loc, base::StringID memberName, DataType memberType, AttributeList&& attributes, const base::Array<base::parser::Token*>& initializationTokens)
        {
        ...
        bool CompositeType::packLayoutVertex(const CompositeType::Member& prop, uint32_t& inOutPackingOffset, MemberLayoutInfo& outLayout, base::parser::IErrorReporter& err) const
        {
            ...
            {
                outLayout.linearOffset = base::Align(inOutPackingOffset, alignment);
            }
            else
            {
        ...
        bool CompositeType::packLayoutStd140(const CompositeType::Member& prop, uint32_t& inOutPackingOffset, MemberLayoutInfo& outLayout, base::parser::IErrorReporter& err) const
        {
            ...
            {
                err.reportWarning(prop.location, base::TempString("Ignored packing layout for structure '{}' member '{}' as it is not supported here", m_name, prop.name));
            }
            if (prop.attributes.has("format"_id))
            {
                err.reportWarning(prop.location, base::TempString("Ignored packing format for structure '{}' member '{}' as it is not supported here", m_name, prop.name));
            }
            ...
            else
            {
                ...
                alignment = std::min<uint32_t>(16, base::NextPow2(sizeToPack));
            }
            ...
            if (prop.type.isArray())
            {
                ...
                sizeToPack = base::Align<uint32_t>(sizeToPack, minimalAlignment) * numElements;
            }
            ...
            if (!prop.attributes.has("offset"_id))
            {
                outLayout.linearOffset = range_cast<uint16_t>(base::Align(inOutPackingOffset, alignment));
            }
            else
            {
                int offset = prop.attributes.valueAsIntOrDefault("offset"_id, -1);
            ...
            if (outLayout.linearArrayCount > 0)
                outLayout.linearArrayStride = base::Align<uint32_t>(outLayout.linearSize, minimalAlignment);

            // packed
            return true;
        }

        bool CompositeType::packLayoutStd430(const CompositeType::Member& prop, uint32_t& inOutPackingOffset, MemberLayoutInfo& outLayout, base::parser::IErrorReporter& err) const
        {
            // TODO!
            return packLayoutStd140(prop, inOutPackingOffset, outLayout, err);
        }

        bool CompositeType::computeMemoryLayout(bool& outNeedsMorePasses, bool& outUpdated, base::parser::IErrorReporter& err)
        {
            if (m_layoutComputed)
                return true;
            ...
            for (auto& member : m_members)
            {
                ...
                {
                    err.reportError(member.location, base::TempString("Structure '{}' member '{}' is using a resource type that is only allowed in descriptor layouts not in general structures",
                                                                      name(), member.name));
                    return false;
                }
                ...
                {
                    err.reportError(member.location, base::TempString("Structure '{}' is using unknown packing mode", name()));
                    return false;
                }
            }
        ...
        void CompositeType::print(base::IFormatStream& f) const
        {
        ...
        ResourceTable::ResourceTable(base::StringID name, const AttributeList& attributes)
            : m_name(name)
            , m_attributes(attributes)
        {}

        int ResourceTable::memberIndex(const base::StringID name) const
        {
            for (uint32_t i=0; i<m_members.size(); ++i)
        ...
        base::StringID ResourceTable::memberName(uint32_t index) const
        {
            if (index < m_members.size())
        ...
        const CompositeType* TypeLibrary::findCompositeType(base::StringID name) const
        {
            const CompositeType* ret = nullptr;
            m_compositeTypeMap.find(name, ret);
            return ret;
        ...
        const ResourceTable* TypeLibrary::findResourceTable(base::StringID name) const
        {
        ...
        {
            // member names
            const base::StringID memberNames[4] =
            {
                "x"_id,
                "y"_id,
                "z"_id,
            ...
            static CompositeType* CreateVectorType(base::mem::LinearAllocator& mem, const char* core, DataType memberType, uint32_t numComponents, CompositeTypeHint typeHint)
            {
                base::StringBuf name = base::TempString("{}{}", core, numComponents);
                ...
                if (typeHint == CompositeTypeHint::MatrixType)
                {
                    ...
                    if (numVectorComponents == numComponents)
                        name = base::TempString("mat{}", numComponents);
                }

                auto t = mem.createNoCleanup<CompositeType>(base::StringID(name.c_str()), CompositePackingRules::Std140, typeHint);

                for (uint32_t i = 0; i < numComponents; ++i)
                    t->addMember(base::parser::Location(), memberNames[i], memberType, AttributeList());
        ...
        bool TypeLibrary::calculateCompositeLayouts(base::parser::IErrorReporter& err)
        {
            while (1)
            {
        ...
        static uint64_t CalcResourceTypeHash(base::StringID typeName, const AttributeList& attributes)
        {
            base::CRC64 crc;
            crc << typeName;
        ...
        static bool MutateTextureName(base::StringID& typeName, AttributeList& attributes)
        {
            bool valid = true;
