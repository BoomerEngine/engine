...
namespace rendering
{
    namespace compiler
    {
        ...
        static DataType DetermineCompareType(TypeLibrary& typeLibrary, uint32_t numArgs, DataType* argTypes, const base::parser::Location& loc, base::parser::IErrorReporter& err, bool equalityTest)
        {
            ...
            {
                err.reportError(loc, base::TempString("Type '{}' is not valid type for compare operation", argTypes[0]));
                return DataType();
            }
            else if (baseType1 == BaseType::Invalid)
            {
                err.reportError(loc, base::TempString("Type '{}' is not valid type for compare operation", argTypes[1]));
                return DataType();
            }
            ...
            if (baseType0 != baseType1)
            {
                ...
                {
                    err.reportError(loc, base::TempString("Comparison functions can only operate on types that are the same type '{}' is not the same as '{}'", argTypes[0], argTypes[1]));
                    return DataType();
                }
            }
        ...
        public:
            virtual DataType determineReturnType(TypeLibrary& typeLibrary, uint32_t numArgs, DataType* argTypes, const base::parser::Location& loc, base::parser::IErrorReporter& err) const override final
            {
                ...
                {
                    err.reportError(loc, base::TempString("Type '{}' is not valid type for this function, this function requires boolean vector", argTypes[0]));
                    return DataType();
                }
