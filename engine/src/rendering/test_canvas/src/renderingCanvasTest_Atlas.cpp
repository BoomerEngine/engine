    ...
    namespace test
    {
        ...
        public:
            ...
            GeometryPtr generateDirtyPatch(base::MTRandState& rnd, uint32_t width, uint32_t height, uint32_t circles, uint32_t maxSize, const Patch& styles) const
            {
                ...
                {
                    uint32_t size = 1 + rnd.next() % maxSize;
                    int minX = size + 1;
                    int minY = size + 1;
                    ...
                    {
                        float x = base::Lerp((float)minX, (float)minY, rnd.unit());
                        float y = base::Lerp((float)minY, (float)minY, rnd.unit());
        ...
        private:
            ...
            void generateLevel()
            {
                ...
                // generate islands
                {
                    ...
                    {
                        uint32_t size = 10 + (rnd.next() % 20);

                        uint32_t x = rnd.next() % m_width;
                        uint32_t y = rnd.next() % m_height;
