...
//#define ALWAYS_ALLOC_TO_EXTERNAL_MEMORY
        ...
        bool CommandWriter::opAcquireOutput(ObjectID output, base::Rect& outViewportRect, ImageView& outColorView, ImageView* outDepthView /*= nullptr*/)
        {
            DEBUG_CHECK_EX(output, "Invalid output");
            if (output)
            {
                // query output state
                DriverOutputFrameInfo outputSetup;
                if (base::GetService<DeviceService>()->device()->prepareOutputFrame(output, outputSetup))
                {
                    DEBUG_CHECK_EX(outputSetup.colorFormat != ImageFormat::UNKNOWN, "Output's color format should be always valid");
                    DEBUG_CHECK_EX(!outDepthView || outputSetup.depthFormat != ImageFormat::UNKNOWN, "Output's depth format is unknown yet a depth view was requested");

                    // prevent from rendering if RT formats are invalid
                    if (outputSetup.colorFormat == ImageFormat::UNKNOWN || (outDepthView && outputSetup.depthFormat == ImageFormat::UNKNOWN))
                        return false;

                    auto op = allocCommand<OpAcquireOutput>();
                    op->output = output;
                    op->viewport = outputSetup.viewport;

                    // viewport
                    outViewportRect = outputSetup.viewport;

                    // do we flip the screen ?
                    auto flipFlag = (ImageViewFlag)0;
                    if (outputSetup.flippedY)
                        flipFlag = ImageViewFlag::FlippedY;

                    // allocate color
                    auto colorId = ObjectID::AllocTransientID();
                    ImageViewFlags colorFlags = { ImageViewFlag::RenderTarget, ImageViewFlag::SwapChain, flipFlag };
                    outColorView = ImageView(colorId, ImageViewKey(outputSetup.colorFormat, ImageViewType::View2D, 0, 1, 0, 1), outputSetup.samples, outputSetup.width, outputSetup.height, 1, colorFlags, ObjectID::DefaultPointSampler());
                    op->colorView = outColorView;

                    // allocate depth
                    if (outDepthView)
                    {
                        auto depthId = ObjectID::AllocTransientID();
                        ImageViewFlags depthFlags = { ImageViewFlag::RenderTarget, ImageViewFlag::SwapChain, ImageViewFlag::Depth, flipFlag };
                        *outDepthView = ImageView(depthId, ImageViewKey(outputSetup.depthFormat, ImageViewType::View2D, 0, 1, 0, 1), outputSetup.samples, outputSetup.width, outputSetup.height, 1, depthFlags, ObjectID::DefaultPointSampler());
                        op->depthView = *outDepthView;
                    }
        ...
        void CommandWriter::opSwapOutput(ObjectID output, bool doSwap /*= true*/)
        {
            DEBUG_CHECK_EX(output, "Invalid output");
            if (output)
            {
...
#define DEBUG_CHECK_PASS_ONLY() \
        ...
        void CommandWriter::opDrawInstanced(const ShaderLibrary* shader, uint32_t firstVertex, uint32_t vertexCount, uint16_t firstInstance, uint16_t numInstances)
        {
            ...
            DEBUG_CHECK_EX(shader, "Trying to render with invalid shader");
            DEBUG_CHECK_EX(vertexCount > 0, "Invalid vertex count");
            DEBUG_CHECK_EX(numInstances > 0, "Invalid instance count");

            if (vertexCount && numInstances)
            {
                PipelineIndex shaderIndex;
                rendering::ObjectID shaderObject;
                if (shader && shader->resolve(shaderObject, shaderIndex, nullptr))
                {
        ...
        void CommandWriter::opDrawIndexedInstanced(const ShaderLibrary* shader, uint32_t firstVertex, uint32_t firstIndex, uint32_t indexCount, uint16_t firstInstance, uint16_t numInstances)
        {
            ...
            DEBUG_CHECK_EX(shader, "Trying to render with invalid shader");
            DEBUG_CHECK_EX(indexCount > 0, "Invalid index count");
            DEBUG_CHECK_EX(numInstances > 0, "Invalid instance count");

            if (indexCount && numInstances)
            {
                PipelineIndex shaderIndex;
                rendering::ObjectID shaderObject;
                if (shader && shader->resolve(shaderObject, shaderIndex, nullptr))
        ...
        {
            DEBUG_CHECK_EX(shader, "Trying to disatch compute with invalid shader");
            DEBUG_CHECK_EX(countX > 0 && countY > 0 && countZ > 0, "Invalid dispatch counts");
        ...
        void CommandWriter::opUpdateDynamicBuffer(const BufferView& dynamicBuffer, uint32_t dataOffset, uint32_t dataSize, const void* dataPtr)
        {
            ...
            DEBUG_CHECK_EX(dataSize && dataPtr, "Empty data for update");

            if (dynamicBuffer.id() && dynamicBuffer.dynamic() && dataSize && dataPtr)
            {
                ...
                if (dataOffset < dynamicBuffer.size())
                {
                    ...
                    memcpy(op->dataBlockPtr, dataPtr, dataSize);
        ...
        void CommandWriter::linkTransientBufferOp(OpAllocTransientBuffer* op)
        {
            if (m_writeBuffer->m_gatheredState.triansienBufferAllocTail)
                m_writeBuffer->m_gatheredState.triansienBufferAllocTail->next = op;
            else
                m_writeBuffer->m_gatheredState.triansienBufferAllocHead = op;
            m_writeBuffer->m_gatheredState.triansienBufferAllocTail = op;
        }

        void CommandWriter::opAllocTransientBuffer(const TransientBufferView& buf)
        {
            DEBUG_CHECK_EX(buf.transient(), "Only transient buffers can be allocated");

            if (buf.transient() && buf.id())
            {
                auto op = allocCommand<OpAllocTransientBuffer>();
                op->buffer = buf;
                op->initializationData = nullptr;
                op->initializationDataSize = 0;
                op->next = nullptr;

                linkTransientBufferOp(op);
            }
        }

        void CommandWriter::opAllocTransientBufferWithData(const TransientBufferView& buf, const void* initialData, uint32_t initialDataSize)
        {
            auto ptr  = opAllocTransientBufferWithUninitializedData(buf, initialDataSize);
            if (initialData && ptr)
            {
                initialDataSize = std::min<uint32_t>(initialDataSize, buf.size()); // don't risk buffer overwrites...
                memcpy(ptr, initialData, initialDataSize);
            }
        }

        void* CommandWriter::opAllocTransientBufferWithUninitializedData(const TransientBufferView& buf, uint32_t initialDataSize)
        {
            DEBUG_CHECK_EX(buf.transient(), "Only transient buffers can be allocated");
            DEBUG_CHECK_EX(initialDataSize <= buf.size(), "More initial data provided than buffer has space for");

            if (buf.transient() && buf.id())
            {
                initialDataSize = std::min<uint32_t>(initialDataSize, buf.size());
                auto maxLocalPayload = std::max<uint32_t>(1024, m_writeEndPtr - m_writePtr);

                DEBUG_CHECK_EX(!buf.stride() || (0 == (initialDataSize % buf.stride())), "Initial data for structued buffer must be multiple of stride");

                OpAllocTransientBuffer* op;
                if (initialDataSize < maxLocalPayload)
                {
                    op = allocCommand<OpAllocTransientBuffer>(initialDataSize);
                    op->initializationData = op->payload();
                }
                else
                {
                    op = allocCommand<OpAllocTransientBuffer>();
                    op->initializationData = m_writeBuffer->m_pages->allocateOustandingBlock(initialDataSize, 16);
                }

                op->buffer = buf;
                op->initializationDataSize = initialDataSize;
                op->next = nullptr;

                linkTransientBufferOp(op);
                return op->initializationData;
            }
            else
            {
                return nullptr;
