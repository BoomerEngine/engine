...
namespace rendering
{
    ...
    namespace command
    {
        ...
        public:
            ...
            bool opAcquireOutput(ObjectID output, base::Rect& outViewportRect, ImageView& outColorView, ImageView* outDepthView = nullptr);

            // swap the previously acquired back buffer
            void opSwapOutput(ObjectID output, bool doSwap=true);
            ...
            void opAllocTransientBuffer(const TransientBufferView& buf);

            /// allocate a transient buffer with some initial data
            void opAllocTransientBufferWithData(const TransientBufferView& buf, const void* initialData, uint32_t initialDataSize);

            /// allocate a transient buffer with some initial data
            void* opAllocTransientBufferWithUninitializedData(const TransientBufferView& buf, uint32_t initialDataSize);

            /// allocate a transient buffer with some initial data
            template< typename T >
            INLINE T* opAllocTransientBufferWithUninitializedTypedData(const TransientBufferView& buf, uint32_t count)
            {
                return (T*)opAllocTransientBufferWithUninitializedData(buf, count * sizeof(T));
            }
        ...
        private:
            ...
            void linkTransientBufferOp(OpAllocTransientBuffer* op);
