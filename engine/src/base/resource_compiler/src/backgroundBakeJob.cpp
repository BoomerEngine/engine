...
namespace base
{
    namespace res
    {
        ...
        BakingJob::BakingJob(const ResourceKey& key, bool saveResults)
            : IBakingJob(key)
        ...
        ResourcePtr BakingJob::finalData() const
        {
            if (m_finished)
                return m_finalData;
            else
        ...
        void BakingJob::reportProgress(uint64_t currentCount, uint64_t totalCount, StringView<char> text)
        {
            ...
            m_lastStatusText = StringBuf(text);
        }

        //--

        BakingJobList::BakingJobList(depot::DepotStructure* depot, IResourceLoader* loader)
            : m_depot(depot)
            , m_loader(loader)
        {}
        ...
        void BakingJobList::handleBackgroundBakingStarted(const ResourceKey& key, float waitTime)
        {
            auto lock = CreateLock(m_listenerLock);
        ...
        void BakingJobList::handleBackgroundBakingFinished(const ResourceKey& key, bool valid, float timeTaken)
        {
            auto lock = CreateLock(m_listenerLock);
            for (auto* listener : m_listeners)
                if (listener)
        ...
        BakingJobPtr BakingJobList::issueJob(const ResourceKey& key, bool cancelExisting, bool saveResults)
        {
            if (!key)
                return false;
            ...
            auto job = CreateSharedPtr<BakingJob>(key, saveResults);
                ...
                if (job->m_finished.load())
                {
                    if (job->m_finalStatus.load() == BakingJobResult::Finished)
                    {
                        if (!job->m_canceled.load() && job->m_saveResults)
                        {
                            ...
                            {
                                const auto saveExtension = IResource::GetResourceExtensionForClass(job->m_finalData->cls());
                                DEBUG_CHECK_EX(saveExtension, "Baked file should have some kind of extension");
                                if (saveExtension)
                                {
                                    StringBuilder saveDepotPath;
