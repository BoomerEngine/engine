...
namespace base
{
    namespace res
    {
        ...
        bool BackgroundSaveThread::scheduleSave(const StringBuf& depotPath, const ResourcePtr& data)
        {
        ...
        void BackgroundSaveThread::processSavingThread()
        {
            ...
            while (!m_saveThreadRequestExit.exchange(0))
            {
                ...
                // save to memory
                ScopeTimer serializeTimer;
                auto data = SaveUncachedToBuffer(job->unsavedResource, job->mountPoint);
                if (!data)
                {
