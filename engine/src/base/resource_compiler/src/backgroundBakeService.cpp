...
namespace base
{
    namespace res
    {
        ...
        IBakingJob::IBakingJob(const ResourceKey& key)
            : m_key(key)
        {}
        ...
        void BackgroundBaker::handleBackgroundBakingRequested(const ResourceKey& key, bool forced)
        {
            auto lock = CreateLock(m_listenerLock);
        ...
        void BackgroundBaker::handleBackgroundBakingMissingBakedResource(const ResourceKey& key)
        {
            auto lock = CreateLock(m_listenerLock);
        ...
        {
            auto lock = CreateLock(m_jobsLock);
        ...
        BakingJobPtr BackgroundBaker::bake(const ResourceKey& key, bool cancelExisting /*= true*/, bool saveResult /*= true*/)
        {
            auto lock = CreateLock(m_jobsLock);
        ...
        void BackgroundBaker::onSyncUpdate()
        {
            ...
            {
                auto lock2 = CreateLock(m_newJobsLock);
        ...
        void BackgroundBaker::onResourceMissingBakedResource(const ResourceKey& key)
        {
            handleBackgroundBakingMissingBakedResource(key);
            scheduleResourceForBackgroundBaking(key);
        }

        void BackgroundBaker::scheduleResourceForBackgroundBaking(const ResourceKey& key)
        {
            auto lock = CreateLock(m_newJobsLock);
