    ...
    namespace parser
    {
        ...
        bool TextFilePreprocessor::processInclude(const Token* head, TokenList& line)
        {
            ...
            if (m_includeHandler.loadInclude(includeGlobal, includePath, head->location().contextName(), loadedContent, loadedContentContextPath))
            {
                ...
                {
                    m_errorHandler.reportError(head->location(), base::TempString("To many #include recursion levels when trying to include '{}'", includePath));
                    return false;
                }
            ...
            {
                m_errorHandler.reportError(head->location(), base::TempString("Unable to resolve include '{}'", includePath));
                return false;
            }
