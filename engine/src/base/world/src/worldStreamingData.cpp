...
namespace base
{
    namespace world
    {
        ...
        WorldSectorData::WorldSectorData(uint8_t level, int x, int y, const Box& worldBox, const Box& streamingBox, uint32_t entityCount, Buffer entityData)
            : m_gridLevel(level)
            , m_gridX(x)
            , m_gridY(y)
            , m_worldBox(worldBox)
        ...
        WorldSectorUnpackedDataPtr WorldSectorData::unpack(res::IResourceLoader* loader) const
        {
            return nullptr;
        }
        ...
        WorldSectorGridLevel::WorldSectorGridLevel(uint8_t level, uint32_t sectorPerSize, float sectorSize, Array<WorldSectorDataPtr>&& sectors)
            : m_gridLevel(level)
        ...
        RTTI_BEGIN_TYPE_CLASS(WorldSectors);
            RTTI_METADATA(res::ResourceExtensionMetadata).extension("v4sec");
            RTTI_METADATA(res::ResourceDescriptionMetadata).description("World Streaming");
        ...
        WorldSectors::WorldSectors(float worldSize, Array<WorldSectorGridLevelPtr>&& levels)
