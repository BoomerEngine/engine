...
namespace base
{
    namespace mesh
    {
        ...
        MeshStreamBuilder::MeshStreamBuilder(MeshTopologyType topology /*= MeshTopologyType::Triangles*/)
            : m_topology(topology)
        {
            memset(m_verticesRaw, 0, sizeof(m_verticesRaw));
        }

        MeshStreamBuilder::MeshStreamBuilder(const MeshStreamBuilder& source)
        ...
        void MeshStreamBuilder::reserveVertices(uint32_t maxVertices, MeshStreamMask newStreamsToAllocate)
        {
        ...
        /*void MeshStreamBuilder::reserveIndices(uint32_t maxIndices)
        {
        ...
        void MeshStreamBuilder::bind(const MeshModelChunk& chunk, MeshStreamMask meshStreamMask)
        {
            clear();
        ...
        /*void MeshStreamBuilder::makeIndexBufferResident()
        {
            if (!m_ownedIndices)
            {
        ...
        void MeshStreamBuilder::makeVertexStreamResident(MeshStreamType type)
        {
        ...
        void MeshStreamBuilder::migrate(MeshStreamBuilder&& source)
        {
            clear();

            memcpy(this, &source, sizeof(MeshStreamBuilder));
            memset(&source, 0, sizeof(MeshStreamBuilder));
        }
        ...
        /*void MeshStreamBuilder::generateAutomaticIndexBuffer(base::mesh::MeshTopologyType top, bool flipFaces)
        {
        ...
        void MeshStreamBuilder::removeIndexBufferAndDuplicateVertices()
        {
            if (m_indicesRaw)
            {
        ...
        void MeshStreamBuilder::expandQuadsToTriangles()
        {
        ...
        void MeshStreamBuilder::extract(MeshModelChunk& outChunk) const
        {
        ...
        static bool ComparePoint(const base::Vector3& a, const base::Vector3& b)
        {
            auto EPSILON = 0.0001f;
            return a.squareDistance(b) < (EPSILON * EPSILON);
        }

        static bool CompareVector(const base::Vector3& a, const base::Vector3& b)
        {
            auto EPSILON = 0.00001f;
            return a.squareDistance(b) < (EPSILON * EPSILON);
        }

        static bool CompareVector(const base::Vector2& a, const base::Vector2& b)
        {
            auto EPSILON = 0.00001f;
            return a.squareDistance(b) < (EPSILON * EPSILON);
        }
        ...
        LocalID MeshGeometryBuilder::addMaterial(const base::StringBuf &materialName)
        {
        ...
        void MeshGeometryBuilder::computeBoxUVProjects(const base::Vector3 &normal, float scale, const MeshBuilderVertex &a, const MeshBuilderVertex &b, const MeshBuilderVertex &c, base::Vector2 &outA, base::Vector2 &outB, base::Vector2 &outC)
        {
            // find best UV axis
            base::Vector3 axisU, axisV;
            if (normal.largestAxis() == 2)
            {
                axisU = base::Vector3::EX();
                axisV = base::Vector3::EY();
            }
            else if (normal.largestAxis() == 1)
            {
                axisU = base::Vector3::EX();
                axisV = base::Vector3::EZ();
            }
            else
            {
                axisU = base::Vector3::EY();
                axisV = base::Vector3::EZ();
            }

            // calculate the projected UV
            outA.x = base::Dot(axisU, a.Position) * scale;
            outA.y = base::Dot(axisV, a.Position) * scale;
            outB.x = base::Dot(axisU, b.Position) * scale;
            outB.y = base::Dot(axisV, b.Position) * scale;
            outC.x = base::Dot(axisU, c.Position) * scale;
            outC.y = base::Dot(axisV, c.Position) * scale;
        }
        ...
        void MeshGeometryBuilder::transform(const base::Matrix &transform)
        {
        ...
        void MeshGeometryBuilder::translate(const base::Vector3 &offset)
        {
            auto localToMesh = m_currentTransform.m_localToMesh;
        ...
        {
            translate(base::Vector3(x, y, z));
        }

        void MeshGeometryBuilder::rotate(const base::Quat &rotation)
        {
            auto transformMatrix = rotation.toMatrix();
            transform(transformMatrix);
        }

        void MeshGeometryBuilder::rotate(const base::Angles &rotation)
        {
        ...
        {
            rotate(base::Quat(base::Vector3::EX(), degs));
        }

        void MeshGeometryBuilder::rotateWorldY(float degs)
        {
            rotate(base::Quat(base::Vector3::EY(), degs));
        }

        void MeshGeometryBuilder::rotateWorldZ(float degs)
        {
            rotate(base::Quat(base::Vector3::EZ(), degs));
        }

        void MeshGeometryBuilder::scale(const base::Vector3 &scaling)
        {
        ...
        const base::RefPtr<GeometryChunkGroup> MeshGeometryBuilder::extractData() const
        {
            base::Task task(3, "");

            auto ret = base::CreateSharedPtr<GeometryChunkGroup>();
            ...
            // compute normals on all elements that require normal computation
            {
                ...
                {
                    base::Task task(numChunksToProcess, "Generating normals");
                    ...
                    for (uint32_t i = 0; i < m_allChunks.size(); ++i)
                    {
                        ...
                        if (chunk->m_setup.m_generateNormals)
                        {
                            base::Task task(1, base::TempString("Chunk {}", i));
            ...
            // compute tangents on all elements that require normal computation
            {
                ...
                if (numChunksToProcess > 0)
                {
                    base::Task task(numChunksToProcess, "Generating tangents");
            ...
            // build chunks
            {
                base::Task task(m_allChunks.size(), "");
                ...
                for (uint32_t i = 0; i < m_allChunks.size(); ++i)
                {
                    ...
                    // create target chunk
                    auto &targetChunk = ret->m_chunks.emplaceBack(base::CreateSharedPtr<GeometryChunk>());
                    targetChunk->m_material = base::StringID(m_allMaterials[srcChunk->m_setup.m_material]->name.c_str());

                    // pack vertex data
                    auto streamBuilder = base::CreateUniquePtr<MeshBuilderVertexMerger>(srcChunk->m_setup.m_streamMask);
        ...
        void MeshGeometryBuilder::generateNormals(Chunk& chunk, bool respectSmoothGroups, float maxSmoothAngle) const
        {
            ...
            for (auto face  = chunk.m_faces.head; face != nullptr; face = face->next)
            {
                ...
                for (uint32_t i = 0; i < 3; ++i)
                {
                    ...
                    base::Vector3 vn = face->m_normal * face->area;
                    ...
                    for (;otherFacesAtPoint.valid(); ++otherFacesAtPoint)
                    {
                        ...
                        if (otherFaceId != face->m_faceId)
                        {
                            ...
                            {
                                if (base::Dot(otherFace->m_normal, face->m_normal) > maxNormalSmoothAngleCos)
                                {
        ...
        void MeshGeometryBuilder::generateTangents(Chunk& chunk) const
        {
            ...
            for (auto face  = chunk.m_faces.head; face != nullptr; face = face->next)
            {
                ...
                for (uint32_t i = 0; i < 3; ++i)
                {
                    ...
                    base::Vector3 vn = face->vertices[i].Normal;
                    base::Vector3 vt = face->m_tangent * face->area;
                    base::Vector3 vb = face->m_bitangent * face->area;
                    ...
                    for (;otherFacesAtPoint.valid(); ++otherFacesAtPoint)
                    {
                        ...
                        if (otherFaceId != face->m_faceId)
                        {
                            ...
                            auto tDot = base::Dot(face->m_tangent, otherFace->m_tangent);
                            auto bDot = base::Dot(face->m_bitangent, otherFace->m_bitangent);
                    ...
                    vt = vt - vn*base::Dot(vt, vn);
                    vt.normalize();

                    // and the bitangent as well
                    vb = vb - vn*base::Dot(vb, vn) - vt*base::Dot(vt, vb);
                    vb.normalize();
