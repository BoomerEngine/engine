...
namespace base
{
    namespace shape
    {
        ...
        VozelizedShape::VozelizedShape(uint32_t gridSize)
        {
            voxels.resize(gridSize * gridSize * gridSize / 64);
            clear();
        }

        void VozelizedShape::clear()
        {
            memset(voxels.data(), 0, voxels.dataSize());
        }

        void VozelizedShape::insert(const VoxelizedGridSettings& grid, const IShape& shape)
        {
            if (shape.cls()->is<TriMesh>())
                return;

            auto shapeBounds = shape.calcBounds();
        ...
        void VozelizedShape::merge(const VozelizedShape& other)
        {
        ...
        void VozelizedShape::intersect(const VozelizedShape& other)
        {
            ASSERT(other.voxels.size() == voxels.size());
            auto ptr  = voxels.typedData();
            auto otherPtr  = other.voxels.typedData();
        ...
        void VozelizedShape::debugSave(const VoxelizedGridSettings& grid, const io::AbsolutePath& debugFile) const
        {
        ...
        bool VozelizedShape::calculateCenter(const VoxelizedGridSettings& grid, Vector3& outCenter, float& outVolume) const
        {
            auto centerX = 0ULL;
            auto centerY = 0ULL;
            auto centerZ = 0ULL;
        ...
        bool VozelizedShape::calculateInertiaTensor(const VoxelizedGridSettings& grid, const Vector3& comPosition, Matrix33& outTensor) const
        {
            double xx = 0.0;
            double yy = 0.0;
            double zz = 0.0;
