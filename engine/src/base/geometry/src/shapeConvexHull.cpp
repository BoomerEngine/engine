...
namespace base
{
    namespace shape
    {
        ...
        RTTI_BEGIN_TYPE_CLASS(ConvexHull);
        ...
        void ConvexHull::translate(const Vector3& offset)
        {
            auto numVertices = this->numVertices();
            auto vertex  = const_cast<Vertex*>(vertices());
            for (uint32_t i=0; i<numVertices; ++i, ++vertex)
                *vertex += offset;

            auto numPlanes = this->numPlanes();
            auto plane  = const_cast<Plane*>(planes());
            for (uint32_t i=0; i<numPlanes; ++i, ++plane)
                TranslatePlane(*plane, offset);
        }

        void ConvexHull::transform(const Matrix& currentToNew)
        {
            auto numVertices = this->numVertices();
            auto vertex  = const_cast<Vertex*>(vertices());
            for (uint32_t i=0; i<numVertices; ++i, ++vertex)
                *vertex = currentToNew.transformPoint(*vertex);

            TransformPlanes(const_cast<Plane*>(planes()), numPlanes(), currentToNew);
        }
        ...
        bool ConvexHull::buildFromMesh(const ISourceMeshInterface& mesh, float shinkBy /*= 0.0f*/, const Matrix* localToParent /*= nullptr*/)
        {
            base::Array<Vector3> transformedPoints;

            mesh.processTriangles([&transformedPoints, localToParent](const Vector3* vertices, uint32_t chunkIndex)
                                  {
                                    if (localToParent)
                                    {
                                        transformedPoints.pushBack(localToParent->transformPoint(vertices[0]));
                                        transformedPoints.pushBack(localToParent->transformPoint(vertices[1]));
                                        transformedPoints.pushBack(localToParent->transformPoint(vertices[2]));
                                    }
                                    else
                                    {
                                        transformedPoints.pushBack(vertices[0]);
                                        transformedPoints.pushBack(vertices[1]);
                                        transformedPoints.pushBack(vertices[2]);
                                    }
                                  });

            return buildFromVertices(transformedPoints.typedData(), transformedPoints.size(), shinkBy, nullptr);
        }

        static mem::PoolID POOL_CONVEX_HULL("Engine.ConvexHull");

        bool ConvexHull::buildFromVertices(const Vector3* points, uint32_t numPoints, float shinkBy, const Matrix* localToParent)
        {
            ...
            if (localToParent)
            {
                ...
                for (uint32_t i=0; i<numPoints; ++i)
                    transformedPoints[i] = localToParent->transformPoint(points[i]);

                hull.compute(transformedPoints.data(), false, sizeof(Vector3), numPoints);
            }
            else
            {
                hull.compute(points, false, sizeof(Vector3), numPoints);
            }
            ...
            memoryNeeded += sizeof(Edge) * buildEdges.size();
            ...
            // setup edges
            {
                ...
                auto edges  = AllocPtr<Edge>(basePtr, writePtr, buildEdges.size(), &header->edgeOffset);
        ...
        bool ConvexHull::intersect(const Vector3& origin, const Vector3& direction, float maxLength /*= VERY_LARGE_FLOAT*/, float* outEnterDistFromOrigin /*= nullptr*/, Vector3* outEntryPoint /*= nullptr*/, Vector3* outEntryNormal /*= nullptr*/) const
        {
        ...
        float ConvexHull::calcVolume() const
        {
            return 1.0f; // TODO
        }

        void ConvexHull::calcCRC(base::CRC64& crc) const
        {
//            m_data.calcHash(crc);
        }

        ShapePtr ConvexHull::copy() const
        {
            auto ret = CreateSharedPtr<ConvexHull>();
            ret->m_data = m_data;
            return ret;
        }

        uint32_t ConvexHull::calcDataSize() const
        {
            return sizeof(ConvexHull) + m_data.size();
        }

        Box ConvexHull::calcBounds() const
        {
            Box ret;
        ...
        void ConvexHull::render(IShapeRenderer& renderer, ShapeRenderingMode mode /*= ShapeRenderingMode::Solid*/, ShapeRenderingQualityLevel qualityLevel /*= ShapeRenderingQualityLevel::Medium*/) const
        {
            // TODO
        }
