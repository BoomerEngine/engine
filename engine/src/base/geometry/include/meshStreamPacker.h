...
namespace base
{
    namespace mesh
    {
        ...
        /// stream for packing
        struct BASE_GEOMETRY_API PackedStreamInfo
        {
        ...
        extern BASE_GEOMETRY_API PackedDataFormat GetPackedFormatForStream(MeshStreamType stream);

        /// get packed stream stride
        extern BASE_GEOMETRY_API uint32_t GetPackedStreamStride(PackedDataFormat format);

        /// calculate size needed to pack streams
        extern BASE_GEOMETRY_API uint64_t CalcPackedSize(const PackedStreamInfo* streams, uint32_t numStreams, uint64_t numElements);

        /// pack a data streams into a buffer, streams are packed as AOS
        extern BASE_GEOMETRY_API void PackStreams(const PackedStreamInfo* streams, uint32_t numStreams, uint64_t numElements, void* writePtr, void* writePtrEnd);
