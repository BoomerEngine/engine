...
#include "build.h"
#include "base/script_compiler/src/scriptFunctionCodeParserHelper.h"

#ifdef PLATFORM_MSVC
	#pragma warning ( disable: 4702 ) //: unreachable code
#endif

using FunctionParsingNode = base::script::FunctionParsingNode;
using Op = base::script::FunctionNodeOp;

static int bfc_lex(base::script::FunctionParsingNode* outNode, base::script::FunctionParsingContext& ctx, base::script::FunctionParsingTokenStream& tokens)
{
    return tokens.readToken(*outNode);
}

static int bfc_error(base::script::FunctionParsingContext& ctx, base::script::FunctionParsingTokenStream& tokens, const char* txt)
{
...
%define api.value.type {base::script::FunctionParsingNode}
%define parse.error verbose

%parse-param { base::script::FunctionParsingContext& context }
%parse-param { base::script::FunctionParsingTokenStream& tokens }

%lex-param { base::script::FunctionParsingContext& context }
%lex-param { base::script::FunctionParsingTokenStream& tokens }
...
variable_statement
    : var_statement_type var_names ';' {
        $$.node = $2.node;
        context.currentType = base::script::FunctionTypeInfo();
    }
    ;
