...
#define ZPREC 7
        ...
        Glyph* GlyphCache::buildGlyph(const FontStyleParams& styleParams, FT_Face faceData, FontID fontId, uint32_t ch)
        {
            ...
            if (faceData->glyph->bitmap.width > 0 && ch > ' ')
            {
                ...
                {
                    image::ImageView sourceView(base::image::NATIVE_LAYOUT, image::PixelFormat::Uint8_Norm, 1, faceData->glyph->bitmap.buffer, faceData->glyph->bitmap.width, faceData->glyph->bitmap.rows);
