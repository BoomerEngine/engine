...
namespace ui
{
    ...
    base::StringBuf TextBuffer::text() const
    {
        return text(0, m_lastPos);
    }

    base::StringBuf TextBuffer::text(int startPos, int endPos) const
    {
        // validate range
        ...
        for (auto pos = rangeStart; pos < rangeEnd; ++pos)
        {
            ...
            bufferSize += base::utf8::ConvertChar(temp, code);
        }

        // create the output buffer
        base::Array<char> tempBuffer;
        ...
        {
            auto code = m_chars[pos].m_code;
            writePtr += base::utf8::ConvertChar(writePtr, code);
        }
    ...
    void TextBuffer::style(const IElement* styleOwner)
    {
        ...
        if (const auto* stylePtr = styleOwner->evalStyleValueIfPresentPtr<base::Color>("color"_id))
            m_textColor = *stylePtr;

        if (const auto* stylePtr = styleOwner->evalStyleValueIfPresentPtr<base::Color>("selection"_id))
            m_selectionColor = *stylePtr;

        if (const auto* stylePtr = styleOwner->evalStyleValueIfPresentPtr<base::Color>("highlight"_id))
    ...
    void TextBuffer::insertCharacters(int pos, const base::StringBuf& txt)
    {
        TChars newChars;
    ...
    void TextBuffer::renderText(const ElementArea& drawArea, float pixelScale, base::canvas::GeometryBuilder& builder)
    {
        // generate the text
    ...
    void TextBuffer::generateChars(base::StringView txt, TChars& outChars) const
    {
        ...
        while (auto code = base::utf8::NextChar(cur, end))
        {
            if (code == '\r')
                continue;
    ...
    void TextBuffer::generateLayout()
    {
        ...
        // place the glyphs
        base::Vector2 placementPos(0.0f, ascenderLine);
        ...
        for (uint32_t i=0; i<3; ++i)
        {
            ...
            if (i == 1)
            {
                ...
                m_finalGlyph.m_bounds.m_boxMin = base::Vector2(placementPos.x, placementPos.y - ascenderLine);
                m_finalGlyph.m_bounds.m_boxMax = base::Vector2(placementPos.x, placementPos.y - descenderLine);
            }
        }
