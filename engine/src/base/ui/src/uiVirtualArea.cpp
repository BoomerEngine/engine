...
namespace ui
{
    ...
    base::ConfigProperty<int> cvVirtualAreaAutoScrollBorder("Editor.VirtualArea", "AutoScrollBorder", 100);
    base::ConfigProperty<float> cvVirtualAreaAutoScrollSpeed("Editor.VirtualArea", "AutoScrollSpeed", 800.0f);
    base::ConfigProperty<float> cvVirtualAreaAutoScrollPowerFactor("Editor.VirtualArea", "AutoScrollPowerFactor", 3.0f);
    ...
    void VirtualAreaElement::updateActualPosition(const base::Vector2& pos)
    {
    ...
    void VirtualAreaElement::virtualPosition(const VirtualPosition& pos, bool updateInContainer /*= true*/)
    {
        if (pos != m_virtualPosition)
        {
            ...
            {
                if (auto container = base::rtti_cast<VirtualArea>(parentElement()))
    ...
    void VirtualArea::attachChild(IElement* childElement)
    {
        ...
        if (auto ve = base::rtti_cast<VirtualAreaElement>(childElement))
        {
...
/*    void VirtualArea::moveElement(IVirtualAreaElement* elem, Position newVirtualPlacement)
    ...
    Position VirtualArea::absoluteToVirtual(const base::Point& absolutePos) const
    {
    ...
    VirtualAreaElement* VirtualArea::findElementAtAbsolutePos(const base::Point& absolutePos, float margin) const
    {
    ...
    VirtualArea::VirtualElementProxy* VirtualArea::findProxyAtAbsolutePos(const base::Point& absolutePos, float margin) const
    {
    ...
    bool VirtualArea::enumerateProxyAtAbsolutePos(const base::Point& absolutePos, const std::function<bool(VirtualElementProxy * proxy)>& enumFunc) const
    {
        auto actualPos = (absolutePos - cachedDrawArea().absolutePosition()) - m_viewOffset;
        for (auto* cur = m_drawOrderTail; cur; cur = cur->drawPrev)
        {
    ...
    void VirtualArea::select(const base::Array<const VirtualAreaElement*>& items, ItemSelectionMode mode /*= ItemSelectionModeBit::Default*/, bool postEvent /*= true*/)
    {
        base::InplaceArray<VirtualElementProxy*, 128> proxies;
    ...
    void VirtualArea::select(const VirtualAreaElement* item, ItemSelectionMode mode /*= ItemSelectionModeBit::Default*/, bool postEvent /*= true*/)
    {
        ...
        base::InplaceArray<VirtualElementProxy*, 1> proxies;
    ...
    void VirtualArea::selectArea(VirtualPosition tl, VirtualPosition br, ItemSelectionMode mode /*= ItemSelectionModeBit::Default*/, bool postEvent /*= true*/)
    {
        ...
        base::InplaceArray<VirtualElementProxy*, 256> proxies;
        for (auto* proxy : m_elementProxies)
        {
    ...
    void VirtualArea::applySelection(const base::Array<const VirtualAreaElement*>& newSelection, bool postEvent /*= true*/)
    {
    ...
    void VirtualArea::select(const base::Array<VirtualElementProxy*>& items, ItemSelectionMode mode /*= ItemSelectionModeBit::Default*/, bool postEvent /*= true*/)
    {
        // capture current list
        base::HashSet<const VirtualAreaElement*> currentSelectionSet;
        for (const auto* proxy : m_selection.keys())
            if (proxy && proxy->element)
                currentSelectionSet.insert(proxy->element);

        // build new selection list
        base::HashSet<const VirtualAreaElement*> selectionSet;
    ...
    void VirtualArea::scrollToOffset(base::Vector2 offset)
    {
    ...
    bool VirtualArea::autoScrollNearEdge(const base::Point& absolutePosition, float dt, base::Vector2* outViewScrollAmount /*= nullptr*/)
    {
        if (cvVirtualAreaAutoScrollBorder.get() > 1)
        {
            ...
            if (!innerArea.empty())
            {
                ...
                if (outViewScrollAmount)
                    *outViewScrollAmount = -base::Vector2(xSpeed * dt, ySpeed * dt);
                return true;
            }
        }
        ...
        /*{
            ...
            for (const auto* elem : m_elementProxies)
            {
                if (elem->visible && !elem->fresh)
                {
                    ...
                    b.strokeColor(base::Color::CYAN);
                    b.beginPath();
                    b.rect(elemDrawArea.tl(), elemDrawArea.br());
                    b.stroke();

                    b.strokeColor(base::Color::BLUE);
                    b.beginPath();
    ...
    void VirtualArea::processScheduledZoomChange()
    {
        if (1 == m_scheduledZoomChangeMode)
        {
            ...
            ui::Position virtualMinPos = Position::INF();
            ui::Position virtualMaxPos = -Position::INF();
            ...
            for (auto* proxy : m_elementProxies)
            {
                if (proxy->visible)
                {
                    if (proxy->sizeCached)
                    {
                        virtualMinPos = base::Min(proxy->virtualPosition, virtualMinPos);
                        virtualMaxPos = base::Max(proxy->virtualPosition + proxy->virtualSize, virtualMaxPos);
                    }
                    else
                    {
    ...
    bool VirtualArea::handleKeyEvent(const base::input::KeyEvent& evt)
    {
        if (evt.pressedOrRepeated() && evt.keyCode() == base::input::KeyCode::KEY_HOME)
        {
            zoomToFit();
            return true;
        }
    ...
    bool VirtualArea::handleMouseWheel(const base::input::MouseMovementEvent& evt, float delta)
    {
    ...
    InputActionPtr VirtualArea::handleOverlayMouseClick(const ElementArea& area, const base::input::MouseClickEvent& evt)
    {
        if (evt.leftClicked())
        {
    ...
    {
    public:
        VirtualAreaRectSelector(VirtualArea* area, const base::Point& initialPoint, bool shiftKey, bool ctrlKey, base::Color selectionColor)
            : MouseInputAction(area, base::input::KeyCode::KEY_MOUSE0)
    ...
    {
    public:
        VirtualAreaMoveSelectedBlocks(VirtualArea* area, const base::Point& initialPoint)
            : MouseInputAction(area, base::input::KeyCode::KEY_MOUSE0)
            , m_lastAbsolutePoint(initialPoint)
            , m_area(area)
        {
    ...
    bool VirtualArea::actionMoveElements(const base::Array<VirtualAreaElementPositionState>& oldPositions, const base::Array<VirtualAreaElementPositionState>& newPositions)
    {
        applyMove(newPositions);
        return true;
    }

    void VirtualArea::applyMove(const base::Array<VirtualAreaElementPositionState>& positionState)
    {
    ...
    void VirtualArea::updateBlockMove(base::Vector2 virtualDelta)
    {
        for (auto* proxy : m_selection)
        {
    ...
    void VirtualArea::finishBlockMove(bool apply /*= true*/)
    {
        if (apply)
        {
            base::InplaceArray<VirtualAreaElementPositionState, 256> oldPlacements, newPlacements;
