...
namespace wavefront
{
    ...
    base::res::ResourcePtr OBJPrefabImporter::importResource(base::res::IResourceImporterInterface& importer) const
    {
        ...
        static const auto meshComponentClass = RTTI::GetInstance().findClass("rendering::MeshComponent"_id);
        ...
        for (const auto& obj : prefabSourceObjects)
        {
...

            {
                ...
                componentTemplate->templateClass(meshComponentClass);
                componentTemplate->writeProperty("mesh"_id, obj.mesh->exportPath);
                componentTemplate->parent(node);
