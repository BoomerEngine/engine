...
namespace RigidMotionEstimator {
    ...
    Eigen::Affine3d point_to_point(Eigen::MatrixBase<Derived1>& X,
                                   ...
                                   const Eigen::MatrixBase<Derived3>& w, float* outDistance = nullptr) {
        ...
        TRACE_INFO("Xmean: [{},{},{}] YMean: [{},{},{}]", X_mean(0), X_mean(1), X_mean(2), Y_mean(0), Y_mean(1), Y_mean(2));
        X.colwise() -= X_mean;
        Y.colwise() -= Y_mean;
        /// Compute transformation
        Eigen::Affine3d transformation;
        Eigen::Matrix3d sigma = X * w_normalized.asDiagonal() * Y.transpose();
        Eigen::Matrix3d sigmaCopy = sigma;
        ...
        //transformation.linear().transposeInPlace();
        transformation.translation().noalias() = Y_mean - transformation.linear()*X_mean;
        //transformation.translation().noalias() = X_mean - transformation.linear().transpose() * Y_mean;
        TRACE_INFO( "Translation: [{},{},{}]", transformation.translation()(0), transformation.translation()(1), transformation.translation()(2));
        TRACE_INFO("Orientation: [{},{},{}] [{},{},{}] [{},{},{}]",
                transformation.linear().coeff(0,0), transformation.linear().coeff(0,1), transformation.linear().coeff(0,2),
                transformation.linear().coeff(1,0), transformation.linear().coeff(1,1), transformation.linear().coeff(1,2),
                transformation.linear().coeff(2,0), transformation.linear().coeff(2,1), transformation.linear().coeff(2,2));

            /// Apply transformation
        X = transformation*X;

        Eigen::Vector3d X2_mean;
        for(int i=0; i<3; ++i)
        {
            X2_mean(i) = (X.row(i).array() * w_normalized.transpose().array()).sum();
        }
        TRACE_INFO("X2mean: [{},{},{}]", X2_mean(0), X2_mean(1), X2_mean(2));
        ...
        /// Calculate distance
        if (nullptr != outDistance)
        {
            *outDistance = (float) ((X - Y).colwise().norm().maxCoeff());
            TRACE_INFO("Distance: {}", *outDistance);
        }
    ...
    inline Eigen::Affine3d point_to_point(Eigen::MatrixBase<Derived1>& X,
                                          Eigen::MatrixBase<Derived2>& Y, float* outDistance = nullptr) {
        return point_to_point(X, Y, Eigen::VectorXd::Ones(X.cols()), outDistance);
    }
    /// @param Source (one 3D point per column)
    /// @param Target (one 3D point per column)
...
namespace SICP {
    ...
    /// @param Parameters
    template <typename Derived1, typename Derived2>
    float point_to_point(Eigen::MatrixBase<Derived1>& X,
                        Eigen::MatrixBase<Derived2>& Y,
                        Parameters par,
        ...
        for(int icp=0; icp<par.max_icp; ++icp) {
            ...
            for(int outer=0; outer<par.max_outer; ++outer) {
                ...
                for(int inner=0; inner<par.max_inner; ++inner) {
                    ...
                    auto nextTransform = RigidMotionEstimator::point_to_point(X, U);
                    trasnform = trasnform * nextTransform;
                    /// Stopping criteria
            ...
            Xo2 = X;
            bestDistance = (float)stop;
            if(stop < par.stop) break;
        }

        outTransform = RigidMotionEstimator::point_to_point(Xo0, X);
        //outTransform = trasnform;
        return bestDistance;
    }
