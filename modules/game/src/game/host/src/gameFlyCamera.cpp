...
namespace game
{
    ...
    namespace config
    {
        base::ConfigProperty<float> cvCameraNormalSpeed("Game.FlyCamera", "CameraSpeed", 3.0f);
        base::ConfigProperty<float> cvCameraFastMultiplier("Game.FlyCamera", "FastCameraSpeedMultiplier", 5.0f);
        base::ConfigProperty<float> cvCameraSlowMultiplier("Game.FlyCamera", "SlowCameraSpeedMultiplier", 0.1f);
        base::ConfigProperty<float> cvCameraMaxAcceleration("Game.FlyCamera", "MaxAcceleration", 50.0f);
        base::ConfigProperty<float> cvCameraMaxDeacceleration("Game.FlyCamera", "MaxDeacceleration", 50.0f);
        base::ConfigProperty<float> cvCameraMouseSensitivity("Game.FlyCamera", "MouseSensitivity", 0.2f);
    }

    //---

    enum FlyCameraMovementKey
    {
        KEY_FOWARD,
        KEY_BACKWARD,
        KEY_LEFT,
    ...
    FlyCameraMovementKey MapMovementKey(const base::input::KeyCode keyCode)
    {
        switch (keyCode)
        {
        ...
        return FlyCameraMovementKey::KEY_MAX;
    }
    ...
    void FlyCamera::animate(float timeDelta)
    {
        computeMovement(timeDelta);
    }

    bool FlyCamera::processKeyEvent(const base::input::KeyEvent& evt)
    {
        const auto key = MapMovementKey(evt.keyCode());
        if (key != FlyCameraMovementKey::KEY_MAX)
        {
            if (evt.pressed())
    ...
    void FlyCamera::processMouseEvent(const base::input::MouseMovementEvent& evt)
    {
    ...
    void FlyCamera::moveTo(const base::Vector3& position, const base::Angles& rotation)
    {
        m_velocity = base::Vector3(0, 0, 0);
        m_rotation = rotation;
        m_position = position;
    ...
    void FlyCamera::computeMovement(float timeDelta)
    {
        // speed
        float speed = 1.0f;
        if (m_movementKeys[(int)FlyCameraMovementKey::KEY_FAST]) 
            speed *= config::cvCameraFastMultiplier.get();
        if (m_movementKeys[(int)FlyCameraMovementKey::KEY_SLOW]) 
    ...
    void FlyCamera::computeRenderingCamera(rendering::scene::CameraSetup& outCamera) const
    {
