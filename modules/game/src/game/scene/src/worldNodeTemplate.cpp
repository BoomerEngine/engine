...
namespace game
{
    ...
    ComponentDataTemplatePtr ComponentDataTemplate::copy() const
    {
        auto ret = base::CreateSharedPtr<ComponentDataTemplate>();
        ret->m_name = m_name;
        ret->m_componentClass = m_componentClass;
        ret->m_enabled = m_enabled;

        CopyProperties(m_parameters, ret->m_parameters, ret);
    ...
    ComponentPtr ComponentDataTemplate::createComponent() const
    {
        ...
        auto ret = m_componentClass->create<Component>();
        applyProperties(ret);
        return ret;
    }
    ...
    EntityDataTemplate::EntityDataTemplate()
        : m_enabled(true)
        , m_entityClass(Entity::GetStaticClass())
    {}
    ...
    base::RefPtr<Entity> EntityDataTemplate::compile() const
    {
        ...
        auto ret = entityClass->create<Entity>();
        applyProperties(ret);
        return ret;
    }

    EntityDataTemplatePtr EntityDataTemplate::copy() const
    {
        auto ret = base::CreateSharedPtr<EntityDataTemplate>();
        ret->m_enabled = m_enabled;
        ret->m_entityClass = m_entityClass;
    ...
    NodeTemplatePtr NodeTemplate::copy() const
    {
        auto ret = base::CreateSharedPtr<NodeTemplate>();
        ret->m_name = m_name;
        ret->m_placement = m_placement;
        ret->m_streamingModel = m_streamingModel;
        ret->m_streamingDistanceOverride = m_streamingDistanceOverride;
        ret->m_prefabAssets = m_prefabAssets;
    ...
    public:
        ...
        void addDependency(const PrefabRef& prefab, uint32_t dataVersion)
        {
            if (m_dependencies)
            {
                ...
                {
                    if (dep.m_sourcePrefab == prefab.acquire())
                    {
                ...
                newEntry.m_sourcePrefab = prefab.acquire();
        ...
        NodeTemplateContainerPtr prefabContent(const PrefabRef& prefab)
        {
            ...
            const auto& content = data->nodeContainer(&dataVersion);
