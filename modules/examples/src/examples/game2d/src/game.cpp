    ...
    void Game::createLevel()
    {
                ...
                if (asset)
                {
                    for (uint32_t y = 0; y < TERRAIN_HEIGHT; ++y)
                    {
                        ...
                        if (!code.c && code.b)
                        {
                            ...
                            auto sprite = RefNew<GameSprite>(pos, asset);
        ...
        // Cloud near
        {
            ...
            for (int i = 0; i < 20; ++i)
            {
                ...
                auto sprite = RefNew<GameSprite>(Vector2(x,y), asset);
        ...
        // Cloud far
        /*{
            const auto asset = RefNew<GameSpriteAsset>(LoadResource<Image>("/examples/canvas/cloud/cloud3.png"));
            for (int i = 0; i < 10; ++i)
            {
                float x = (TERRAIN_WIDTH * (rand() / (float)RAND_MAX)) * terrainAssets->tileSize();
