...
namespace rendering
{
	namespace api
	{
		namespace gl4
		{
			...
			base::ConfigProperty<uint32_t> cvUniformPoolBufferSize("Rendering.GL4", "UniformPoolBufferSize", 65536); // 64KB
			base::ConfigProperty<uint32_t> cvUniformPoolInitialCount("Rendering.GL4", "UniformPoolInitialCount", 64); // 4MB of uniforms - most games fit in this with ~4-5k rendered objects
			base::ConfigProperty<uint32_t> cvUniformPoolResizeCount("Rendering.GL4", "UniformPoolResizeCount", 16); // 1MB of extra uniforms each time we run out of space

			//--

			UniformBufferPool::UniformBufferPool()
				: m_owningThreadID(base::GetCurrentThreadID())
			{
				...
				m_bufferSize = std::clamp<uint32_t>(base::Align<uint32_t>(cvUniformPoolBufferSize.get(), bufferDataAlignment), 4096, maxBufferSize);
			...
			{
				DEBUG_CHECK_RETURN_EX_V(m_owningThreadID == base::GetCurrentThreadID(), "Calling from wrong thread", UniformBuffer());
				...
				base::mem::PoolStats::GetInstance().notifyAllocation(POOL_API_DYNAMIC_CONSTANT_BUFFER, m_bufferSize);
			...
			{
				DEBUG_CHECK_RETURN_EX(m_owningThreadID == base::GetCurrentThreadID(), "Calling from wrong thread");
				DEBUG_CHECK_RETURN_EX(m_allOwnedBuffers.contains(buffer.glBuffer), "Freeing buffer that does not belong to the pool");

				base::mem::PoolStats::GetInstance().notifyFree(POOL_API_DYNAMIC_CONSTANT_BUFFER, m_bufferSize);
			...
			void UniformBufferPool::createBufferBatch(uint32_t count)
			{
				...
				base::InplaceArray<GLuint, 32> glBuffers;
