...
namespace rendering
{
    namespace gl4
    {
        ...
        DriverThreadWinApi::DriverThreadWinApi(Driver* drv, WindowManager* windows)
            : DriverThread(drv, windows)
            , m_hRC(NULL)
            , m_hFakeDC(NULL)
        {
        ...
        DriverThreadWinApi::~DriverThreadWinApi()
        {

        }

        bool DriverThreadWinApi::initialize(const base::app::CommandLine& cmdLine)
        {
            bool result = false;
        ...
        void DriverThreadWinApi::shutdownContext_Thread()
        {
            if (NULL != m_hRC)
            {
        ...
        bool DriverThreadWinApi::initializeContext_Thread(const base::app::CommandLine& cmdLine)
        {
        ...
        ObjectID DriverThreadWinApi::createOutput(const DriverOutputInitInfo& info)
        {
            ...
            if (info.m_class == DriverOutputClass::NativeWindow || info.m_class == DriverOutputClass::Fullscreen)
            {
                ...
                // configure
                output->colorFormat = ImageFormat::RGBA8_UNORM;
                output->depthFormat = ImageFormat::D24S8;
                output->deviceHandle = (uint64_t)hDC;
                output->windowHandle = window;
            }
            else
            {
        ...
        void DriverThreadWinApi::releaseOutput(ObjectID id)
        {
            auto* output = ResolveStaticObject<DriverOutput>(id);
            DEBUG_CHECK_EX(output != nullptr, "Not an output object");

            if (output != nullptr)
            {
                // delete DC, if used
                if (output->deviceHandle)
                {
                    // make sure the DC is not active when we delete it :)
                    ...
                    // delete the DC
                    ReleaseDC((HWND)output->windowHandle, (HDC)output->deviceHandle);
                    output->deviceHandle = 0;
                }

                // delete window, if used
                if (output->windowHandle)
                {
                    // post window to close queue
                    postWindowForDeletion_Thread(output->windowHandle);
                    output->windowHandle = 0;
                }
            }
        }

        void DriverThreadWinApi::bindOutput(ObjectID id)
        {
            auto* output = ResolveStaticObject<DriverOutput>(id);
            DEBUG_CHECK_EX(output != nullptr, "Not an output object");

            if (output != nullptr && output->deviceHandle)
            {
                if (m_hActiveDC != (HDC)output->deviceHandle)
                {
                    PC_SCOPE_LVL0(wglMakeCurrent);
        ...
        void DriverThreadWinApi::swapOutput(ObjectID id)
        {
            auto* output = ResolveStaticObject<DriverOutput>(id);
            DEBUG_CHECK_EX(output != nullptr, "Not an output object");
